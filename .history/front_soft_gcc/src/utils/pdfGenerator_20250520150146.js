...
// Suppression de l'import direct qui cause l'erreur
// Le logo sera ajouté via une fonction qui gère l'absence potentielle du fichier
...
export const generateEvaluationPDF = (
  ...
  validationData,
  trainingSuggestions = [],
  logoBase64 = null // Ajout d'un paramètre optionnel pour le logo en base64
) => {
  ...
  // Logo - Gérer de manière plus robuste l'ajout du logo
  if (logoBase64) {
    // Si un logo en base64 est fourni, l'utiliser
    try {
      const imgWidth = 30; // Largeur du logo en mm
      doc.addImage(logoBase64, 'PNG', margin, margin, imgWidth, imgWidth * 0.5);
    } catch (error) {
      console.error("Erreur lors du chargement du logo fourni en base64:", error);
    }
  } else {
...
/**
 * Génère un PDF d'évaluation et retourne son URL de données
 ...
 * @param {Array} trainingSuggestions - Suggestions de formation
 * @param {String} logoBase64 - Logo en format base64 (optionnel)
 * @returns {{dataUrl: String, fileName: String}} Objet contenant l'URL de données du PDF et le nom du fichier
 */
export const getEvaluationPDFDataUrl = (
  evaluationData,
  employeeData,
  ratings,
  average,
  ...
  return { dataUrl, fileName, doc }; // Retourner aussi le doc pour le téléchargement
};
