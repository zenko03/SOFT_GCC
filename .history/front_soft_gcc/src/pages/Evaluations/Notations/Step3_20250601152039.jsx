...
const Step3 = ({ ratings, average, evaluationId, validationData, onValidationChange }) => {
  ...
  const [evaluationTypeInfo, setEvaluationTypeInfo] = useState(null);
  const [competenceNames, setCompetenceNames] = useState({});
  
  // Référence pour éviter les appels multiples à l'API
  const isApiCalled = useRef(false);

  // Récupérer les données de l'employé et d'évaluation une fois au chargement
  useEffect(() => {
      ...
      try {
        ...
        try {
          ...
          if (evaluationResponse.data) {
            ...
            if (evaluationResponse.data.evaluationTypeId) {
              // Déterminer le type d'évaluation (1 = Annuelle, 2 = Recrutement, etc.)
              let typeLabel = "Non spécifié";
              switch (evaluationResponse.data.evaluationTypeId) {
                case 1:
                  typeLabel = "Évaluation annuelle";
                  break;
                case 2:
                  typeLabel = "Évaluation de recrutement";
                  break;
                case 3:
                  typeLabel = "Évaluation de compétences";
                  break;
                default:
                  typeLabel = `Type ${evaluationResponse.data.evaluationTypeId}`;
              }
              
              setEvaluationTypeInfo({
                id: evaluationResponse.data.evaluationTypeId,
                label: typeLabel
              });
            }
  ...
  // Récupérer les questions complètes et commentaires
  useEffect(() => {
    const fetchQuestionsAndComments = async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn("Impossible de récupérer les commentaires:", err);
        }
      } catch (err) {
        console.error("Erreur lors de la récupération des questions et commentaires:", err);
      }
    };
  ...
  useEffect(() => {
    const fetchCompetenceNames = async () => {
      try {
        // Essayer plusieurs endpoints possibles
        let competencesData = null;
        let apiUrls = [
          'https://localhost:7082/api/CompetenceLine',
          'https://localhost:7082/api/Competence/lines',
          'https://localhost:7082/api/Competence',
          'https://localhost:7082/api/CompetenceLines'
        ];
        
        for (const url of apiUrls) {
          try {
            console.log(`Tentative de récupération des compétences depuis: ${url}`);
            const response = await axios.get(url);
            if (response.data && (Array.isArray(response.data) || typeof response.data === 'object')) {
              competencesData = response.data;
              console.log(`Compétences récupérées depuis: ${url}`, competencesData);
              break;
            }
          } catch (err) {
            console.warn(`Échec de récupération depuis ${url}: ${err.message}`);
          }
        }
        
        if (!competencesData) {
          // Si aucun endpoint ne fonctionne, récupérer les compétences à partir des questions elles-mêmes
          console.log('Tentative de récupération des compétences depuis les questions');
          const competenceIds = selectedQuestions
            .map(q => q.competenceLineId)
            .filter((id, index, self) => id && self.indexOf(id) === index);
          
          if (competenceIds.length > 0) {
            try {
              const response = await axios.post('https://localhost:7082/api/Competence/batch', competenceIds);
              if (response.data) {
                competencesData = response.data;
                console.log('Compétences récupérées par batch:', competencesData);
              }
            } catch (err) {
              console.warn('Échec de récupération par lots: ', err.message);
            }
          }
        }
        ...
        if (competencesData) {
          ...
          // Traiter selon que les données sont un tableau ou un objet
          if (Array.isArray(competencesData)) {
            competencesData.forEach(comp => {
              console.log('Traitement de la compétence:', comp);
              const id = comp.competenceLineId || comp.id || comp.competenceId;
              // Vérifier toutes les propriétés possibles avec différentes casses
              const name = comp.Description || comp.description || comp.Name || 
                          comp.name || comp.competenceName || comp.CompetenceName || 
                          comp.label || `Compétence ${id}`;
              if (id) {
                namesMap[id] = name;
                console.log(`Mapping créé: ${id} => ${name}`);
              }
            });
          } else {
            // Si c'est un objet, parcourir les propriétés
            Object.entries(competencesData).forEach(([key, comp]) => {
              console.log('Traitement de la compétence (objet):', key, comp);
              const id = parseInt(key) || comp.competenceLineId || comp.id || comp.competenceId;
              const name = comp.Description || comp.description || comp.Name || 
                          comp.name || comp.competenceName || comp.CompetenceName || 
                          comp.label || `Compétence ${id}`;
              if (id) {
          ...
          // Créer un mapping manuel basé sur les données fournies
          const compLinesData = [
            { id: 23, name: 'Développement d\'applications complètes avec React et APIs REST' },
            { id: 25, name: 'Création d\'applications web complètes avec Django et templates modernes' },
            // Ajouter plus d'entrées si nécessaire...
          ];
          
          compLinesData.forEach(item => {
            namesMap[item.id] = item.name;
          });
          
          setCompetenceNames(namesMap);
          console.log('Mapping final des compétences:', namesMap);
        } else {
          console.error('Aucune donnée de compétence trouvée');
          
          // Créer un mapping de fallback pour les compétences connues
          const fallbackMap = {
            1: 'Maîtrise avancée de JavaScript ES6+',
            2: 'Développement avec React',
            3: 'Création d\'applications avec Angular',
            4: 'Développement avec Vue.js',
            5: 'Gestion de versions Git',
            10: 'Développement Python',
            21: 'Développement Full Stack',
            23: 'Développement d\'applications avec React et APIs REST',
            25: 'Applications web avec Django'
          };
          
          // Pour chaque question, s'assurer qu'on a une description
          selectedQuestions.forEach(question => {
            if (question.competenceLineId && !fallbackMap[question.competenceLineId]) {
              fallbackMap[question.competenceLineId] = `Compétence ${question.competenceLineId}`;
            }
          });
          
          setCompetenceNames(fallbackMap);
          console.log('Utilisation du mapping de fallback:', fallbackMap);
        }
      } catch (err) {
        console.error('Erreur lors de la récupération des noms de compétences:', err);
        
        // Utiliser un mapping fallback en cas d'erreur
        const fallbackMap = {};
        selectedQuestions.forEach(question => {
          fallbackMap[question.competenceLineId] = `Compétence ${question.competenceLineId}`;
        });
  ...
  const handlePreviewPDF = async () => {
    try {
      ...
      // Préparer l'appréciation basée sur la note moyenne
      let appreciation = '';
      if (average >= 4.5) appreciation = 'Excellent';
      else if (average >= 4) appreciation = 'Très bien';
      else if (average >= 3) appreciation = 'Bien';
      else if (average >= 2) appreciation = 'Passable';
      ...
      const evaluationDataForPDF = {
        ...
        type: evaluationTypeInfo ? evaluationTypeInfo.label : 'Non spécifié',
        typeId: evaluationTypeInfo ? evaluationTypeInfo.id : null,
        appreciation: appreciation
      };

      // Préparation des données de questions avec texte complet
      const questionsWithText = {};
      selectedQuestions.forEach(question => {
        // Récupérer le nom de la compétence à partir de l'ID
        const competenceName = competenceNames[question.competenceLineId] || 
                              `Compétence ${question.competenceLineId}`;
        
        questionsWithText[question.questionId] = {
          text: question.text,
          competenceName: competenceName, // Utiliser le nom récupéré
          competenceLineId: question.competenceLineId,
          note: ratings[question.questionId] || 0,
          comment: comments[question.questionId] || 'Pas de commentaire',
          responseType: question.responseType
        };
      });
      
      console.log('Données des questions envoyées au PDF:', questionsWithText);

      // Générer l'URL de prévisualisation du PDF (fonction asynchrone)
      const url = await previewEvaluationPDF(
