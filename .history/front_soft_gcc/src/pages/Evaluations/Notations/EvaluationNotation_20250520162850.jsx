...
const EvaluationNotation = () => {
  ...
  useEffect(() => {
    // Ne générer le PDF que si la validation est réussie et que l'utilisateur a explicitement demandé le PDF
    if (validationSuccess && !isGeneratingPDF && userRequestedPdf && !pdfGenerationRequested) {
      setIsGeneratingPDF(true);
      setPdfGenerationRequested(true);
      
      console.log("Démarrage du processus de génération PDF suite à une demande utilisateur");
      
      if (generatePDFFunc) {
        // Attendre un court instant avant de générer le PDF pour éviter le téléchargement immédiat
        const pdfTimer = setTimeout(() => {
          console.log("Génération du PDF...");
          generatePDFFunc();
        }, 1000);
        
        // Redirection après le délai de visualisation du message de succès
  ...
  }, [validationSuccess, navigate, generatePDFFunc, isGeneratingPDF, pdfGenerationRequested, userRequestedPdf]);
  
  // Réinitialiser l'état de génération PDF si l'étape change
  useEffect(() => {
  ...
  const saveEvaluationResults = async () => {
    ...
    try {
      ...
      // Pas besoin d'afficher une alerte
      ...
      // Garder une alerte d'erreur pour informer l'utilisateur
