...
const EvaluationNotation = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (employeeId) {
          ...
          console.log('Données employé reçues:', employeeResponse.data);
          console.log('Structure des données employé:', {
          ...
          // Vérifier si la structure est correcte et extraire les informations
          if (employeeResponse.data) {
            setSelectedEmployee({
              firstName: employeeResponse.data.FirstName || '',
              lastName: employeeResponse.data.LastName || '',
              // Essayer différentes propriétés possibles pour le poste et département
              position: employeeResponse.data.Position || employeeResponse.data.position_name || '',
              department: employeeResponse.data.Department || employeeResponse.data.department_name || '',
              evaluationId: employeeResponse.data.EvaluationId || evaluationId
            });
          }
        }
        // Sinon, essayer de récupérer l'évaluation directement
          ...
          console.log('Données évaluation reçues:', evaluationResponse.data);
          console.log('Structure des données évaluation:', {
            employeeName: evaluationResponse.data.employeeName,
            position: evaluationResponse.data.position,
            positionName: evaluationResponse.data.position_name || evaluationResponse.data.Position,
            department: evaluationResponse.data.department,
            departmentName: evaluationResponse.data.department_name || evaluationResponse.data.Department
          });
          
          // Extraire les informations de l'employé depuis l'évaluation
          if (evaluationResponse.data) {
            // Récupérer les informations d'une manière adaptative
            const firstName = evaluationResponse.data.firstName || 
                              evaluationResponse.data.FirstName || 
                              (evaluationResponse.data.employeeName ? evaluationResponse.data.employeeName.split(' ')[0] : '');
                              
            const lastName = evaluationResponse.data.lastName || 
                             evaluationResponse.data.LastName || 
                             (evaluationResponse.data.employeeName ? evaluationResponse.data.employeeName.split(' ').slice(1).join(' ') : '');
            
            setSelectedEmployee({
              firstName: firstName,
              lastName: lastName,
              position: evaluationResponse.data.position || 
                        evaluationResponse.data.Position || 
                        evaluationResponse.data.position_name || '',
              department: evaluationResponse.data.department || 
                          evaluationResponse.data.Department || 
                          evaluationResponse.data.department_name || '',
              evaluationId: evaluationId
            });
            
            // Si l'employéId est disponible et que nous n'avons pas de poste/département, essayons de récupérer directement
            if (evaluationResponse.data.userId && 
                (!evaluationResponse.data.position && !evaluationResponse.data.Position && 
                 !evaluationResponse.data.department && !evaluationResponse.data.Department)) {
              try {
                const employeeIdFromEval = evaluationResponse.data.userId;
                const employeeResponse = await axios.get(`https://localhost:7082/api/User/${employeeIdFromEval}`);
                console.log('Données employé complémentaires:', employeeResponse.data);
                
                // Mettre à jour avec les infos supplémentaires
                setSelectedEmployee(prev => ({
                  ...prev,
                  position: employeeResponse.data.Position || employeeResponse.data.position_name || prev.position,
                  department: employeeResponse.data.Department || employeeResponse.data.department_name || prev.department
                }));
              } catch (employeeError) {
                console.error('Erreur lors de la récupération des données employé complémentaires:', employeeError);
              }
            }
          }
        }
  ...
  const validateEvaluation = async () => {
    const payload = {
      ...
      serviceApprovalDate: validationData.serviceDate,
      dgApprovalDate: validationData.dgDate
    };
          ...
          <>
            <div className="header-section">
              ...
              <div className="debug-info" style={{ background: '#f8f9fa', padding: '10px', margin: '10px 0', borderRadius: '5px', fontSize: '12px' }}>
                ...
                <p>evaluationId: {selectedEmployee?.evaluationId}</p>
                <p>Variable: {JSON.stringify(selectedEmployee)}</p>
              </div>
            </div>
