...
const Step3 = ({ ratings, average, evaluationId, validationData, onValidationChange, onGeneratePDF }) => {
  ...
  useEffect(() => {
    // Éviter d'enregistrer la fonction plusieurs fois
    if (onGeneratePDF && !pdfFunctionRegistered) {
      // Passer une fonction wrapper qui ne s'exécutera que sur demande explicite
      onGeneratePDF(() => {
        console.log("Fonction de génération PDF appelée explicitement");
        return generatePDF();
      });
      
      // Marquer que la fonction a été enregistrée pour éviter les appels multiples
      setPdfFunctionRegistered(true);
    }
    // Dépendance uniquement sur onGeneratePDF et le drapeau
