...
const Step3 = ({ ratings, average, evaluationId, validationData, onValidationChange, onGeneratePDF }) => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      // Force la génération du PDF même si pas explicitement demandé par l'utilisateur
      // si appelé explicitement via la fonction passée au parent
      const forcePdfGeneration = userRequestedPdf === true;
      
      // Générer et télécharger le PDF
      if (forcePdfGeneration) {
        console.log("Génération du PDF à la demande");
  ...
  useEffect(() => {
    ...
    const pdfGenerationHandler = () => {
      console.log("Fonction de génération PDF appelée depuis le parent");
      // Forcer la génération du PDF même si userRequestedPdf n'est pas défini
      setUserRequestedPdf(true);
      return generatePDF();
    };
    
    // Enregistrer immédiatement la fonction pour être sûr qu'elle est disponible
