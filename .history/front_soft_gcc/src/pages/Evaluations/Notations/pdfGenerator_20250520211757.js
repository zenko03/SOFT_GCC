...
export const downloadEvaluationPDF = (
  ...
  try {
    ...
    const fileName = `Evaluation_${safeEvaluationData.evaluationId}_${new Date().toISOString().slice(0, 10)}.pdf`;
    
    // Créer le document PDF avec contenu simplifié pour garantir le téléchargement
    const doc = new jsPDF({
      orientation: 'portrait',
      unit: 'mm',
      format: 'a4'
    ...
    // Ajouter un contenu minimal pour garantir que le PDF n'est pas vide
    doc.setFontSize(18);
    doc.text('Évaluation des compétences', 105, 20, { align: 'center' });
    
    // Ajouter le contenu au PDF et créer le blob
    const docBlob = doc.output('blob');
    
    // Utiliser l'API File System pour télécharger
    ...
    setTimeout(() => {
      ...
      // Forcer un second téléchargement avec le PDF complet si le premier échoue
      try {
        // Générer complètement le PDF avec tous les détails
        ...
        console.error("Erreur lors de la génération du PDF détaillé:", detailError);
        alert("Le PDF simplifié a été téléchargé. Le PDF détaillé n'a pas pu être généré.");
      }
    }, 300);
    ...
    // Essayer une méthode de secours extrêmement simple
    try {
      alert("Problème lors de la génération du PDF. Une version simplifiée va être téléchargée.");
      
      const doc = new jsPDF();
      doc.text('Rapport d\'évaluation', 10, 10);
      
      if (employeeData) {
        doc.text(`Employé: ${employeeData.firstName} ${employeeData.lastName}`, 10, 20);
      }
      
      if (average) {
        doc.text(`Note moyenne: ${average}/5`, 10, 30);
      }
      
      const simpleFileName = `Evaluation_simplifiée_${new Date().getTime()}.pdf`;
      doc.save(simpleFileName);
      return simpleFileName;
    } catch (fallbackError) {
      console.error("Même la méthode de secours a échoué:", fallbackError);
      return null;
    }
  } finally {
    // Libérer le verrou après un court délai pour éviter les conflits
    setTimeout(() => {
