    ...
    const fetchData = async () => {
      try {
        ...
        // Sinon, essayer de récupérer l'évaluation directement
          ...
          if (evaluationResponse.data && evaluationResponse.data.employee) {
            setSelectedEmployee({
              firstName: evaluationResponse.data.employee.firstName || '',
              lastName: evaluationResponse.data.employee.lastName || '',
              position: evaluationResponse.data.employee.position || '',
              department: evaluationResponse.data.employee.department || '',
              evaluationId: evaluationId
            });
          } else {
            // Si l'employé n'est pas dans l'évaluation, récupérer l'employé à partir de employeeId dans l'évaluation
            const employeeId = evaluationResponse.data.employeeId;
            if (employeeId) {
              const employeeResponse = await axios.get(`https://localhost:7082/api/User/${employeeId}`);
