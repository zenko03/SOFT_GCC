...
import { downloadEvaluationPDF } from './pdfGenerator';
...
const EvaluationNotation = () => {
  ...
  // Référence pour la fonction de génération PDF
  const pdfGenerationFunctionRef = useRef(null);
  ...
  // Stocke la référence à la fonction de génération PDF
  useEffect(() => {
    console.log("Mise à jour de la référence à la fonction de génération PDF:", generatePDFFunc);
    if (generatePDFFunc && typeof generatePDFFunc === 'function') {
      // Stocker la fonction telle quelle, sans l'exécuter
      pdfGenerationFunctionRef.current = generatePDFFunc;
      console.log("Fonction de génération PDF stockée en référence");
    }
  }, [generatePDFFunc]);
  ...
  const generatePDFAfterValidation = () => {
    ...
    // Approche 1: Essayer d'utiliser la fonction stockée en référence
    if (pdfGenerationFunctionRef.current && typeof pdfGenerationFunctionRef.current === 'function') {
      try {
        console.log("*** Exécution de la fonction de génération PDF via référence ***");
        
        // Récupérer la fonction
        let pdfFunction = pdfGenerationFunctionRef.current;
        
        // IMPORTANT: N'extraire la fonction interne QU'UNE SEULE FOIS
        // pour éviter les appels multiples
        let extractedFunction = null;
        
        try {
          // Si la fonction retourne une fonction, extraire cette fonction
          extractedFunction = pdfFunction();
          if (typeof extractedFunction === 'function') {
            console.log("*** La fonction renvoie une autre fonction, extraction... ***");
            // Appeler la fonction extraite
            const result = extractedFunction();
            console.log("*** Résultat de la génération du PDF:", result, "***");
            return result !== null;
          } else {
            // Si ce n'est pas une fonction, utiliser la fonction originale directement
            console.log("*** Type retourné:", typeof extractedFunction, "utilisation de la fonction originale ***");
            const result = pdfFunction();
            console.log("*** Résultat de la génération du PDF:", result, "***");
            return result !== null;
          }
        } catch (funcError) {
          console.error("*** Erreur lors de l'extraction/exécution de la fonction:", funcError, "***");
          // Continuer avec l'approche directe
        }
      } catch (error) {
        console.error("*** Erreur lors de la génération du PDF via référence:", error, "***");
        // On continue avec l'approche alternative si celle-ci échoue
      }
    } else {
      console.warn("*** Fonction de génération PDF non disponible via référence, passage à la méthode directe ***");
    }
    
    // Approche 2: Méthode directe - Générer le PDF directement ici
    try {
      ...
      const fileName = downloadEvaluationPDF(
        ...
        averageRating,
        validationData,
        [] // Pas de suggestions de formation dans cette méthode directe
      );
  ...
  const handleNextStep = async () => {
    if (currentStep === 3) {
      ...
      try {
        ...
        console.log("Validation réussie, attente de la génération PDF");
        // Le changement d'état validationSuccess va déclencher l'useEffect qui gère la génération du PDF
