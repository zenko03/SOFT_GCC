...
export const downloadEvaluationPDF = (
  ...
  try {
    ...
    try {
      ...
      // Ajouter des suggestions de formation si disponibles
      if (trainingSuggestions && trainingSuggestions.length > 0) {
        yPos += 10;
        // Vérifier s'il faut ajouter une page
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(14);
        doc.text('Suggestions de formation:', 20, yPos);
        yPos += 10;
        ...
        trainingSuggestions.forEach(suggestion => {
          ...
          doc.text(`${suggestion.question || 'Domaine'}: ${suggestion.training}`, 20, yPos);
          yPos += 8;
        });
      }
