...
import './Step1.css';
import { FaInfoCircle, FaTimes, FaLightbulb, FaCheck, FaTimes as FaX } from 'react-icons/fa';
...
const Step1 = ({ evaluationId, setRatings }) => {
        ...
        formattedQuestions.forEach(question => {
          ...
          if (question.response) {
            ...
            if (question.responseType === 'QCM') {
              ...
              newRatings[question.questionId] = isCorrect ? 5 : 0;
  ...
  // Nouvelle fonction pour ouvrir le modal de référence
  const openReferenceModal = (questionId) => {
    setActiveReferenceModal(questionId);
  };

  // Nouvelle fonction pour fermer le modal de référence
  ...
  return (
    <div className="step1-container">
      ...
      {/* Modal pour afficher les réponses de référence */}
      {activeReferenceModal !== null && references[activeReferenceModal] && (
        <div className="reference-modal-overlay" onClick={closeReferenceModal}>
          <div className="reference-modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="reference-modal-header">
              <h3>
                <FaLightbulb className="reference-icon" /> Réponse de référence
              </h3>
              <button className="reference-close-btn" onClick={closeReferenceModal}>
                <FaTimes />
              </button>
            </div>
            <div className="reference-modal-body">
              <p className="reference-text">{references[activeReferenceModal].referenceText}</p>
              {references[activeReferenceModal].keyPoints && (
                <div className="key-points">
                  <h6>Points clés attendus :</h6>
                  {renderKeyPoints(references[activeReferenceModal].keyPoints)}
                </div>
              )}
            </div>
          </div>
      ...
      {selectedQuestions.length > 0 ? (
        <div className="questions-section">
          ...
          {selectedQuestions.map((question, index) => (
            <div key={`question-${question.questionId}-${index}`} className={`question-item ${evaluationType === 1 && question.responseType === 'QCM' ? 'annual-qcm-question' : ''}`}>
              <div className="question-response-section">
                <div className="question-header">
                  ...
                  <div className="question-rating-container">
                    {/* Bouton pour afficher la réponse de référence */}
                    {references[question.questionId] && !(evaluationType === 1 && question.responseType === 'QCM') && (
                      <button 
                        className="reference-btn" 
                        onClick={() => openReferenceModal(question.questionId)}
                        title="Voir la réponse de référence"
                      >
                        <FaInfoCircle />
                      </button>
                    )}
                        ...
                        <div className="qcm-response">
                          {(() => {
                            ...
                            if (questionOptions[question.questionId] && !isNaN(optionId)) {
                              ...
                              if (option) {
                                // Pour les évaluations annuelles (type 1), afficher simplement la réponse sans indication
                                if (evaluationType === 1) {
                                  return (
                                    <p className="annual-qcm-response">{option.optionText}</p>
                                  );
                                } 
                                // Pour les autres types d'évaluation, conserver l'affichage original
                                else {
                                  return (
                                    <>
