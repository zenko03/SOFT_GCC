...
const Step3 = ({ ratings, average, evaluationId, validationData, onValidationChange }) => {
  ...
  // Chargement des résultats par compétence
  useEffect(() => {
    if (evaluationId) {
      fetchCompetenceResults();
    }
  }, [evaluationId]);

  // Fonction pour récupérer les résultats par compétence
  const fetchCompetenceResults = async () => {
    setLoadingCompetences(true);
    setCompetenceError('');
    try {
      // D'abord calculer les résultats par compétence
      console.log(`Calcul des résultats par compétence pour l'évaluation ${evaluationId}...`);
      await axios.post(`https://localhost:7082/api/EvaluationCompetence/calculate/${evaluationId}`);
      
      // Ensuite récupérer les résultats
      console.log(`Récupération des résultats par compétence pour l'évaluation ${evaluationId}...`);
      const response = await axios.get(`https://localhost:7082/api/EvaluationCompetence/evaluation/${evaluationId}`);
      console.log('Résultats par compétence:', response.data);
      setCompetenceResults(response.data);
    } catch (err) {
      console.error('Erreur lors de la récupération des résultats par compétence:', err);
      setCompetenceError(`Impossible de récupérer les résultats par compétence: ${err.response?.data?.message || err.message}`);
    } finally {
  ...
  // Fonction pour calculer la différence entre la note globale et la note par compétence
  const getScoreDifference = (competenceScore) => {
    const diff = competenceScore - parseFloat(average);
    return diff.toFixed(2);
  };

  // Fonction pour obtenir le symbole de tendance
  ...
  const getTrendClass = (diff) => {
    if (diff > 0.2) return 'trend-up';
    if (diff < -0.2) return 'trend-down';
    return 'trend-same';
  };
  ...
  return (
    <div className="step3-container">
      ...
      {/* Nouvelle section: Résultats par compétence */}
      <div className="competence-results-section">
        <h4>Résultats par compétence</h4>
        {loadingCompetences ? (
          <div className="text-center mt-3">
            <div className="spinner-border text-primary" role="status">
              <span className="visually-hidden">Chargement...</span>
            </div>
            <p className="mt-2">Chargement des résultats par compétence...</p>
          </div>
        ) : competenceError ? (
          <div className="alert alert-danger mt-3" role="alert">
            ...
            <div className="mt-2">
              <button 
                ...
                onClick={fetchCompetenceResults}
              >
                Réessayer
              </button>
              {competenceError.includes("Null") && (
                <p className="mt-2 small">
                  <strong>Info :</strong> Les résultats doivent d&apos;abord être calculés. Ce problème peut survenir si l&apos;évaluation vient d&apos;être finalisée.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="competence-cards">
            {competenceResults.map(comp => {
              const scoreDiff = getScoreDifference(comp.score);
              const trendIcon = getTrendIcon(scoreDiff);
              const trendClass = getTrendClass(scoreDiff);
              
              return (
                <div key={comp.competenceId} className={`competence-card ${getScoreClass(comp.score)}`}>
                  <div className="competence-header">
                    <h5 className="competence-name">{comp.competenceName}</h5>
                    <div className={`competence-score ${getScoreClass(comp.score)}`}>
                      {comp.score.toFixed(2)}
                    </div>
                  </div>
                  <div className="competence-body">
                    <p className="competence-description">{comp.description}</p>
                    <div className={`comparison-badge ${trendClass}`}>
                      <span className="trend-icon">{trendIcon}</span>
                      <span className="diff-value">
                        {scoreDiff > 0 ? '+' : ''}{scoreDiff}
                      </span>
                      <span className="vs-global">vs. moyenne globale</span>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="alert alert-info mt-3" role="alert">
            Aucun résultat par compétence disponible pour cette évaluation.
            <p className="small mb-2">Causes possibles :</p>
            <ul className="small">
              <li>L&apos;évaluation n&apos;est pas encore terminée</li>
              <li>Les questions évaluées ne sont pas associées à des compétences</li>
              <li>Une erreur technique est survenue lors du calcul</li>
            </ul>
            <button 
      ...
      <div className="validation-section">
        ...
        <p className="validation-instructions">
          Cette étape finalise l&apos;évaluation. En cliquant sur &quot;Valider l&apos;évaluation&quot;, les notes et commentaires seront 
          définitivement enregistrés, et les résultats par compétence seront automatiquement calculés. Une fois validée, 
          l&apos;évaluation ne pourra plus être modifiée.
        </p>
        <div className="alert alert-info">
          <strong>Information :</strong> Le score global de l&apos;évaluation est automatiquement attribué à toutes les compétences évaluées.
          Les résultats par compétence seront disponibles après la validation finale.
        </div>
