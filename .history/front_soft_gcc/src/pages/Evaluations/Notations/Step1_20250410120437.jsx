...
const Step1 = ({ evaluationId, setRatings, evaluationTypes }) => {
      ...
      try {
        ...
        if (evaluationResponse.data && evaluationResponse.data.evaluationTypeId) {
          ...
          if (evalType) {
            ...
            // Charger les questions sélectionnées pour cette évaluation
            try {
              ...
              console.log('Questions sélectionnées reçues:', selectedQuestionsResponse.data);
              ...
              if (selectedQuestionsResponse.data && selectedQuestionsResponse.data.questions) {
                ...
                setQuestions(selectedQuestionsResponse.data.questions);
              }
            } catch (error) {
              console.error('Erreur lors du chargement des questions sélectionnées:', error);
              setError('Impossible de charger les questions sélectionnées');
            }
          }
        }
  ...
  const handleEvaluationTypeChange = async (e) => {
    ...
    try {
      // Charger les questions sélectionnées pour ce type d'évaluation
      ...
      setError('Impossible de charger les questions pour ce type d\'évaluation');
    } finally {
      setLoading(false);
    }
  };
            ...
            <div key={question.questionId} className="question-item">
              ...
              <div className="rating-buttons">
                {[1, 2, 3, 4, 5].map((rating) => (
                  <button
                    key={`${question.questionId}-${rating}`}
