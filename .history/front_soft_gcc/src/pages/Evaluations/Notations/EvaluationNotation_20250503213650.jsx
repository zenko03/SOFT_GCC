    ...
    const fetchData = async () => {
      try {
        ...
        if (employeeId) {
          ...
          if (employeeResponse.data) {
            setSelectedEmployee({
              ...
              position: employeeResponse.data.Position || employeeResponse.data.PositionName || '',
              department: employeeResponse.data.Department || employeeResponse.data.DepartmentName || '',
        ...
        // Sinon, essayer de récupérer l'évaluation directement
          ...
          if (evaluationResponse.data && evaluationResponse.data.employeeName) {
            // Si l'API renvoie directement les informations de l'employé dans l'évaluation
            const nameParts = evaluationResponse.data.employeeName ? evaluationResponse.data.employeeName.split(' ') : ['', ''];
            
            setSelectedEmployee({
              firstName: nameParts[0] || '',
              lastName: nameParts.slice(1).join(' ') || '',
              position: evaluationResponse.data.position || evaluationResponse.data.Position || '',
              department: evaluationResponse.data.department || evaluationResponse.data.Department || '',
              evaluationId: evaluationId
            });
          } else {
            // Si l'employé n'est pas dans l'évaluation, récupérer l'employé à partir de employeeId dans l'évaluation
            const employeeId = evaluationResponse.data.userId;
            if (employeeId) {
              const employeeResponse = await axios.get(`https://localhost:7082/api/User/${employeeId}`);
              console.log('Données employé via évaluation:', employeeResponse.data);
              console.log('Structure des données via évaluation:', {
                firstName: employeeResponse.data.FirstName,
                lastName: employeeResponse.data.LastName,
                position: employeeResponse.data.Position,
                positionName: employeeResponse.data.PositionName,
