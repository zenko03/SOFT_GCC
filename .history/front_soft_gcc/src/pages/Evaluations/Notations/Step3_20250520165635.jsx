...
const Step3 = ({ ratings, average, evaluationId, validationData, onValidationChange, onGeneratePDF }) => {
  ...
  const [pdfFunctionRegistered, setPdfFunctionRegistered] = useState(false);

  // Ajout d'un drapeau pour éviter les génération multiples
  // Modifier l'initialisation des états pour ajouter un drapeau
  ...
  useEffect(() => {
    const fetchEmployeeData = async () => {
      try {
        ...
        // Tentative 1: Récupérer via l'API d'évaluation spécifique
        ...
        if (response.data &&
          (response.data.firstName !== 'Non trouvé' &&
            response.data.lastName !== 'Non trouvé' &&
            response.data.department !== 'Département inconnu' &&
            response.data.position !== 'Poste inconnu')) {
          console.log("Données d'employé complètes trouvées:", response.data);
          setEmployeeData(response.data);
        } else {
          console.warn("Données d'employé incomplètes, tentative alternative...");

          // Tentative 2: Essayer de récupérer l'employeeId d'abord
          const evaluationResponse = await axios.get(`https://localhost:7082/api/Evaluation/${evaluationId}`);

          if (evaluationResponse.data && (evaluationResponse.data.employeeId || evaluationResponse.data.userId)) {
            const employeeId = evaluationResponse.data.employeeId || evaluationResponse.data.userId;

            // Récupérer les détails de l'employé avec l'ID trouvé
            const employeeResponse = await axios.get(`https://localhost:7082/api/User/${employeeId}`);

            if (employeeResponse.data) {
              console.log("Données d'employé récupérées via l'ID:", employeeResponse.data);
              setEmployeeData({
            ...
            throw new Error("Impossible de trouver l'ID de l'employé");
          }
        }
      } catch (err) {
  ...
  // Exposer la fonction de génération de PDF au composant parent, mais seulement après le montage initial
  useEffect(() => {
    ...
    return () => {
      // Réinitialiser le drapeau d'enregistrement lors du démontage
      setPdfFunctionRegistered(false);
    };
  }, []);

  // Séparer l'enregistrement de la fonction dans un autre useEffect qui dépend du montage
  // Dans Step3.jsx, modifiez le useEffect qui enregistre la fonction generatePDF

  // Exposer la fonction de génération de PDF au composant parent, mais seulement après le montage initial
  useEffect(() => {
    // Ne pas enregistrer la fonction si elle a déjà été enregistrée
    if (onGeneratePDF && !pdfFunctionRegistered) {
      console.log("Enregistrement de la fonction de génération PDF");

      // Créer une fonction wrapper qui ne sera exécutée que lorsqu'elle est explicitement appelée
      const pdfFunction = () => {
        console.log("Fonction de génération PDF appelée explicitement");
        return generatePDF();
      };

      // Passer la référence de la fonction, pas la fonction exécutée
      onGeneratePDF(pdfFunction);

      // Marquer que la fonction a été enregistrée
      setPdfFunctionRegistered(true);
    }
  }, [onGeneratePDF, pdfFunctionRegistered]);
