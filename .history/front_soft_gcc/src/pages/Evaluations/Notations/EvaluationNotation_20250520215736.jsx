  ...
  const generatePDFAfterValidation = () => {
    ...
    try {
      ...
      console.log("Données préparées pour la génération PDF:", {
        evaluationData: evaluationDataForPDF,
      ...
      try {
        // Appel direct à la fonction avec vérification des arguments
        if (!evaluationDataForPDF || !employeeDataForPDF || !ratings || !averageRating) {
          console.error("Données incomplètes pour la génération du PDF");
          return false;
        }
        
        // Vérifier que les données employé sont non-nulles
        if (!employeeDataForPDF.firstName || !employeeDataForPDF.lastName) {
          console.warn("Données employé incomplètes, utilisation de valeurs par défaut");
          employeeDataForPDF.firstName = employeeDataForPDF.firstName || 'Employé';
          employeeDataForPDF.lastName = employeeDataForPDF.lastName || 'Non identifié';
        }
        
        // Forcer une petite pause pour éviter les conflits d'exécution
        setTimeout(() => {
