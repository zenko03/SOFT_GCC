...
const Step3 = ({ ratings, average, evaluationId, validationData, onValidationChange, onGeneratePDF }) => {
  ...
  // Référence pour stocker la fonction de génération PDF de manière stable
  const generatePDFRef = useRef(null);
  // Référence pour suivre si la fonction a été passée au parent
  ...
  // Définir la fonction de génération de PDF au chargement du composant
  useEffect(() => {
    // Vérifier si nous avons les données nécessaires et si la fonction callback est disponible
    if (onGeneratePDF && employeeData && !hasSetCallback.current) {
      // Créer la fonction de génération de PDF une seule fois
      ...
      // Passer la fonction au composant parent une seule fois
      onGeneratePDF(generatePDFRef.current);
      hasSetCallback.current = true;
    }
  }, [employeeData]); // Uniquement lorsque les données employé changent
