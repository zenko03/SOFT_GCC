  ...
  useEffect(() => {
    const fetchCompetenceNames = async () => {
      try {
        ...
        const competencesResponse = await axios.get('https://localhost:7082/api/CompetenceLine');
        
        if (competencesResponse.data) {
          // Créer un mapping des IDs vers les noms
          const namesMap = {};
          competencesResponse.data.forEach(comp => {
            // Utiliser le champ Description avec un D majuscule
            namesMap[comp.competenceLineId] = comp.Description || comp.name || comp.competenceLineName || comp.description || `Compétence ${comp.competenceLineId}`;
          });
