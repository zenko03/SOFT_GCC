...
const Step3 = ({ ratings, average, evaluationId, validationData, onValidationChange, onGeneratePDF }) => {
  ...
  useEffect(() => {
    // N'exécuter ceci qu'une fois que le composant est monté et uniquement si
    // la fonction n'a pas encore été enregistrée
    if (mounted && onGeneratePDF && !pdfFunctionRegistered) {
      console.log("Enregistrement de la fonction de génération PDF");
      
      // Créer une fonction qui servira de wrapper
      const pdfFunction = () => {
  ...
  }, [mounted, onGeneratePDF, pdfFunctionRegistered]);
