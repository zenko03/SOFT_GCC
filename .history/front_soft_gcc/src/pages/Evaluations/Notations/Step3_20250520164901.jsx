...
const Step3 = ({ ratings, average, evaluationId, validationData, onValidationChange, onGeneratePDF }) => {
  ...
  // Drapeau pour indiquer si l'utilisateur a demandé un PDF
      ...
      try {
        ...
        try {
          ...
          if (response.data && 
            ...
            return; // Sortir de la fonction si les données sont complètes
          }
        } catch (error) {
          console.warn("Erreur API employee-details, tentative alternative...", error.message);
        }
        ...
        try {
          ...
          console.warn("Erreur API Evaluation et User, utilisation des données par défaut", error.message);
        }
        
        // Si toutes les méthodes ont échoué, utiliser des données par défaut
  ...
  const generatePDF = async () => {
    // Vérifier si on est déjà en train de générer un PDF
    if (pdfGenerationLock.current) {
      console.log("Génération de PDF déjà en cours, ignorer l'appel");
      return null;
    }
    
    // Activer le verrou
    pdfGenerationLock.current = true;
    ...
    try {
      ...
      // Générer et télécharger le PDF uniquement si l'utilisateur l'a explicitement demandé
      if (userRequestedPdf) {
        console.log("Génération du PDF à la demande de l'utilisateur");
        ...
        console.log("Pas de génération automatique du PDF");
        return null;
      }
    } catch (err) {
  ...
  // avec un délai pour éviter l'exécution automatique
  useEffect(() => {
    if (!isComponentMounted.current || !onGeneratePDF) return;
    
    // Créer une fonction wrapper qui vérifie si l'utilisateur a demandé un PDF
    const pdfGenerationHandler = () => {
      console.log("Fonction de génération PDF appelée");
      setUserRequestedPdf(true);
      return generatePDF();
    };
    
    // Enregistrer avec un délai pour s'assurer que le parent ne l'appelle pas immédiatement
    const timeoutId = setTimeout(() => {
      console.log("Enregistrement de la fonction de génération PDF");
      onGeneratePDF(pdfGenerationHandler);
    }, 500);
