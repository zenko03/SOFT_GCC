...
const generatePDFContent = async (
  ...
  try {
    ...
    const data = questionsEntries.map(([questionId, question]) => [
      ...
      question.comment.length > 40 ? question.comment.substring(0, 37) + '...' : question.comment
    ]);
    ...
    // Section pour afficher quelques questions complètes avec leurs commentaires détaillés
    // Ajouter une nouvelle page pour les questions détaillées
    doc.addPage();
    y = margin;
    
    // Titre pour les questions détaillées
    doc.setFontSize(14);
    doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
    doc.setFont(undefined, 'bold');
    doc.text('QUESTIONS ET COMMENTAIRES DÉTAILLÉS', pageWidth / 2, y, { align: 'center' });
    
    y += 10;
    
    // Sélectionner jusqu'à 5 questions pour affichage détaillé
    const detailedQuestions = questionsEntries.slice(0, 5);
    
    detailedQuestions.forEach(([questionId, question], index) => {
      ...
      doc.roundedRect(margin, y, pageWidth - (2 * margin), 70, 2, 2, 'FD');
      ...
      // Texte de la question sur plusieurs lignes si nécessaire
      const questionLines = doc.splitTextToSize(question.text, pageWidth - (2 * margin) - 25);
      doc.setFont(undefined, 'normal');
      doc.text(questionLines, margin + 30, y);
      ...
      if (question.competence) {
        doc.setFont(undefined, 'bold');
        doc.text('Compétence: ', margin + 5, y);
        doc.setFont(undefined, 'normal');
        doc.text(question.competence, margin + 40, y);
        y += 7;
      }
      ...
      let noteColor;
      if (question.note >= 4) noteColor = [39, 174, 96]; // Vert
      else if (question.note >= 3) noteColor = [41, 128, 185]; // Bleu
      else if (question.note >= 2) noteColor = [243, 156, 18]; // Orange
      else noteColor = [192, 57, 43]; // Rouge
      
      doc.setTextColor(noteColor[0], noteColor[1], noteColor[2]);
      doc.setFont(undefined, 'normal');
      doc.text(`${question.note}/5`, margin + 40, y);
      ...
      if (question.comment) {
        doc.setTextColor(0, 0, 0);
        doc.setFont(undefined, 'bold');
        doc.text('Commentaire: ', margin + 5, y);
        
        // Commentaire sur plusieurs lignes si nécessaire
        const commentLines = doc.splitTextToSize(question.comment, pageWidth - (2 * margin) - 40);
        doc.setFont(undefined, 'normal');
    ...
    // Ajouter une nouvelle page pour la validation et les suggestions
    doc.addPage();
    y = margin;
