  ...
  useEffect(() => {
    ...
    const fetchEvaluation = async () => {
      try {
        ...
        // Si les informations de poste ou département sont manquantes ou "non défini", récupérer les détails de l'employé
        if (!evaluationInfo.position || evaluationInfo.position === 'Non défini' || 
          ...
          // Essayer différentes propriétés qui pourraient contenir l'ID de l'utilisateur
          const userId = evaluationInfo.userId || evaluationInfo.employeeId;
          
          // Vérifier si nous avons un ID d'utilisateur valide
          if (userId && userId !== 'undefined' && userId !== undefined) {
            try {
              console.log('Récupération des détails complémentaires pour l\'employé:', userId);
              const employeeDetailsResponse = await axios.get(`https://localhost:7082/api/User/${userId}`, {
                headers: { Authorization: `Bearer ${token}` }
              });
              
              console.log('Détails employé reçus:', employeeDetailsResponse.data);
              
              // Mettre à jour les informations d'évaluation avec les détails complémentaires
              evaluationInfo = {
                ...evaluationInfo,
                ...
                // Autres détails utiles à récupérer si nécessaire
                // ...
              };
            ...
            console.warn('Impossible de récupérer les détails de l\'employé: ID utilisateur non disponible dans les données d\'évaluation');
            console.log('Données d\'évaluation disponibles:', evaluationInfo);
          }
        }
