...
const EvaluationPage = () => {
  ...
  // Ajout des états pour la configuration du temps
  const [showTimeConfig, setShowTimeConfig] = useState(false);
  const [timeConfig, setTimeConfig] = useState({});
  const [isAdmin, setIsAdmin] = useState(false); // À déterminer depuis les droits de l'utilisateur
  ...
  const initializeQuestionTimer = (questionId, maxTimeInMinutes) => {
    // Utiliser le temps défini dans les données ou le temps par défaut (15 minutes)
  ...
  useEffect(() => {
    if (evaluationData && evaluationData.questions && activeStep < evaluationData.questions.length) {
      ...
      if (!questionTimers[currentQuestion.questionId]) {
        // Utiliser le temps spécifique à la question si disponible
      ...
      const timer = setInterval(() => {
        ...
        setTimeSpent(prev => ({
          ...prev,
          [currentQuestion.questionId]: (prev[currentQuestion.questionId] || 0) + 1
        }));

        // Mettre à jour le temps restant
        setQuestionTimeRemaining(prev => {
          const remaining = prev[currentQuestion.questionId] - 1;
          // Avertir l'utilisateur lorsqu'il reste moins d'une minute
          if (remaining === 60) {
            toast.warning("Il vous reste moins d'une minute pour cette question!", {
              position: "top-right",
              autoClose: 5000
            });
          }
          ...
          if (remaining <= 0) {
            // Si le temps est écoulé, demander à l'utilisateur s'il veut passer à la question suivante
            clearInterval(timer);
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        const payload = JSON.parse(atob(token.split('.')[1]));
        // Vérifier si le rôle est admin (ajuster selon votre système d'autorisation)
  ...
  // Initialiser la configuration du temps pour chaque question
  useEffect(() => {
    if (evaluationData && evaluationData.questions) {
      const initialTimeConfig = {};
      evaluationData.questions.forEach(question => {
        initialTimeConfig[question.questionId] = question.maxTimeInMinutes || 15;
      });
      setTimeConfig(initialTimeConfig);
    }
  }, [evaluationData]);
  
  // Appliquer les changements de configuration du temps
  const applyTimeConfig = async () => {
    try {
      // Mise à jour locale des temps maximum
      if (evaluationData && evaluationData.questions) {
        const updatedQuestions = evaluationData.questions.map(q => ({
          ...q,
          maxTimeInMinutes: timeConfig[q.questionId] || q.maxTimeInMinutes || 15
        }));
        ...
        // Réinitialiser les timers avec les nouvelles valeurs
        updatedQuestions.forEach(q => {
          if (questionTimers[q.questionId]) {
            initializeQuestionTimer(q.questionId, timeConfig[q.questionId]);
          }
        });
        
        toast.success('Configuration du temps mise à jour avec succès');
        setShowTimeConfig(false);
      }
    } catch (error) {
      console.error('Erreur lors de la mise à jour de la configuration du temps:', error);
  ...
  const handleNext = () => {
    if (evaluationData && evaluationData.questions && evaluationData.questions.length > 0) {
      ...
      // Validation selon le type de question (simplifié à TEXT et QCM)
  ...
  const TimeRemainingDisplay = () => {
    ...
    if (percentRemaining > 50) {
      timeColor = '#4caf50'; // Vert - beaucoup de temps restant
    } else if (percentRemaining > 25) {
      timeColor = '#ff9800'; // Orange - moins de la moitié du temps restant
    } else {
      timeColor = '#f44336'; // Rouge - temps presque écoulé
    }
    ...
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '150px' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography variant="body1" sx={{ color: timeColor, fontWeight: 'bold' }}>
  ...
  return (
    <Container component="main" maxWidth="md">
      ...
      {/* Modal pour la configuration du temps */}
      <Dialog open={showTimeConfig} onClose={() => setShowTimeConfig(false)} maxWidth="md">
        <DialogTitle>Configuration du temps par question</DialogTitle>
        <DialogContent>
          {evaluationData && evaluationData.questions && evaluationData.questions.map((question, index) => (
            <Box key={question.questionId} sx={{ mb: 2, p: 2, border: '1px solid #e0e0e0', borderRadius: 1 }}>
              <Typography variant="subtitle1" gutterBottom>
                Q{index + 1}: {question.text.substring(0, 60)}{question.text.length > 60 ? '...' : ''}
              </Typography>
              <Grid container spacing={2} alignItems="center">
                <Grid item xs={6}>
                  <TextField
                    ...
                    value={timeConfig[question.questionId] || 15}
                    onChange={(e) => setTimeConfig({
                      ...timeConfig,
                      [question.questionId]: parseInt(e.target.value) || 15
                    })}
                    fullWidth
                  />
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="body2" color="text.secondary">
                    Temps actuel: {question.maxTimeInMinutes || 15} minutes
                  </Typography>
                </Grid>
              </Grid>
            </Box>
        ...
        <DialogActions>
          <Button onClick={() => setShowTimeConfig(false)}>Annuler</Button>
          <Button onClick={applyTimeConfig} variant="contained" color="primary">
            Appliquer
          </Button>
        </DialogActions>
      </Dialog>
