    ...
    const fetchEvaluation = async () => {
      try {
        ...
        if (!evaluationInfo.position || evaluationInfo.position === 'Non défini' || 
          ...
          try {
            console.log('Récupération des détails complémentaires pour l\'employé:', evaluationInfo.userId);
            const employeeDetailsResponse = await axios.get(`https://localhost:7082/api/User/${evaluationInfo.userId}`, {
              headers: { Authorization: `Bearer ${token}` }
            });
