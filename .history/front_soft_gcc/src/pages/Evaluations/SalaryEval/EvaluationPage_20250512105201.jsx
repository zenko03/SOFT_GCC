...
import { 
  ...
  DialogContent, DialogActions, Slider, IconButton
} from '@mui/material';
import { 
  FaArrowRight, FaArrowLeft, FaPaperPlane,
  FaSpinner, FaBriefcase, FaBuilding,
  FaIdCard, FaSignOutAlt, FaClock, FaCog
} from 'react-icons/fa';
...
const EvaluationPage = () => {
  ...
  const [showTimeConfigDialog, setShowTimeConfigDialog] = useState(false);
  ...
  // Modifier le useEffect pour gérer le timer par question avec alertes visuelles
  useEffect(() => {
  ...
  // Vérifier si l'utilisateur est administrateur à partir du token
  useEffect(() => {
    const token = localStorage.getItem('evaluationToken');
    if (token) {
      try {
        ...
        // Adapter cette logique selon votre système d'authentification
  ...
  // Fonction pour appliquer les configurations de temps
  const applyTimeConfigurations = () => {
    if (!evaluationData || !evaluationData.questions) return;
    
    // Créer une copie mise à jour des questions
    const updatedQuestions = evaluationData.questions.map(question => ({
      ...question,
      maxTimeInMinutes: timeConfigs[question.questionId] || question.maxTimeInMinutes || 15
    }));
    ...
    // Réinitialiser les timers pour les questions qui ont changé
    updatedQuestions.forEach(question => {
      if (questionTimers[question.questionId]) {
        initializeQuestionTimer(question.questionId, timeConfigs[question.questionId]);
      }
    });
    
    // Fermer le dialog
    setShowTimeConfigDialog(false);
  ...
  return (
    <Container component="main" maxWidth="md">
      <AppBar position="static">
        <Toolbar>
          ...
          {isAdmin && (
            <IconButton 
              color="inherit" 
              onClick={() => setShowTimeConfigDialog(true)}
              title="Configurer le temps par question"
              sx={{ ml: 1 }}
            >
              <FaCog />
            </IconButton>
      ...
      {/* Dialog de configuration du temps */}
      <Dialog 
        open={showTimeConfigDialog} 
        onClose={() => setShowTimeConfigDialog(false)}
        maxWidth="md"
        fullWidth
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            Définissez le temps maximum (en minutes) pour chaque question.
          </Typography>
          
          {evaluationData && evaluationData.questions && evaluationData.questions.map((question, index) => (
            <Box key={question.questionId} sx={{ mb: 3, p: 2, border: '1px solid #eee', borderRadius: 1 }}>
              <Typography variant="subtitle1" gutterBottom>
                Question {index + 1}: {question.text.length > 60 ? `${question.text.substring(0, 60)}...` : question.text}
              </Typography>
        ...
        <DialogActions>
          <Button onClick={() => setShowTimeConfigDialog(false)}>Annuler</Button>
          <Button onClick={applyTimeConfigurations} variant="contained" color="primary">
            Appliquer
          </Button>
        </DialogActions>
      </Dialog>
