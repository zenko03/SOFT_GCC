...
import { 
  ...
  FaTimesCircle, FaUserTie, FaClipboardCheck
} from 'react-icons/fa';
  ...
  useEffect(() => {
    ...
    const fetchEvaluation = async () => {
      try {
        ...
        // Initialiser les réponses avec les valeurs existantes ou par défaut
        const initialAnswers = {};
        if (response.data.questions && response.data.questions.length > 0) {
          response.data.questions.forEach(question => {
            if (question.existingResponse) {
              initialAnswers[question.questionId] = JSON.parse(question.existingResponse.value);
            } else {
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} md={6}>
                <Typography variant="h6">
                  ...
                  {evaluationData.employeeInfo.name}
                </Typography>
                <Typography variant="subtitle1" color="textSecondary">
                  <FaBriefcase style={{ marginRight: '8px' }} />
                  {evaluationData.employeeInfo.position}
                </Typography>
              </Grid>
              <Grid item xs={12} md={6}>
                <Typography variant="h6">
                  <FaBuilding style={{ marginRight: '8px' }} />
                  {evaluationData.employeeInfo.department}
                </Typography>
                <Typography variant="subtitle1" color="textSecondary">
                  <FaUserTie style={{ marginRight: '8px' }} />
                  {evaluationData.supervisors.map(s => s.name).join(', ')}
                </Typography>
              </Grid>
            </Grid>
          </CardContent>
          ...
          {activeStep < evaluationData.questions.length ? (
            <Grid container spacing={2}>
              <Grid item xs={12}>
                ...
                <Chip 
                  label={`Type: ${evaluationData.questions[activeStep].responseType}`}
                  color="primary"
                  size="small"
                  sx={{ mt: 1, mb: 2 }}
                />
            ...
            <Box>
              ...
              <TextField
                label="Vos commentaires sur l'évaluation"
                variant="outlined"
                fullWidth
                multiline
                rows={4}
