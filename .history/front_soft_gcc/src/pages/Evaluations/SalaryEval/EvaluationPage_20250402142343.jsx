...
const EvaluationPage = () => {
  ...
  // Ajouter cette fonction pour récupérer les options des questions
  const fetchQuestionOptions = async () => {
    try {
      const response = await axios.get(
        `https://localhost:7082/api/evaluation/${evaluationData.evaluationId}/options`,
  ...
  // Ajouter cette fonction pour sauvegarder la progression automatiquement
  const setupAutoSave = () => {
    const interval = setInterval(async () => {
      try {
        await axios.post(
          `https://localhost:7082/api/evaluation/${evaluationData.evaluationId}/save-progress`,
          {
  ...
  // Modifier le useEffect pour le temps restant
  useEffect(() => {
    const fetchTimeRemaining = async () => {
      try {
        const response = await axios.get(
          `https://localhost:7082/api/evaluation/${evaluationData.evaluationId}/time-remaining`,
          { headers: { Authorization: `Bearer ${localStorage.getItem('evaluationToken')}` } }
        );
    ...
    fetchTimeRemaining();
    const timer = setInterval(fetchTimeRemaining, 60000); // Mise à jour toutes les minutes
