...
const EvaluationHistory = () => {
  ...
  const calculateAdditionalStats = useCallback((evaluations) => {
    ...
    evaluations.forEach(eval => {
      // Départements
      if (eval.department) {
        deptMap[eval.department] = (deptMap[eval.department] || 0) + 1;
      }
      
      // Types
      if (eval.evaluationType) {
        typeMap[eval.evaluationType] = (typeMap[eval.evaluationType] || 0) + 1;
      }
      
      // Scores
      const score = eval.overallScore;
    ...
    // Extraire la liste des types d'évaluation uniques
    setEvaluationTypes(Array.from(new Set(evaluations.map(e => e.evaluationType).filter(Boolean))));
  }, []);
      ...
      <div className="container mt-4">
        ...
        <div className="card shadow mb-4">
          <div className="card-header">
            <h5 className="mb-0">Statistiques supplémentaires</h5>
          </div>
          <div className="card-body">
            <div className="row">
              <div className="col-md-4">
                  ...
                  <div className="progress-container">
                    <div className="progress" style={{ height: "20px", marginBottom: "10px" }}>
                      <div 
                        ...
                        role="progressbar" 
                        style={{ width: `${distributions.scores.low / evaluations.length * 100}%` }}
                        aria-valuenow={distributions.scores.low} 
                        aria-valuemin="0" 
                        aria-valuemax={evaluations.length}
                      >
                        {distributions.scores.low} ({((distributions.scores.low / evaluations.length) * 100).toFixed(1)}%)
                      </div>
                      <div 
                        className="progress-bar bg-warning" 
                        role="progressbar" 
                        style={{ width: `${distributions.scores.medium / evaluations.length * 100}%` }}
                        aria-valuenow={distributions.scores.medium} 
                        aria-valuemin="0" 
                        aria-valuemax={evaluations.length}
                      >
                        {distributions.scores.medium} ({((distributions.scores.medium / evaluations.length) * 100).toFixed(1)}%)
                      </div>
                      <div 
                        className="progress-bar bg-success" 
                        role="progressbar" 
                        style={{ width: `${distributions.scores.high / evaluations.length * 100}%` }}
                        aria-valuenow={distributions.scores.high} 
                        aria-valuemin="0" 
                        aria-valuemax={evaluations.length}
                      >
                        {distributions.scores.high} ({((distributions.scores.high / evaluations.length) * 100).toFixed(1)}%)
                      </div>
                    </div>
              ...
              <div className="col-md-4">
                <div className="stat-card">
                  ...
                  <ul className="list-group">
                    {distributions.evaluationTypes.map(type => (
                  ...
                  {performanceData.length > 1 ? (
                    <div>
                      <div className="trend-arrow" style={{ 
                        textAlign: 'center', 
                        fontSize: '2rem',
                        color: performanceData[performanceData.length-1].score > performanceData[0].score ? '#4caf50' : '#e74c3c'
                      }}>
                        {performanceData[performanceData.length-1].score > performanceData[0].score ? '▲' : '▼'}
                      </div>
                      <div className="trend-value text-center">
                        <strong>
                          {((performanceData[performanceData.length-1].score - performanceData[0].score) / performanceData[0].score * 100).toFixed(2)}%
                        </strong>
                        <div className="small text-muted">
                          {`De ${performanceData[0].score.toFixed(2)} à ${performanceData[performanceData.length-1].score.toFixed(2)}`}
                        </div>
                      </div>
                    </div>
                  ) : (
