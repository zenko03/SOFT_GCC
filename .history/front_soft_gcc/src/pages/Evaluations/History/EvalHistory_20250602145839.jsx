...
const EvaluationHistory = () => {
  ...
  const calculateAdditionalStats = useCallback((evaluationsData) => {
    ...
    evaluationsData.forEach(item => {
      ...
      if (item.evaluationType) {
        typeMap[item.evaluationType] = (typeMap[item.evaluationType] || 0) + 1;
      }
      
      // Scores
      const score = item.overallScore;
      if (score < 2.5) lowScore++;
      else if (score < 4) mediumScore++;
      else highScore++;
    });
    ...
    setDetailedStats({
      scoreDistribution: { low: lowScore, medium: mediumScore, high: highScore },
      departmentDistribution: deptData,
      evaluationTypeDistribution: typeData,
      performanceByYear: [],
      trendData: { isIncreasing: false, percentageChange: 0 }
    });
  }, []);
  ...
  const fetchEvaluations = useCallback(async () => {
    ...
    try {
      ...
      calculateAdditionalStats(response.data.evaluations);
  ...
  }, [fetchDepartments, fetchKpis, fetchEvaluations, fetchDetailedStatistics]);
              ...
              <div className="row">
                <div className="col-md-4">
                  <div className="stat-card">
                    <h6>Distribution des scores</h6>
                    <div className="progress-container">
                      {loadingDetailedStats ? (
                        <div className="text-center py-3"><FaSync className="fa-spin" /> Chargement...</div>
                      ) : (
                        <>
                          <div className="progress" style={{ height: "20px", marginBottom: "10px" }}>
                            <div 
                              ...
                              role="progressbar" 
                              style={{ width: `${detailedStats.scoreDistribution.low / (detailedStats.scoreDistribution.low + detailedStats.scoreDistribution.medium + detailedStats.scoreDistribution.high) * 100}%` }}
                              aria-valuenow={detailedStats.scoreDistribution.low} 
                              aria-valuemin="0" 
                              aria-valuemax="100"
                            >
                              {detailedStats.scoreDistribution.low} ({((detailedStats.scoreDistribution.low / (detailedStats.scoreDistribution.low + detailedStats.scoreDistribution.medium + detailedStats.scoreDistribution.high)) * 100).toFixed(1)}%)
                            </div>
                            <div 
                              className="progress-bar bg-warning" 
                              role="progressbar" 
                              style={{ width: `${detailedStats.scoreDistribution.medium / (detailedStats.scoreDistribution.low + detailedStats.scoreDistribution.medium + detailedStats.scoreDistribution.high) * 100}%` }}
                              aria-valuenow={detailedStats.scoreDistribution.medium} 
                              aria-valuemin="0" 
                              aria-valuemax="100"
                            >
                              {detailedStats.scoreDistribution.medium} ({((detailedStats.scoreDistribution.medium / (detailedStats.scoreDistribution.low + detailedStats.scoreDistribution.medium + detailedStats.scoreDistribution.high)) * 100).toFixed(1)}%)
                            </div>
                            <div 
                              className="progress-bar bg-success" 
                              role="progressbar" 
                              style={{ width: `${detailedStats.scoreDistribution.high / (detailedStats.scoreDistribution.low + detailedStats.scoreDistribution.medium + detailedStats.scoreDistribution.high) * 100}%` }}
                              aria-valuenow={detailedStats.scoreDistribution.high} 
                              aria-valuemin="0" 
                              aria-valuemax="100"
                            >
                              {detailedStats.scoreDistribution.high} ({((detailedStats.scoreDistribution.high / (detailedStats.scoreDistribution.low + detailedStats.scoreDistribution.medium + detailedStats.scoreDistribution.high)) * 100).toFixed(1)}%)
                            </div>
                          </div>
                          <div className="legend d-flex justify-content-between">
                            <span><small className="text-danger">■</small> Faible (&lt; 2.5)</span>
                            <span><small className="text-warning">■</small> Moyen (2.5-4)</span>
                            <span><small className="text-success">■</small> Élevé (&gt; 4)</span>
                          </div>
                        </>
                      )}
                    </div>
                  ...
                  <div className="stat-card">
                    ...
                    {loadingDetailedStats ? (
                      ...
                      <ul className="list-group">
                        {detailedStats.evaluationTypeDistribution.map(type => (
                          <li key={type.label} className="list-group-item d-flex justify-content-between align-items-center">
                            {type.label}
                            <span className="badge badge-pill" style={{ backgroundColor: '#D4AC0D', color: 'white' }}>
                              {type.value}
                            </span>
                          </li>
                        ))}
                    ...
                    {loadingDetailedStats ? (
                      ...
                      <div>
                        <div className="trend-arrow" style={{ 
                          textAlign: 'center', 
                          fontSize: '2rem',
                          color: detailedStats.trendData.isIncreasing ? '#4caf50' : '#e74c3c'
                        }}>
                          {detailedStats.trendData.isIncreasing ? '▲' : '▼'}
                        </div>
                        <div className="trend-value text-center">
                          <strong>
                            {detailedStats.trendData.percentageChange.toFixed(2)}%
                          </strong>
                          <div className="small text-muted">
                            {`De ${detailedStats.trendData.startValue.toFixed(2)} à ${detailedStats.trendData.endValue.toFixed(2)}`}
                          </div>
                        </div>
                      </div>
                    ) : (
                      <div className="text-center text-muted">
                        Données insuffisantes pour calculer une tendance
                      </div>
                    )}
                  </div>
                </div>
