...
const GlobalPerformanceGraph = ({ filters = {} }) => {
  ...
  const fetchGlobalPerformanceData = useCallback(async () => {
    ...
    try {
      console.log("Début de la récupération des performances avec filtres:", filters);
      
      // Préparation des paramètres avec une conversion robuste de dates
      const params = {};
      ...
      // Conversion robuste des dates - même logique que pour fetchKPIs
      if (filters.startDate) {
        try {
          const date = new Date(filters.startDate);
          if (!isNaN(date.getTime())) {
      ...
      console.log("Réponse brute de GlobalPerformance:", response.data);
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        if (formattedData.length === 0) {
          console.warn("Aucune donnée valide après formatage");
        }
  ...
  const calculateAverageScore = () => {
    ...
    // Filtrer les scores non nuls ou non valides
    ...
    const totalScore = validItems.reduce((sum, item) => sum + (item.averageScore || 0), 0);
    const average = totalScore / validItems.length;
    
    // Comparer avec le score moyen des KPIs pour débogage
    console.log("Comparaison des scores moyens:", {
      scoreGraphique: average.toFixed(2),
      scoreKPI: kpis.overallAverage.toFixed(2),
      différence: Math.abs(average - kpis.overallAverage).toFixed(2)
    });
  ...
  const calculateTotalEvaluations = () => {
    ...
    const total = chartData.reduce((sum, item) => sum + (parseInt(item.evaluationCount) || 0), 0);
    
    // Comparer avec le total des KPIs pour débogage
    console.log("Comparaison du nombre d'évaluations:", {
      totalGraphique: total,
      totalKPI: kpis.totalEvaluations,
      égal: total === kpis.totalEvaluations ? "Oui" : "Non"
    });
          ...
          <div className="row g-3">
            <div className="col-md-4">
              <div className="kpi-card participation">
                ...
                <div className="kpi-content">
                  <h3>{kpis.participationRate > 0 ? kpis.participationRate.toFixed(2) + '%' : '--'}</h3>
            ...
            <div className="col-md-4">
              <div className="kpi-card approval">
                ...
                <div className="kpi-content">
                  <h3>{kpis.approvalRate > 0 ? kpis.approvalRate.toFixed(2) + '%' : '--'}</h3>
            ...
            <div className="col-md-4">
              <div className="kpi-card average">
                ...
                <div className="kpi-content">
                  <h3>{kpis.overallAverage > 0 ? kpis.overallAverage.toFixed(2) + '/5' : '--'}</h3>
                  <p>Score Moyen Global ({kpis.totalEvaluations} éval.)</p>
                </div>
              </div>
            </div>
            ...
            <div className="row g-3">
              <div className="col-md-3">
                <div className="stat-card average-score">
                  <div className="stat-value">{calculateAverageScore() > 0 ? calculateAverageScore().toFixed(2) : '--'}<span>/5</span></div>
                  <div className="stat-label">Score Moyen</div>
                </div>
              </div>
              <div className="col-md-3">
                <div className="stat-card total-evals">
                  <div className="stat-value">{calculateTotalEvaluations() > 0 ? calculateTotalEvaluations() : '--'}</div>
                  <div className="stat-label">Total Évaluations</div>
                </div>
              </div>
              <div className="col-md-3">
                <div className="stat-card best-year">
                  <div className="stat-value">{getBestYear() !== 'N/A' ? getBestYear() : '--'}</div>
                  <div className="stat-label">Meilleure Année</div>
                </div>
              </div>
              <div className="col-md-3">
                <div className={`stat-card ${getTrend().isPositive ? 'trend-up' : 'trend-down'}`}>
                  <div className="stat-value">{getTrend().text !== 'N/A' ? getTrend().text : '--'}</div>
