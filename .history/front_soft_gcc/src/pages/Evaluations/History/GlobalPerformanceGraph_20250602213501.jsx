...
const GlobalPerformanceGraph = ({ filters = {} }) => {
  ...
  const fetchKPIs = useCallback(async () => {
    try {
      // Préparer les paramètres avec des valeurs par défaut pour éviter les problèmes
      const params = {
        departmentName: filters.department || undefined
      };
      
      // Ajouter les dates uniquement si elles sont valides
      if (filters.startDate) {
        try {
          // S'assurer que la date est au format ISO correctement
      ...
      console.log("Paramètres pour KPIs:", params);
      ...
      if (response.data) {
        ...
        setKpis({
          participationRate: parseFloat(response.data.participationRate) || 0,
          approvalRate: parseFloat(response.data.approvalRate) || 0,
          overallAverage: parseFloat(response.data.overallAverage) || 0,
          totalEvaluations: response.data.totalEvaluations || 0
        });
      } else {
        console.warn("Données KPI vides ou nulles");
