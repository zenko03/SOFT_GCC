              ...
              return (
                <div key={`employee-${employeeId}`} className="employee-questions">
                  ...
                  {competenceLines[employeeId]?.map(competence => {
                    ...
                    return (
                      <div key={`competence-${employeeId}-${competence.competenceLineId}`} className="competence-section mb-4">
                        <h5 className="competence-title bg-light p-3 rounded d-flex justify-content-between align-items-center">
                          <span>
                        ...
                        <div className="d-flex justify-content-between align-items-center px-3 pt-3">
                          <div>
                            <span className="badge badge-success mr-2">
                              {selectedQuestions[employeeId]?.[competence.competenceLineId]?.length || 0} 
                              /{filteredQuestions.length} sélectionnée{(selectedQuestions[employeeId]?.[competence.competenceLineId]?.length || 0) > 1 ? 's' : ''}
                            </span>
                          </div>
                          <div className="btn-group btn-group-sm">
                            <button 
                              className="btn btn-outline-primary"
                              onClick={() => handleSelectAllQuestionsForCompetence(employeeId, competence.competenceLineId, filteredQuestions, true)}
                            >
                              <i className="fas fa-check-square mr-1"></i> Tout sélectionner
                            </button>
                            <button 
                              className="btn btn-outline-secondary"
                              onClick={() => handleSelectAllQuestionsForCompetence(employeeId, competence.competenceLineId, filteredQuestions, false)}
                            >
                              <i className="fas fa-square mr-1"></i> Tout désélectionner
                            </button>
                            <button 
                              className="btn btn-outline-info"
                              onClick={() => handleRandomSelection(employeeId, competence.competenceLineId, filteredQuestions, 2)}
                              title="Sélectionner 2 questions aléatoirement"
                            >
                              <i className="fas fa-random mr-1"></i> Aléatoire (2)
                            </button>
                          </div>
                        </div>
                        ...
                        <div className="questions-list p-3">
                          {filteredQuestions.map(question => {
                            ...
                            return (
                              <div key={questionKey} className={`question-item ${isUsedByOtherEmployee ? 'border-warning' : ''}`}>
                  ...
                  {/* Message si aucune compétence n'a de questions */}
