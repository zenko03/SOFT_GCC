...
function EvaluationConfiguration({ selectedEmployees, employees, onBack, onComplete, onRemoveEmployee, autoReminderEnabled }) {
  ...
  const getBaseDuration = (employeeCount) => {
    // Même un seul employé nécessite un minimum de temps pour une évaluation de qualité
    const baseMinimum = 5; // 5 jours minimum (1 semaine)
    
    // Pour chaque employé supplémentaire, on ajoute du temps mais avec un effet diminuant
    // Pour éviter une croissance linéaire insoutenable
    let additionalDays = 0;
    if (employeeCount > 1) {
      // Approche logarithmique : chaque employé supplémentaire ajoute moins de temps
      additionalDays = Math.ceil(3 * Math.log2(employeeCount));
    }
    
    return Math.min(baseMinimum + additionalDays, 25); // Maximum 25 jours (5 semaines)
  };
  ...
  const calculateRecommendedDuration = (employeeCount, evaluationTypeId, averageQuestionCount = 10, competenceCount = 3) => {
    // Durée de base selon le nombre d'employés (approche logarithmique)
        ...
        {selectedEvaluationType && selectedEmployees.length > 0 && (
          <div className="alert alert-info mt-2">
            ...
            <strong>Recommandation actuelle:</strong> {calculateRecommendedDuration(
              ...
              selectedEmployees.length,
              parseInt(selectedEvaluationType, 10)
            ).weeks} semaines) pour évaluer {selectedEmployees.length} employé{selectedEmployees.length > 1 ? 's' : ''}.
          </div>
        )}
      </div>
    </div>
