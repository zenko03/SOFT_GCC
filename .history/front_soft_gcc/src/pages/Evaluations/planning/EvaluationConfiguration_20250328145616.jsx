...
function EvaluationConfiguration({ selectedEmployees, employees, onBack, onComplete, onRemoveEmployee }) {
  ...
  const handleQuestionSelection = (employeeId, competenceLineId, questionId, isSelected) => {
    ...
    const compId = Number(competenceLineId);
    const qId = Number(questionId);

    // Vérifier si les conversions ont réussi
    if (isNaN(empId) || isNaN(compId) || isNaN(qId)) {
      console.error("Conversion d'ID en nombre échouée:", {
        employeeId,
        competenceLineId,
        questionId
      });
      return; // Ne pas continuer avec des IDs invalides
    }
                ...
                <div key={employeeId} className="employee-questions">
                  ...
                  {competenceLines[employeeId]?.map(competence => (
                    <div key={competence.competenceLineId} className="competence-section">
                      ...
                      // Partie du rendu des questions avec traitement amélioré des IDs
                        {questions[employeeId]?.[competence.competenceLineId]?.map(question => {
                          // S'assurer que l'ID de la question est valide
                          if (!question.questionId && question.questionId !== 0) {
                            console.error("Question sans ID valide:", question);
                            return null; // Ignorer les questions sans ID valide
                          }

                          // Utiliser directement les IDs originaux pour la clé, pour éviter NaN
                          const uniqueKey = `${employeeId}-${competence.competenceLineId}-${question.questionId}`;

                          // Convertir explicitement en nombres pour la logique de sélection
                          const qId = Number(question.questionId);
                          const cId = Number(competence.competenceLineId);
                          const eId = Number(employeeId);

                          // Si l'une des conversions échoue, journaliser et continuer
                          if (isNaN(qId) || isNaN(cId) || isNaN(eId)) {
                            console.error("Conversion d'ID en nombre échouée:", {
                              employeeId,
                              competenceLineId: competence.competenceLineId,
                          ...
                          // Vérifier si la question est cochée en utilisant les valeurs originales
                          const isChecked = selectedQuestions[employeeId]?.[competence.competenceLineId]?.includes(qId) || false;

                          return (
                            <div key={uniqueKey} className="question-item">
                              <div className="form-check">
                                <input
                                  ...
                                  onChange={(e) => handleQuestionSelection(
                                    ...
                                    question.questionId, // Utilisez l'ID original
                                    e.target.checked
                                  )}
                                />
                                <label className="form-check-label" htmlFor={`question-${uniqueKey}`}>
                                  {question.question}
                                </label>
                              </div>
                            </div>
