...
function EvaluationConfiguration({ selectedEmployees, employees, onBack, onComplete, onRemoveEmployee, autoReminderEnabled }) {
  ...
  const validateDates = async (startDateStr, endDateStr) => {
    const today = new Date();
    ...
    if (!startDate || !endDate) {
      ...
      if (startDate < today) {
        ...
        // Récupérer la recommandation de durée depuis l'API
        const recommendation = durationRecommendation || await fetchRecommendedDuration();
        
        // Vérifier si la durée planifiée est suffisante
        if (durationInDays < recommendation.days) {
          error = `La durée planifiée (${durationInDays} jours) est inférieure à la durée recommandée (${recommendation.days} jours, soit ${recommendation.weeksDisplay || `${recommendation.weeks} semaine${recommendation.weeks > 1 ? 's' : ''}`}) pour ce type d'évaluation avec ${selectedEmployees.length} employé(s). Veuillez augmenter la durée ou réduire le nombre d'employés.`;
        }
      }
    }
  ...
  const validateConfiguration = async () => {
    ...
    // Appel asynchrone à validateDates
      ...
      <div className="form-group">
        ...
        {selectedEvaluationType && selectedEmployees.length > 0 && (
          <div className="alert alert-info mt-2">
            ...
            <strong>Recommandation actuelle:</strong> {durationRecommendation?.days} jours ({durationRecommendation?.weeksDisplay || `${durationRecommendation?.weeks} semaine${durationRecommendation?.weeks > 1 ? 's' : ''}`}) pour évaluer {selectedEmployees.length} employé{selectedEmployees.length > 1 ? 's' : ''} avec {selectedSupervisors.length || 1} superviseur{(selectedSupervisors.length > 1 || selectedSupervisors.length === 0) ? 's' : ''}.
