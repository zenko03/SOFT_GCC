...
function EvaluationConfiguration({ selectedEmployees, employees, onBack, onComplete, onRemoveEmployee }) {
  ...
  // Fonction de calcul de durée par paliers
  const getBaseDuration = (employeeCount) => {
    if (employeeCount <= 1) return 5; // 5 jours (1 semaine) pour 1 employé
    if (employeeCount <= 5) return 10; // 2 semaines pour 2-5 employés
    if (employeeCount <= 15) return 15; // 3 semaines pour 6-15 employés
    if (employeeCount <= 30) return 20; // 4 semaines pour 16-30 employés
    return 25; // 5 semaines maximum, même pour plus de 30 employés
  };

  // Fonction pour calculer la durée recommandée basée sur plusieurs facteurs
  const calculateRecommendedDuration = (employeeCount, evaluationTypeId, averageQuestionCount = 10, competenceCount = 3) => {
    // Durée de base selon le nombre d'employés (approche par paliers)
    const baseDuration = getBaseDuration(employeeCount);
    
    // Facteur de complexité basé sur le nombre moyen de questions
    // Plus il y a de questions, plus l'évaluation prend du temps mais avec un plafond
    const complexityFactor = Math.min(1.5, 0.8 + (averageQuestionCount / 20));
    
    // Facteur basé sur le nombre de compétences à évaluer
    const competenceFactor = Math.min(1.3, 0.9 + (competenceCount / 10));
    
    // Facteur basé sur le type d'évaluation
    let typeFactor = 1;
    const evaluationTypes = {
      1: { name: 'Évaluation annuelle', factor: 1.5 }, // Plus complète, examen de toute l'année
      2: { name: 'Évaluation de performance', factor: 1.2 }, // Focus sur les résultats
      3: { name: 'Évaluation de compétences', factor: 1.2 }, // Similaire à la performance
      4: { name: 'Évaluation de progression', factor: 0.8 }, // Plus courte, suivi ciblé
      5: { name: 'Autre', factor: 1 } // Valeur par défaut
    };
    
    if (evaluationTypeId && evaluationTypes[evaluationTypeId]) {
      typeFactor = evaluationTypes[evaluationTypeId].factor;
    }
    
    // Calcul de la durée ajustée en jours (arrondi au jour supérieur)
    const adjustedDays = Math.ceil(baseDuration * complexityFactor * competenceFactor * typeFactor);
    
    // Convertir en semaines (pour l'affichage)
    const adjustedWeeks = Math.ceil(adjustedDays / 7);
    
    // Retourner à la fois les jours et les semaines
    return {
      days: adjustedDays,
      weeks: adjustedWeeks,
      typeName: evaluationTypeId ? evaluationTypes[evaluationTypeId]?.name || 'Évaluation standard' : 'Évaluation standard'
    };
  };
  ...
  const validateDates = (startDateStr, endDateStr) => {
    ...
    if (!startDate || !endDate) {
      ...
      if (startDate < today) {
        ...
        // Calculer la durée recommandée basée sur divers facteurs
        ...
        const recommended = calculateRecommendedDuration(
          ...
          selectedEvaluationType ? parseInt(selectedEvaluationType, 10) : null,
          averageQuestionCount,
          totalCompetences
        );
        
        // Vérifier si la durée planifiée est suffisante
        if (durationInDays < recommended.days) {
          error = `La durée planifiée (${durationInDays} jours) est inférieure à la durée recommandée (${recommended.days} jours) pour ce type d'évaluation avec ${selectedEmployees.length} employé(s). Veuillez augmenter la durée ou réduire le nombre d'employés.`;
        }
      }
    }
    ...
    <div className="step-content">
      ...
      <div className="form-group">
        ...
        <div className="mt-2 text-muted">
          <strong>Durées recommandées par type d&apos;évaluation:</strong>
          <table className="table table-sm table-bordered mt-1">
            ...
            <tbody>
              ...
              <tr>
                <td>Évaluation de performance/compétences</td>
                <td>1 semaine</td>
                <td>1-2 semaines</td>
                <td>2-3 semaines</td>
                <td>3-4 semaines</td>
              ...
              <tr>
                <td>Évaluation de progression</td>
                <td>3-5 jours</td>
                <td>1 semaine</td>
                <td>1-2 semaines</td>
                <td>2-3 semaines</td>
          ...
          <small className="text-muted mt-1 d-block">
            ...
            Note : Les durées sont calculées selon le nombre d&apos;employés, le type d&apos;évaluation, 
            le nombre de compétences à évaluer et la complexité des questions.
          </small>
          {selectedEvaluationType && selectedEmployees.length > 0 && (
            <div className="alert alert-info mt-2">
              <i className="mdi mdi-chart-timeline-variant mr-1"></i>
              <strong>Recommandation actuelle:</strong> {calculateRecommendedDuration(
                selectedEmployees.length,
                parseInt(selectedEvaluationType, 10)
              ).days} jours ({calculateRecommendedDuration(
                selectedEmployees.length,
                parseInt(selectedEvaluationType, 10)
              ).weeks} semaines)
            </div>
