...
function EvaluationConfiguration({ selectedEmployees, employees, onBack, onComplete, onRemoveEmployee }) {
  ...
  const handleQuestionSelection = (employeeId, competenceLineId, questionId, isSelected) => {
    console.log('Selection de question:', {
      employeeId,
      competenceLineId,
      questionId,
      isSelected,
      typeOf: typeof questionId
    });

    // Pas besoin de convertir si c'est déjà un nombre
    const qId = typeof questionId === 'number' ? questionId : parseInt(questionId, 10);
    ...
    setSelectedQuestions(prev => {
      ...
      console.log('État actuel:', newSelection[employeeId][competenceLineId]);

      if (isSelected) {
        // Vérifier si la question n'est pas déjà sélectionnée
        if (!newSelection[employeeId][competenceLineId].includes(qId)) {
          newSelection[employeeId][competenceLineId].push(qId);
          console.log('Après ajout:', newSelection[employeeId][competenceLineId]);
        }
      } else {
        newSelection[employeeId][competenceLineId] = newSelection[employeeId][competenceLineId]
          .filter(id => id !== qId);
        console.log('Après suppression:', newSelection[employeeId][competenceLineId]);
      }
                    ...
                    <div key={competence.competenceLineId} className="competence-section">
                      ...
                      <div className="questions-list">
                        {questions[employeeId]?.[competence.competenceLineId]?.map(question => {
                          // Convertir l'ID de la question en nombre pour garantir la cohérence
                          const questionIdAsNumber = typeof question.questionId === 'number'
                            ? question.questionId
                            : parseInt(question.questionId, 10);

                          return (
                            <div key={`${employeeId}-${competence.competenceLineId}-${questionIdAsNumber}`} className="question-item">
                              <div className="form-check">
                                <input
                                  type="checkbox"
                                  className="form-check-input"
                                  id={`question-${employeeId}-${competence.competenceLineId}-${questionIdAsNumber}`}
                                  checked={selectedQuestions[employeeId]?.[competence.competenceLineId]?.includes(questionIdAsNumber) || false}
                                  onChange={(e) => handleQuestionSelection(
                                    employeeId,
                                    competence.competenceLineId,
                                    questionIdAsNumber, // Passer directement le nombre
                                    e.target.checked
                                  )}
                                />
                                <label
                                  className="form-check-label"
                                  htmlFor={`question-${employeeId}-${competence.competenceLineId}-${questionIdAsNumber}`}
                                >
                                  {question.question}
                                </label>
                              </div>
