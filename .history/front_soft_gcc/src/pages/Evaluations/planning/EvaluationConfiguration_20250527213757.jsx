...
function EvaluationConfiguration({ selectedEmployees, employees, onBack, onComplete, onRemoveEmployee }) {
  ...
  // Modification de la fonction calculateDatesFromDuration
  ...
  // Modification de la fonction validateDates
    ...
    try {
      ...
      const planningData = {
        ...
        employeeQuestions: selectedEmployees.map(employeeId => {
          ...
          // Créer un objet pour chaque question avec sa compétence associée
          const questionsWithCompetence = selectedQuestionIds.map(questionId => {
            // Trouver la compétence associée à cette question
  ...
  const renderStep1Content = () => (
    <div className="step-content">
      ...
      {/* Dates */}
      <div className="form-group">
        <label className="h5 mb-3">Période d&apos;évaluation</label>
        
        {/* Durée en semaines */}
        <div className="form-group mb-4">
          <label className="fw-bold">Durée en semaines</label>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (weeks >= 1 && weeks <= 12) {
                  ...
                  calculateDatesFromDuration(weeks);
                }
              }}
            />
            <span className="input-group-text">semaines</span>
          </div>
        </div>

        <div className="date-range">
          <div className="date-input mb-3">
            <label className="fw-bold">Date et heure de début</label>
            <div className="input-group">
              ...
              <input
                type="date"
                className={`form-control form-control-lg ${errors.dates ? 'is-invalid' : ''}`}
                value={startDate}
                onChange={(e) => {
                  setStartDate(e.target.value);
                  calculateDatesFromDuration(durationWeeks);
          ...
          <div className="date-input mb-3">
            <label className="fw-bold">Date et heure de fin</label>
            <div className="input-group">
              <span className="input-group-text bg-light">
                <i className="fas fa-calendar text-primary"></i>
              </span>
        ...
        <div className="mt-4 p-3 bg-light rounded">
          <h6 className="fw-bold mb-3">Durées recommandées :</h6>
          <ul className="list-unstyled mb-0">
            <li className="mb-2">
              <i className="fas fa-clock text-primary me-2"></i>
              Évaluation annuelle : {selectedEmployees.length <= 1 ? '3-5 jours' : '2-3 semaines'}
            </li>
            <li className="mb-2">
              <i className="fas fa-clock text-primary me-2"></i>
              Évaluation de performance : {selectedEmployees.length <= 1 ? '2-3 jours' : '1-2 semaines'}
            </li>
            <li className="mb-2">
              <i className="fas fa-clock text-primary me-2"></i>
              Évaluation de compétences : {selectedEmployees.length <= 1 ? '2-3 jours' : '1-2 semaines'}
            </li>
            <li>
              <i className="fas fa-clock text-primary me-2"></i>
              Évaluation de progression : {selectedEmployees.length <= 1 ? '1-2 jours' : '5-7 jours'}
            </li>
          </ul>
        ...
        <div className="mt-2 text-muted small">
          ...
          <ul className="mb-0">
            <li>Évaluation annuelle : {selectedEmployees.length <= 1 ? '3-5 jours' : '2-3 semaines'}</li>
            <li>Évaluation de performance : {selectedEmployees.length <= 1 ? '2-3 jours' : '1-2 semaines'}</li>
            <li>Évaluation de compétences : {selectedEmployees.length <= 1 ? '2-3 jours' : '1-2 semaines'}</li>
            <li>Évaluation de progression : {selectedEmployees.length <= 1 ? '1-2 jours' : '5-7 jours'}</li>
          </ul>
