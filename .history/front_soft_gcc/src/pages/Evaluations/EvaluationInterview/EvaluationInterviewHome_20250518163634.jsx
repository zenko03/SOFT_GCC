...
function EvaluationInterviewHome() {
  ...
  const handleOpenModal = useCallback(async (employeeId) => {
    ...
    console.log("Propriétés disponibles dans employee:", Object.keys(employee || {}));
    ...
    try {
      // IMPORTANT: Utiliser directement l'employé de la liste au lieu de faire un appel API
      // Cela évite les problèmes potentiels liés à l'incompatibilité des formats
      console.log(`Utilisation directe de l'employé ID: ${employeeId} depuis la liste`);
      
      // Ajouter l'employé concerné dans la liste des participants
      const employeeIdNumber = Number(employeeId);
      
      // Ajouter les participants par défaut, mais sans dupliquer l'employé concerné
      const defaultParticipants = participantsList
        .filter(participant => participant.isDefault)
        .map(participant => Number(participant.employeeId || participant.EmployeeId || participant.id || participant.Id));
      
      // S'assurer que l'employé concerné est en premier dans la liste
      const allParticipants = [employeeIdNumber, ...defaultParticipants.filter(id => id !== employeeIdNumber)];
      
      console.log("Liste finale des participants:", allParticipants);
      
      // Stocker temporairement une référence à l'employé sélectionné pour le modal
      // Ce sera disponible globalement dans le composant
      if (employee) {
        ...
        console.log("Employé stocké globalement:", window.selectedEmployeeForModal);
      } else {
        console.error("ERREUR: Tentative de stocker un employé null/undefined dans la variable globale");
      }
      
      // Définir evaluationDetails avec l'employé sélectionné directement depuis la liste
      setEvaluationDetails({
        evaluationId: parseInt(employee.evaluationId) || '',
        scheduledDate: today,
        participants: allParticipants,
        selectedEmployee: employee, // Utiliser directement l'employé de la liste
      });
      
      console.log("Ouverture du modal avec l'employé de la liste:", employee);
  ...
  const handleMassPlanning = useCallback(async () => {
    try {
      ...
      const selectedEmployee = evaluationDetails.selectedEmployee;
              ...
              <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable custom-modal">
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    {/* Debug info */}
                    <div className="debug-info small text-muted mb-3" style={{whiteSpace: 'pre-wrap'}}>
                      selectedEmployee: {JSON.stringify(evaluationDetails.selectedEmployee, null, 2)}
                    </div>
                    ...
                    <form>
                      ...
                      <ParticipantsSelector
                        ...
                        currentEmployeeId={evaluationDetails.participants && evaluationDetails.participants.length > 0 ? Number(evaluationDetails.participants[0]) : null}
                      />
                    </form>
                  </div>
