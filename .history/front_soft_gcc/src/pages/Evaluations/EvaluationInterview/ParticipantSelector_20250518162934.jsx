...
const ParticipantsSelector = ({
  ...
  useEffect(() => {
    ...
    console.log("ParticipantSelector - currentEmployeeId:", currentEmployeeId);
    
    let employeeFound = null;
    
    // Priorité 1: Utiliser fetchedEmployee s'il est disponible
    if (fetchedEmployee) {
      console.log("ParticipantSelector - propriétés disponibles:", Object.keys(fetchedEmployee));
      employeeFound = fetchedEmployee;
    } 
    // Priorité 2: Chercher l'employé dans participantsList
    else if (currentEmployeeId && participantsList && participantsList.length > 0) {
      employeeFound = participantsList.find(p => {
        const pId = Number(p.employeeId || p.EmployeeId || p.id || p.Id);
        return pId === Number(currentEmployeeId);
      });
      
      if (employeeFound) {
        console.log("ParticipantSelector - employé trouvé dans participantsList:", employeeFound);
      }
    }
    // Priorité 3: Si employees est disponible, chercher dedans
    else if (currentEmployeeId && employees && employees.length > 0) {
      employeeFound = employees.find(e => Number(e.employeeId || e.EmployeeId) === Number(currentEmployeeId));
      
      if (employeeFound) {
        console.log("ParticipantSelector - employé trouvé dans employees:", employeeFound);
      }
    }
    
    // Si un employé a été trouvé d'une façon ou d'une autre
    if (employeeFound) {
      // Normaliser l'employé trouvé
      const employeeId = Number(employeeFound.employeeId || employeeFound.EmployeeId);
      const firstName = employeeFound.firstName || employeeFound.FirstName;
      const lastName = employeeFound.lastName || employeeFound.LastName;
      ...
      const normalizedEmployee = {
        ...
        firstName: firstName,
        lastName: lastName,
        position: employeeFound.position || employeeFound.Position,
        department: employeeFound.department || employeeFound.Department
      };
      ...
      console.log("ParticipantSelector - ATTENTION: Aucun employé trouvé par aucune méthode!");
    }
  }, [fetchedEmployee, currentEmployeeId, participantsList, selectedParticipants, setSelectedParticipants, employees]);
