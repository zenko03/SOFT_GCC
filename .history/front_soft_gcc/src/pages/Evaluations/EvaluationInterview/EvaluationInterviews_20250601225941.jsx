...
const EvaluationInterviews = () => {
  ...
  // Mettre à jour la fonction handleSave pour s'assurer qu'elle fonctionne
  const handleSave = async () => {
    try {
      ...
      // Former le payload avec les noms de champs exacts attendus par le backend
      const payload = {
        notes: JSON.stringify(formData), 
        status: interview.status || 20 // Statut par défaut (IN_PROGRESS) si non spécifié
      };

      console.log("Payload final:", payload);
      
      // Essayer d'abord une approche directe pour mettre à jour uniquement les notes
      const response = await axios.put(
        `https://localhost:7082/api/EvaluationInterview/${interview.interviewId}/notes`,
        payload
      );
      ...
      try {
        ...
        if (updateResponse.status === 200 || updateResponse.status === 204) {
          toast.success("Données d'entretien sauvegardées avec succès (méthode alternative)");
          return;
        }
      } catch (fallbackError) {
