...
function EvaluationInterviewHome() {
  ...
  // Ajout d'un état pour gérer le participant actuellement sélectionné
  ...
  // Fonction pour gérer le changement de participant actuel
  const handleCurrentParticipantChange = useCallback((e) => {
    setCurrentParticipant(e.target.value);
  }, []);

  // Ajout d'un participant à la liste
  const handleAddParticipant = useCallback(() => {
    if (!currentParticipant) return; // Ne rien faire si aucun participant n'est sélectionné

    // Vérifier si le participant n'est pas déjà dans la liste
    if (!evaluationDetails.participants.includes(Number(currentParticipant))) {
      // S'assurer que le participant existe dans la liste des participants disponibles
      const participantExists = participantsList.some(p => String(p.employeeId || p.id) === String(currentParticipant));

      if (participantExists) {
        setEvaluationDetails(prev => ({
          ...prev,
          participants: [...prev.participants, Number(currentParticipant)]
        }));
        setCurrentParticipant(''); // Réinitialiser le sélecteur
      } else {
        toast.error('Participant non trouvé dans la liste');
      }
    } else {
      toast.error('Ce participant est déjà dans la liste');
    }
  }, [currentParticipant, evaluationDetails.participants, participantsList]);

  // Retirer un participant de la liste
  const handleRemoveParticipant = useCallback((participantId) => {
    const selectedEmployee = evaluationDetails.selectedEmployee || window.selectedEmployeeForModal;
    const employeeId = selectedEmployee?.employeeId || selectedEmployee?.EmployeeId;

    // Empêcher la suppression si c'est l'employé concerné par l'évaluation
    if (participantId === employeeId) {
      toast.warning("L'employé évalué ne peut pas être retiré de la liste des participants");
      return;
    }
  ...
  }, [evaluationDetails.selectedEmployee]);
                  ...
                  <div className="modal-body">
                    ...
                    <form>
                      <div className="form-group mb-4">
                        <label htmlFor="scheduledDateTime" className="form-label">Date et heure planifiée</label>
                      ...
                      {/* Nouvelle section de sélection des participants */}
                      <div className="form-group mb-4">
                        <label className="form-label">Participants à l'entretien</label>
                        
                        <div className="input-group mb-3">
                          <select
                            className="form-control"
                            value={currentParticipant}
                            onChange={handleCurrentParticipantChange}
                          >
                            <option value="">Sélectionner un participant</option>
                            {participantsList.map((participant) => (
                              <option 
                                key={participant.employeeId || participant.id} 
                                value={participant.employeeId || participant.id}
                              >
                        ...
                        <div className="selected-participants-list">
                          ...
                          {evaluationDetails.participants.length > 0 ? (
                            <ul className="list-group">
                              {evaluationDetails.participants.map((participantId) => {
                                const participant = participantsList.find(
                                  p => (p.employeeId || p.id) === participantId
                                );
                                
                                const isMainEmployee = evaluationDetails.selectedEmployee?.employeeId === participantId || 
                                                    window.selectedEmployeeForModal?.employeeId === participantId;
                                ...
                                return participant ? (
                                  <li key={participantId} className="list-group-item d-flex justify-content-between align-items-center">
                                    <span>
                                      {participant.firstName} {participant.lastName}
                                      {participant.role && ` (${participant.role})`}
                                      {isMainEmployee && <span className="badge bg-info ms-2">Employé évalué</span>}
                                    </span>
                                    {!isMainEmployee && (
                                      <button
                                        type="button"
                                  ...
                                  <li key={participantId} className="list-group-item text-muted">
                            ...
                            <div className="alert alert-warning">
                              Aucun participant sélectionné. Veuillez ajouter au moins un participant.
                            </div>
                          )}
                        </div>
                      </div>
