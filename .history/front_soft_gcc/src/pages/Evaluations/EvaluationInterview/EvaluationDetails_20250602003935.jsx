...
const EvaluationDetails = ({ interview, employeeId }) => {
    ...
    const handleValidationSubmit = async (e) => {
        ...
        try {
            ...
            // Log détaillé des IDs utilisés pour aider au diagnostic
            console.log("=== DIAGNOSTIC DES IDs ===");
            console.log("ID depuis actualInterviewId:", actualInterviewId);
            console.log("ID depuis interviewDetails:", interviewDetails?.interviewId);
            console.log("ID d'évaluation:", interviewDetails?.evaluationId);
            console.log("ID utilisé pour la requête:", interviewId);
            console.log("=========================");
            
            console.log("Tentative de validation avec ID:", interviewId);
            
            // Créer le payload avec les noms de propriétés correspondant EXACTEMENT aux colonnes de la base de données
            const payload = {
                notes: interviewDetails.notes, // Préserver les notes existantes
                status: 25, // PENDING_VALIDATION
                InterviewId: interviewId, // Ajouter l'ID explicitement
            };
            
            // Ajouter les champs spécifiques selon le rôle - utiliser les noms de colonnes exacts
            if (isManager) {
                payload.manager_approval = validationForm.approved;
                payload.manager_comments = validationForm.comments;
                
                // Si directeur et déjà validé par directeur, préserver ses données
                if (interviewDetails.directorApproval !== null) {
                    payload.director_approval = interviewDetails.directorApproval;
                    payload.director_comments = interviewDetails.directorComments;
                }
            } else if (isDirector) {
                payload.director_approval = validationForm.approved;
                payload.director_comments = validationForm.comments;
                
                // Si manager et déjà validé par manager, préserver ses données
                if (interviewDetails.managerApproval !== null) {
                    payload.manager_approval = interviewDetails.managerApproval;
                    payload.manager_comments = interviewDetails.managerComments;
                }
            }
            
            // Si validation complète, mettre le statut à COMPLETED (30)
            if ((isManager && payload.manager_approval && interviewDetails.directorApproval) || 
                (isDirector && payload.director_approval && interviewDetails.managerApproval)) {
                payload.status = 30; // COMPLETED
            }
            
            console.log("Envoi de la validation avec les noms de colonnes SQL:", payload);
            
            try {
                // Première tentative : Utiliser un endpoint qui met directement à jour l'entretien
                const response = await axios.put(
                    `https://localhost:7082/api/EvaluationInterview/${interviewId}`,
                    payload
                );
                ...
                // Si la première tentative échoue, essayer avec l'endpoint complete-interview
                console.error("Première tentative échouée, essai avec endpoint complete-interview:", firstError);
                
                try {
                    // Même payload mais avec les propriétés au format camelCase
                    ...
                    if (Object.prototype.hasOwnProperty.call(payload, 'manager_approval')) {
                        completePayload.managerApproval = payload.manager_approval;
                    }
                    if (Object.prototype.hasOwnProperty.call(payload, 'manager_comments')) {
                        completePayload.managerComments = payload.manager_comments;
                    }
                    if (Object.prototype.hasOwnProperty.call(payload, 'director_approval')) {
                        completePayload.directorApproval = payload.director_approval;
                    }
                    if (Object.prototype.hasOwnProperty.call(payload, 'director_comments')) {
                        completePayload.directorComments = payload.director_comments;
                    }
                    
                    console.log("Tentative avec complete-interview:", completePayload);
                    ...
                    console.error("Deuxième tentative échouée, essai avec update-interview:", secondError);
                    ...
                    try {
                        ...
                        const updatePayload = {
                            ...
                            // Ajouter les propriétés de validation
                            manager_approval: payload.manager_approval,
                            manager_comments: payload.manager_comments,
                            director_approval: payload.director_approval,
                            director_comments: payload.director_comments
                        };
                        
                        console.log("Tentative avec update-interview:", updatePayload);
                        ...
                        console.error("Toutes les tentatives ont échoué:", fallbackError);
                        throw fallbackError; // Relancer pour être capturé par le catch externe
                    }
                }
            }
            
            // Dernière tentative: appel POST pour création directe
            try {
                console.log("Tentative finale avec un appel POST pour mise à jour directe");
                
                const directPayload = {
                    ...payload,
                    // Ajouter les champs potentiellement manquants
                    evaluationId: interviewDetails.evaluationId,
                    scheduled_date: interviewDetails.interviewDate || new Date().toISOString()
                };
                
                const directResponse = await axios.post(
                    `https://localhost:7082/api/EvaluationInterview/direct-update`,
                    directPayload
                );
                
                if (directResponse.status === 200 || directResponse.status === 204 || directResponse.status === 201) {
                    toast.success("Validation enregistrée avec succès");
                ...
                console.error("Tentative finale échouée:", finalError);
                throw finalError;
            }
            
            // Si on arrive ici, c'est que toutes les tentatives ont échoué mais sans erreur
            throw new Error("Toutes les tentatives de validation ont échoué sans erreur spécifique");
            ...
            if (error.response) {
                ...
                if (error.response.status === 400) {
                    toast.error("Erreur de validation des données. Vérifiez les informations saisies.");
                } else if (error.response.status === 404) {
                    toast.error("Entretien non trouvé. Il a peut-être été supprimé.");
                } else {
                    toast.error(`Erreur ${error.response.status}: ${error.response.statusText || "Erreur serveur"}`);
                }
            } else {
