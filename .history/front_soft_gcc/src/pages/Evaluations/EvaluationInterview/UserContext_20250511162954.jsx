...
export const UserProvider = ({ children }) => {
    ...
    const hasPermission = (permissionName) => {
        ...
        if (!userPermissions || !Array.isArray(userPermissions)) {
            console.log('Aucune permission disponible ou format invalide');
            return false;
        }

        const hasPermission = userPermissions.some(permission => {
            const matches = permission.name === permissionName;
            console.log(`Vérification de ${permission.name} contre ${permissionName}: ${matches}`);
            return matches;
        });

        console.log('Résultat final de la vérification:', hasPermission);
    ...
    const refreshPermissions = async () => {
        try {
            console.log("Début du rechargement des permissions...");
            const token = localStorage.getItem('token');
            if (!token) {
                console.log("Aucun token trouvé pour le rechargement des permissions");
                setUserPermissions([]);
                return;
            }

            const response = await axios.get('https://localhost:7082/api/Permission/user', {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
