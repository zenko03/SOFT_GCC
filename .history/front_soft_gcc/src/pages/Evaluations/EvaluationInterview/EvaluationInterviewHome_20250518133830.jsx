...
function EvaluationInterviewHome() {
  ...
  const handleOpenModal = useCallback(async (employeeId) => {
    ...
    // Ajouter des logs pour vérifier l'ID d'évaluation
    console.log("Employé sélectionné:", employee);
    console.log("ID d'évaluation extrait:", employee.evaluationId);
    ...
    setEvaluationDetails((prev) => ({
      ...prev,
      evaluationId: parseInt(employee.evaluationId) || '',
      scheduledDate: today,
      participants: [
        Number(employee.employeeId),
        ...participantsList
          .filter(participant => participant.isDefault)
          .map(participant => Number(participant.employeeId || participant.id)),
      ],
    }));
    
    // Log après avoir défini evaluationDetails
    console.log("evaluationDetails après mise à jour:", {
      evaluationId: parseInt(employee.evaluationId) || '',
      scheduledDate: today,
      participants: [
        Number(employee.employeeId),
  ...
  }, [evaluationDetails, employees, handleError, fetchEmployeesWithoutEvaluations]);
  ...
  const startInterview = useCallback(async (employeeId) => {
    try {
      ...
      // Variables pour stocker l'entretien récupéré
      let interview = null;
      let usedFallbackMethod = false;
      
      try {
        // Étape 1: Essayer de récupérer l'interview par participant (méthode qui peut échouer)
        console.log("Tentative de récupération de l'entretien via participant...");
        ...
        console.log("Entretien récupéré par API participant:", interview);
      } catch (participantError) {
        console.error("Erreur lors de la récupération par participant:", participantError);
        console.error("Détails:", participantError.response?.data?.message);
        usedFallbackMethod = true;
        
        // Solution de secours: Créer un entretien simple avec l'ID de l'évaluation
        if (employee?.evaluationId) {
          console.warn("UTILISATION D'UNE SOLUTION DE SECOURS: création d'un entretien basique");
          
          // Vérifier si l'employé a une date d'entretien
          const interviewDate = employee.interviewDate || new Date().toISOString();
          
          // Créer un objet entretien basique
          interview = {
            // Si nous ne connaissons pas l'ID d'entretien, nous utiliserons l'ID d'évaluation comme valeur temporaire
            interviewId: parseInt(employee.evaluationId), 
            evaluationId: parseInt(employee.evaluationId),
            interviewDate: interviewDate,
            status: INTERVIEW_STATUS.PLANNED, // Statut planifié (10)
            notes: null,
          ...
          console.log("Entretien créé comme solution de secours:", interview);
        } else {
          throw new Error("Impossible de créer un entretien de secours: évaluation ID manquant");
        }
      }
      ...
      // Si nous n'avons pas utilisé la solution de secours, démarrer l'entretien via l'API
      if (!usedFallbackMethod) {
        ...
        try {
          ...
          console.error("Erreur lors du démarrage de l'entretien:", startError);
          console.error("Détails:", startError.response?.data);
          // Continuer malgré l'erreur, puisque nous avons toujours l'entretien
          toast.warning('Impossible de mettre à jour le statut, mais vous pouvez continuer.');
        }
      } else {
        toast.warning('Utilisation d\'un mode dégradé pour l\'entretien. Certaines fonctionnalités peuvent être limitées.');
      }
  ...
  }, [navigate, handleError, employees, INTERVIEW_STATUS]);
