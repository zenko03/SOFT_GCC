...
function EvaluationInterviewHome() {
  ...
  const renderActionButton = useCallback((employee) => {
    ...
    if (employee.interviewStatus === INTERVIEW_STATUS.PENDING_VALIDATION) {
      ...
      const redirectToValidation = async (employeeId) => {
        try {
          ...
          console.log("Redirection vers la page de détails pour validation avec:", {interview, employeeId});
          
          // Rediriger directement vers la page de détails de l'entretien
          navigate("/evaluation-details", { 
            state: { 
              interview, 
              employeeId,
    ...
    return null;
  }, [user, today, getAndCancelInterview, getAndEditInterview, startInterview, canValidateAsManager, canValidateAsDirector, canImportEvaluation, INTERVIEW_STATUS, handleOpenModal, navigate]);
