...
const EvaluationDetails = ({ interview, employeeId }) => {
    ...
    const handleValidationSubmit = async (e) => {
        ...
        try {
            ...
            // Déterminer les valeurs d'approbation finales
            let managerApproval = interviewDetails?.managerApproval;
            let managerComments = interviewDetails?.managerComments || '';
            let directorApproval = interviewDetails?.directorApproval;
            let directorComments = interviewDetails?.directorComments || '';
            
            // Mettre à jour seulement la valeur correspondant au rôle de l'utilisateur
            if (isManager) {
                managerApproval = validationForm.approved;
                managerComments = validationForm.comments;
                console.log("Mise à jour pour manager:", { managerApproval, managerComments });
            } else if (isDirector) {
                directorApproval = validationForm.approved;
                directorComments = validationForm.comments;
                console.log("Mise à jour pour directeur:", { directorApproval, directorComments });
            }
            
            // Déterminer le statut en fonction des approbations
            let status = 25; // PENDING_VALIDATION par défaut
            
            // Si les deux validations sont positives, marquer comme COMPLETED (30)
            if (managerApproval === true && directorApproval === true) {
                status = 30; // COMPLETED
            }
            
            console.log("=== VALEURS FINALES POUR MISE À JOUR ===");
            console.log("managerApproval:", managerApproval);
            console.log("managerComments:", managerComments);
            console.log("directorApproval:", directorApproval);
            console.log("directorComments:", directorComments);
            console.log("status:", status);
            console.log("=========================");
            
            // Créer le DTO CompleteInterviewDto en utilisant EXACTEMENT les noms de propriétés attendus
            const completeInterviewDto = {
            ...
            console.log("Envoi du DTO CompleteInterview:", completeInterviewDto);
            
            // Appel direct à l'API complete-interview
            const response = await axios.put(
                `https://localhost:7082/api/EvaluationInterview/complete-interview/${interviewId}`,
                completeInterviewDto
            );
            
            console.log("Réponse de l'API complete-interview:", response);
            ...
            if (error.response) {
                ...
                if (error.response.status === 400) {
                    toast.error("Erreur de validation: " + (error.response.data || "Vérifiez les données saisies"));
                } else {
