...
const EvaluationDetails = ({ interview, employeeId }) => {
    ...
    const handleValidationSubmit = async (e) => {
        ...
        try {
            ...
            // Si l'appel principal échoue, essayer avec update-interview
            throw new Error("L'appel à complete-interview n'a pas fonctionné comme prévu");
            
        } catch (error) {
            console.error("Erreur lors de la validation:", error);
            
            // Essayer avec update-interview comme solution de secours
            try {
                console.error("Tentative avec update-interview comme solution de secours");
                
                const interviewId = actualInterviewId || interviewDetails.interviewId;
                const updatePayload = {
                    newStatus: interviewDetails.status || 25, // PENDING_VALIDATION
                    newDate: interviewDetails.interviewDate || new Date().toISOString(),
                    newParticipantIds: [],
                };
                
                // Ajouter les propriétés de validation directement dans le payload de UpdateInterviewDto
                // Note: ce n'est peut-être pas supporté par le backend mais on essaie
                if (isManager) {
                    updatePayload.ManagerApproval = validationForm.approved;
                    updatePayload.ManagerComments = validationForm.comments;
                } else if (isDirector) {
                    updatePayload.DirectorApproval = validationForm.approved;
                    updatePayload.DirectorComments = validationForm.comments;
                }
                
                console.log("Payload pour update-interview:", updatePayload);
                ...
                console.error("Erreur sur la tentative de secours:", fallbackError);
