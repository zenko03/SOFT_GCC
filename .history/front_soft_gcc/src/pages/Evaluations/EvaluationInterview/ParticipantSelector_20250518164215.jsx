...
const ParticipantsSelector = ({
  ...
  // Effet pour ajouter automatiquement l'employé concerné à la liste des participants
  useEffect(() => {
    ...
    if (targetEmployee) {
      console.log("Employé cible trouvé:", targetEmployee);
      
      // Normaliser l'ID de l'employé
      const employeeId = Number(targetEmployee.employeeId || targetEmployee.EmployeeId);
      
      if (employeeId && !selectedParticipants.includes(employeeId)) {
        console.log(`Ajout automatique de l'employé ID ${employeeId} à la liste des participants`);
      ...
      console.warn("Aucun employé cible trouvé pour l'ajout automatique");
    }
  }, [fetchedEmployee, setSelectedParticipants]);
  ...
  const handleRemoveParticipant = (participantId) => {
    ...
    const targetEmployeeId = targetEmployee ? 
      Number(targetEmployee.employeeId || targetEmployee.EmployeeId) : null;
    
    // Empêcher la suppression de l'employé concerné
    if (targetEmployeeId && participantId === targetEmployeeId) {
      console.log("Tentative de suppression de l'employé concerné - opération bloquée");
      return;
    }
