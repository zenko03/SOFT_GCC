...
const EvaluationDetails = ({ interview, employeeId }) => {
    ...
    const handleValidationSubmit = async (e) => {
        ...
        try {
            ...
            // APPROCHE DIRECTE AVEC update-interview
            // Cette approche utilise l'endpoint qui fonctionne déjà correctement pour la mise à jour
            try {
                console.log("Tentative directe avec update-interview");
                
                // Préparer le payload avec les propriétés que update-interview attend
                const updatePayload = {
                    newStatus: 25, // PENDING_VALIDATION ou 30 pour COMPLETED
                    newDate: interviewDetails.interviewDate || new Date().toISOString(),
                    newParticipantIds: [], // Garder la liste existante
                    
                    // Ajouter les notes dans le payload
                    notes: interviewDetails.notes,
                    
                    // Ajouter directement les propriétés d'approbation avec leurs noms de colonnes SQL
                    // pour maximiser les chances qu'elles soient correctement mappées
                    manager_approval: isManager ? validationForm.approved : interviewDetails.managerApproval,
                    manager_comments: isManager ? validationForm.comments : interviewDetails.managerComments,
                    director_approval: isDirector ? validationForm.approved : interviewDetails.directorApproval,
                    director_comments: isDirector ? validationForm.comments : interviewDetails.directorComments
                };
                
                // Si validation complète, mettre le statut à COMPLETED (30)
                if ((isManager && updatePayload.manager_approval && interviewDetails.directorApproval) || 
                    (isDirector && updatePayload.director_approval && interviewDetails.managerApproval)) {
                    updatePayload.newStatus = 30; // COMPLETED
                }
                
                console.log("Payload direct pour update-interview:", updatePayload);
                ...
                console.error("Erreur avec l'approche directe:", directError);
                // On continue avec les autres approches
            }
            
            // APPROCHE STANDARD AVEC complete-interview
            console.log("Tentative standard avec complete-interview");
            
            // Créer le payload en utilisant les noms de propriétés attendus par le DTO du backend
            const payload = {
                Notes: interviewDetails.notes, // Préserver les notes existantes
                Status: 25, // PENDING_VALIDATION
            };
            
            // Ajouter les champs spécifiques selon le rôle avec les noms camelCase attendus par le backend
            if (isManager) {
            ...
            console.log("Envoi de la validation avec le format de DTO attendu:", payload);
            
            // Appel de l'API avec le format attendu par le backend
            ...
            throw new Error("Aucune méthode n'a réussi à mettre à jour l'entretien");
