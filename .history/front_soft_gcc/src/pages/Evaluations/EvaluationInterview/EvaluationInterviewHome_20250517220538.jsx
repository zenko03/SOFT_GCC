  ...
  const startInterview = useCallback(async (employeeId) => {
    try {
      ...
      if (startResponse.status === 200) {
        toast.success(startResponse.data.message || 'Entretien démarré avec succès.');
        
        // Actualiser la liste pour refléter le nouvel état
        ...
        // Si le serveur répond, mais pas avec un statut 200
        toast.warning("Réponse inattendue du serveur lors du démarrage de l'entretien.");
      }
    } catch (error) {
      ...
      if (error.response) {
        ...
        toast.error(error.response.data.message || "Erreur lors du démarrage de l'entretien.");
      } else {
        handleError(error, "Erreur lors du démarrage de l'entretien.");
      }
    }
  }, [navigate, handleError, fetchEmployeesWithoutEvaluations]);
