...
const EvaluationDetails = ({ interview, employeeId }) => {
    ...
    // Fonction pour mettre à jour directement les statuts d'approbation
    const updateApprovalStatus = async (interviewId, payload) => {
        try {
            console.log("TENTATIVE DE MISE À JOUR DIRECTE - ID:", interviewId);
            console.log("Payload:", payload);
            
            // Version 1: update-interview avec les approbations explicitement dans le body
            const updateResponse = await axios.put(
                `https://localhost:7082/api/EvaluationInterview/update-interview/${interviewId}`,
                {
                    ...payload,
                    newStatus: payload.isCompleted ? 30 : 25, // PENDING_VALIDATION ou COMPLETED
            ...
            if (updateResponse.status === 200 || updateResponse.status === 204) {
                return {success: true, method: 'update-interview'};
            }
        } catch (updateError) {
            console.error("Erreur avec update-interview:", updateError);
            
            // Version 2: complete-interview avec les noms corrects
            try {
                const completeResponse = await axios.put(
                    `https://localhost:7082/api/EvaluationInterview/complete-interview/${interviewId}`,
                    {
                        ManagerApproval: payload.manager_approval,
                        ManagerComments: payload.manager_comments,
                        DirectorApproval: payload.director_approval,
                        DirectorComments: payload.director_comments,
                        Notes: payload.notes,
                        Status: payload.isCompleted ? 30 : 25
                    }
                );
                
                if (completeResponse.status === 200 || completeResponse.status === 204) {
                    return {success: true, method: 'complete-interview'};
                }
            } catch (completeError) {
        ...
        throw new Error("Toutes les tentatives de mise à jour ont échoué");
    };
    ...
    const handleValidationSubmit = async (e) => {
        ...
        try {
            ...
            // IMPORTANT: Vérifier les valeurs du formulaire de validation
            console.log("=== VALEURS DU FORMULAIRE DE VALIDATION ===");
            console.log("isManager:", isManager);
            console.log("isDirector:", isDirector);
            console.log("validationForm:", validationForm);
            console.log("validationForm.approved:", validationForm.approved);
            console.log("Type de validationForm.approved:", typeof validationForm.approved);
            console.log("=========================");
            
            // CORRECTION: Forcer les valeurs explicitement selon la décision de l'utilisateur
            const managerApproval = isManager ? validationForm.approved : interviewDetails?.managerApproval;
            const managerComments = isManager ? validationForm.comments : interviewDetails?.managerComments || '';
            const directorApproval = isDirector ? validationForm.approved : interviewDetails?.directorApproval;
            const directorComments = isDirector ? validationForm.comments : interviewDetails?.directorComments || '';
            
            console.log("=== VALEURS FINALES POUR MISE À JOUR ===");
            console.log("managerApproval:", managerApproval);
            console.log("directorApproval:", directorApproval);
            console.log("=========================");
            
            // Créer directement un payload avec les valeurs booléennes explicites
            const updatePayload = {
            ...
            // Déterminer si l'entretien est complété (les deux validations sont faites et positives)
            const isCompleted = 
                (updatePayload.manager_approval === true && updatePayload.director_approval === true);
            
            updatePayload.isCompleted = isCompleted;
            
            console.log("Tentative de mise à jour d'approbation avec payload:", updatePayload);
            
            // Appel direct à l'API update-interview (sans passer par la fonction updateApprovalStatus)
            // Cette approche devrait contourner les problèmes potentiels du service backend
            try {
                console.log("Appel direct à update-interview avec ID:", interviewId);
                
                // Construire un payload spécifique pour update-interview
                const directPayload = {
                    newStatus: isCompleted ? 30 : 25,
                    newDate: interviewDetails.interviewDate || new Date().toISOString(),
                    newParticipantIds: [],
                    
                    // IMPORTANT: inclure explicitement les champs d'approbation avec des valeurs booléennes
                    manager_approval: managerApproval,
                    manager_comments: managerComments,
                    director_approval: directorApproval,
                    director_comments: directorComments,
                    
                    // Inclure les notes pour les préserver
                    notes: interviewDetails.notes
                };
                
                console.log("Payload final pour update-interview:", directPayload);
                ...
                // Si l'appel direct échoue, essayer la fonction précédente comme fallback
                try {
                    const result = await updateApprovalStatus(interviewId, updatePayload);
                    
                    console.log("Résultat de la mise à jour:", result);
                    
                    if (result.success) {
                        toast.success(`Validation enregistrée avec succès (méthode: ${result.method})`);
                    ...
                    console.error("Erreur avec la méthode fallback:", fallbackError);
                    throw fallbackError;
                }
            }
            
            throw new Error("La mise à jour n'a pas réussi malgré un statut de succès");
