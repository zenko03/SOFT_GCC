...
const AdminSettings = () => {
  ...
  const fetchEvaluationQuestions = async (evaluationId) => {
    ...
    try {
      ...
      // Utiliser l'endpoint paginé
      const response = await axios.get(`https://localhost:7082/api/EvaluationQuestion/paginated`, {
        headers: { Authorization: `Bearer ${token}` },
        params: {
  ...
  const handleEvaluationSelect = (evaluation) => {
    ...
    // Réinitialiser la pagination lors de la sélection d'une nouvelle évaluation
    setCurrentPage(1);
