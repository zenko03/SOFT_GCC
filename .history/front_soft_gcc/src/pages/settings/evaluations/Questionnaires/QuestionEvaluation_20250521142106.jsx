    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const requestData = currentQuestion
                ? {
                    // Pour mise à jour, utiliser l'ID existant et le format attendu par le backend
                    // Doit correspondre exactement à la structure EvaluationQuestion.cs
                    questionId: currentQuestion.id || currentQuestion.questionId || currentQuestion.questiondId,
                    question: formData.question,
                    evaluationTypeId: evaluationTypeId,
                    positionId: positionId,
                    CompetenceLineId: competenceLineId,  // Utiliser le bon format de propriété
                    ResponseTypeId: responseTypeId,      // Utiliser le bon format de propriété
            ...
            if (error.response) {
                ...
                alert(`Erreur: ${JSON.stringify(error.response.data) || "Une erreur inconnue s'est produite"}`);
            } else {
                        ...
                        <div className="form-group col-md-6">
                            ...
                            >
                                ...
                                {competenceLines.map(comp => (
                                    <option key={comp.CompetenceLineId || comp.competenceLineId} value={comp.CompetenceLineId || comp.competenceLineId}>
                                        {comp.Description || comp.description || comp.competenceName || "Ligne de compétence sans description"}
                                    </option>
                                ))}
                            </select>
                        </div>
                        <div className="form-group col-md-6">
                            ...
                            >
                                {responseTypes.map(type => (
                                    <option key={type.ResponseTypeId || type.responseTypeId} value={type.ResponseTypeId || type.responseTypeId}>
                                        {(type.TypeName || type.typeName)} - {(type.Description || type.description)}
                                    </option>
                                ))}
                            </select>
                        </div>
                        ...
                        <li key={question.questionId || question.questiondId} className="list-group-item d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                {question.competenceLineId && (
                                    <div>Compétence: {
                                        competenceLines.find(comp => 
                                            (comp.CompetenceLineId || comp.competenceLineId) === question.competenceLineId
                                        )?.Description || 
                                        competenceLines.find(comp => 
                                            (comp.CompetenceLineId || comp.competenceLineId) === question.competenceLineId
                                        )?.description || 
                                ...
                                <div>Type de réponse: {
                                    responseTypes.find(type => 
                                        (type.ResponseTypeId || type.responseTypeId) === question.responseTypeId
                                    )?.TypeName || 
                                    responseTypes.find(type => 
                                        (type.ResponseTypeId || type.responseTypeId) === question.responseTypeId
                                    )?.typeName || 
