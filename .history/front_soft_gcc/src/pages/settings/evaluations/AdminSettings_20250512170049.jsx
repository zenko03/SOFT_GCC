...
const AdminSettings = () => {
    ...
    // Gérer le changement de la taille de page
    ...
    // Effet pour recharger les questions lors du changement de page
    useEffect(() => {
        if (selectedEvaluation) {
            fetchEvaluationQuestions(selectedEvaluation.id);
        }
    ...
    // Modification de fetchEvaluationQuestions dans AdminSettings.jsx
    const fetchEvaluationQuestions = async (evaluationTypeId) => {
        setLoading(true);
        try {
            const token = localStorage.getItem('token');

            // Utiliser le nouvel endpoint paginé
            const response = await axios.get(`https://localhost:7082/api/evaluation/${evaluationTypeId}/questions/paginated`, {
                headers: { Authorization: `Bearer ${token}` },
                params: {
            ...
            const questionsData = response.data.items;
            ...
            {!selectedEvaluation ? (
                ...
                <>
                    ...
                    <Card sx={{ mb: 3 }}>
                        ...
                        // Composants UI pour la pagination (à ajouter en bas du tableau des questions)
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 3 }}>
                            <FormControl sx={{ minWidth: 120 }} size="small">
                                <InputLabel id="page-size-select-label">Par page</InputLabel>
                                <Select
