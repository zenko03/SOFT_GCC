...
const AdminSettings = () => {
  ...
  // Récupérer les questions d'une évaluation spécifique avec pagination
  const fetchEvaluationQuestions = async (evaluationId) => {
    setLoading(true);
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://localhost:7082/api/evaluation/${evaluationId}/questions/paginated`, {
        params: {
      ...
      const { items, totalCount, totalPages: pages } = response.data;
      setQuestions(items);
      setTotalPages(pages);
  ...
  // Récupérer les questions à chaque changement de page
  useEffect(() => {
    if (selectedEvaluation) {
      fetchEvaluationQuestions(selectedEvaluation.id);
    }
  }, [currentPage, pageSize, selectedEvaluation]);
              ...
              {loading ? (
                ...
                <>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 3, mb: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Typography variant="body2" color="text.secondary" sx={{ mr: 2 }}>
                        Affichage de {(currentPage - 1) * pageSize + 1} à {Math.min(currentPage * pageSize, totalQuestions)} sur {totalQuestions} questions
                      </Typography>
                      <Pagination 
                        count={totalPages} 
                        page={currentPage}
