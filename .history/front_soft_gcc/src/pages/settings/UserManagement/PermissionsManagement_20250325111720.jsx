...
function PermissionsManagement() {
    ...
    const handlePermissionToggle = async (permissionId) => {
        ...
        try {
            ...
            // Envoyer les données sous forme d'objet JSON structuré
            const permissionIds = newPermissions.map(p => p.permissionId);
            const requestData = JSON.stringify(permissionIds);
            
            // Log pour debug
            console.log('Données envoyées (format string):', requestData);

            const response = await axios({
                method: 'put',
                url: `https://localhost:7082/api/Permission/role/${selectedRole.roleId}`,
                data: requestData,
                headers: {
