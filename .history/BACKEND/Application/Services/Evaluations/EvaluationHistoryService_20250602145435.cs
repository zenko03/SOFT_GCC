...
namespace soft_carriere_competence.Application.Services.Evaluations
{
    ...
    string? employeeName = null)
        ...
        public async Task<DetailedStatisticsDto> GetDetailedStatisticsAsync(DateTime? startDate, DateTime? endDate, string? department = null, string? evaluationType = null)
        {
            ...
            {
                Low = evaluations.Count(e => e.OverallScore < 2.5),
                Medium = evaluations.Count(e => e.OverallScore >= 2.5 && e.OverallScore < 4),
                High = evaluations.Count(e => e.OverallScore >= 4),
                Average = evaluations.Average(e => e.OverallScore),
                Min = evaluations.Min(e => e.OverallScore),
                Max = evaluations.Max(e => e.OverallScore)
            };
            ...
            var performanceByYear = await query
                ...
                .Select(g => new YearlyPerformanceDto
                {
                    ...
                    BestDepartment = g.GroupBy(e => e.Department)
                                      .OrderByDescending(dg => dg.Average(e => e.OverallScore))
                                      .Select(dg => dg.Key)
                                      .FirstOrDefault() ?? "N/A"
                })
                .OrderBy(y => y.Year)
            ...
            var trendData = new TrendDto
            {
                ...
                PercentageChange = orderedYears.Count > 1 ?
                                  (orderedYears.Last().AverageScore - orderedYears.First().AverageScore) / 
                ...
                StandardDeviation = CalculateStandardDeviation(evaluations.Select(e => e.OverallScore).ToList())
            };
