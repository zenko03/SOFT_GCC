    ...
    public class EvaluationService
    {
       ...
       string weaknesses,
            ...
            bool enableReminders = false)
        {
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    string evaluationTypeName = evaluationType?.Evaluation_type_id != null ? 
                        (await _context.Evaluation_type.FindAsync(evaluationType.Evaluation_type_id))?.designation ?? "Évaluation" : 
                        "Évaluation";
                    ...
                    foreach (var supervisorId in supervisorIds)
                    {
                        ...
                        {
                            string employeeName = employee != null ? $"{employee.FirstName} {employee.LastName}" : "Un employé";
        ...
        public async Task SendReminderEmailAsync(int userId, DateTime evaluationDate)
        {
            ...
            var evaluationType = await _context.Evaluation_type.FindAsync(currentEvaluation.EvaluationTypeId);
