    ...
    public class EvaluationService
    {
        ...
        public async Task<List<TrainingSuggestionResultDto>> GetTrainingSuggestionsByQuestionsAsync(Dictionary<int, int> ratings)
        {
            try
            {
                ...
                {
                    Console.WriteLine("Aucune note fournie");
                    return new List<TrainingSuggestionResultDto>();
                }

                Console.WriteLine($"Vérification des suggestions pour {ratings.Count} questions avec des scores inférieurs à 3");
                foreach (var rating in ratings)
                {
                    Console.WriteLine($"Question ID: {rating.Key}, Score: {rating.Value}");
                }

                // Vérifier toutes les suggestions disponibles pour déboguer
                var allSuggestions = await _context.TrainingSuggestions.ToListAsync();
                Console.WriteLine($"Total des suggestions dans la base de données: {allSuggestions.Count}");
                
                if (allSuggestions.Count > 0)
                {
                    var sample = allSuggestions.First();
                    Console.WriteLine($"Exemple de suggestion - ID: {sample.TrainingSuggestionId}, QuestionId: {sample.questionId}, Seuil: {sample.scoreThreshold}, TrainingId: {sample.Training}");
                }
                
                // Version originale de la requête avec plus de logging
                var suggestions = await _context.TrainingSuggestions
                    .Join(_context.evaluationQuestions,
                        s => s.questionId,
                        q => q.questionId,
                        (s, q) => new { Suggestion = s, Question = q })
                    .Where(sq => ratings.ContainsKey(sq.Suggestion.questionId) && 
                           ratings[sq.Suggestion.questionId] < sq.Suggestion.scoreThreshold)
                    .Select(sq => new TrainingSuggestionResultDto
                    {
                        Question = sq.Question.question,
                        Training = sq.Suggestion.Training,
                        Details = sq.Suggestion.Details
                    })
                    .ToListAsync();

                Console.WriteLine($"{suggestions.Count} suggestions de formation trouvées après filtrage");
                
                if (suggestions.Count == 0)
                {
                    // Vérifier pourquoi aucune suggestion n'est trouvée
                    foreach (var suggestion in allSuggestions)
                    {
                        if (ratings.ContainsKey(suggestion.questionId))
                        {
                            int score = ratings[suggestion.questionId];
                            Console.WriteLine($"Question {suggestion.questionId}: Score={score}, Seuil={suggestion.scoreThreshold}, Condition remplie: {score < suggestion.scoreThreshold}");
                        }
                        else
                        {
                            Console.WriteLine($"Question {suggestion.questionId}: Pas de score correspondant");
                        }
                    }
                }
