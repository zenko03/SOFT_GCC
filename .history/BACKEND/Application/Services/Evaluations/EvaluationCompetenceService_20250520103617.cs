        ...
        public async Task UpdateEmployeeSkillsAfterEvaluation(int evaluationId)
        {
            try
            {
                ...
                foreach (var result in competenceResults)
                {
                    try
                    {
                        // Modification clé : Diviser les requêtes pour éviter les erreurs NULL
                        // D'abord, récupérer la CompetenceLine
                        ...
                        // Puis, récupérer le SkillPosition
                        var skillPosition = await _context.Set<SkillPosition>() // Remplacer par le nom réel de votre DbSet si nécessaire
                        ...
                        else
                        {
                            ...
                            try
                            {
                                // Utiliser un try-catch séparé pour la recherche du domaine
                                Console.WriteLine("Recherche d'un domaine de compétence valide...");

                                // Approche simple - utiliser le premier domaine disponible
                                var firstDomain = await _context.DomainSkill
                                    .FirstOrDefaultAsync(d => d.DomainSkillId > 0);

                                if (firstDomain != null)
                                {
                                    domainSkillId = firstDomain.DomainSkillId;
                                    Console.WriteLine($"Utilisation du domaine {domainSkillId}");
                                }
                                else
                                {
                                    Console.WriteLine("Aucun domaine de compétence trouvé!");
                                    continue;
                                }
                            }
                            catch (Exception ex)
