    ...
    public class EvaluationCompetenceService
    {
        ...
        public async Task<bool> CalculateAndSaveCompetenceResultsAsync(int evaluationId)
        {
            try
            {
                ...
                // 4. Récupérer les questions complètes pour accéder aux compétences associées
                var questions = await _context.evaluationQuestions
                    .Where(q => selectedQuestions.Select(sq => sq.QuestionId).Contains(q.questiondId))
                    .ToListAsync();

                // 5. Regrouper les réponses par compétence
                var competenceResults = new Dictionary<int, List<(int QuestionId, decimal Score)>>();

                foreach (var response in responses)
                {
                    var question = questions.FirstOrDefault(q => q.questiondId == response.QuestionId);
                    if (question != null && question.CompetenceLineId.HasValue)
                    {
                        int competenceId = question.CompetenceLineId.Value;
                ...
                // 6. Supprimer les anciennes entrées pour cette évaluation
                ...
                // 7. Calculer les moyennes et enregistrer les résultats par compétence
                ...
                var historyDtos = results.Select(r => {
                    ...
                    return new CompetenceResultHistoryDto
                    {
                        ...
                        EvaluationType = _context.evaluationTypes
                            .Where(et => et.Evaluation_type_id == evaluation.EvaluationTypeId)
