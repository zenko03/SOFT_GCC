       ...
       string weaknesses,
        ...
        public async Task<int> CreateEvaluationAsync(int userId, int employeeId, int evaluationTypeId, List<int> supervisorIds, DateTime startDate, DateTime endDate)
        {
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                ...
                {
                    User user = null;
                    // Vérifier si l'utilisateur existe seulement si un userId est fourni
                    if (userId > 0)
                    {
                    ...
                    // Vérifier si l'employé existe (toujours obligatoire)
                    ...
                    {
                        UserId = userId, // Peut être 0 si non fourni
                        EmployeeId = employeeId, // L'employé évalué (obligatoire)
                    ...
                    // Créer et envoyer les identifiants temporaires seulement si un employé est concerné
                    var tempAccount = await _temporaryAccountService.CreateTemporaryAccountAsync(userId, employeeId, newEvaluation.EvaluationId);

                    // Si un utilisateur est spécifié, envoyer un e-mail
