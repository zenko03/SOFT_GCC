    ...
    public class EvaluationCompetenceService
    {
        ...
        public async Task UpdateEmployeeSkillsAfterEvaluation(int evaluationId)
        {
            try
            {
                ...
                // 2. Obtenir les résultats d'évaluation par compétence avec SQL direct pour éviter les erreurs NULL
                ...
                try
                {
                    ...
                    {
                        command.CommandText = @"
                            SELECT ResultId, EvaluationId, EmployeeId, CompetenceLineId, Score, CreatedAt, State 
                            FROM Evaluation_Competence_Results 
                            WHERE EvaluationId = @evaluationId";
                            
                        var parameter = command.CreateParameter();
                        parameter.ParameterName = "@evaluationId";
                        parameter.Value = evaluationId;
                        ...
                        using (var reader = await command.ExecuteReaderAsync())
                        {
                            ...
                            {
                                var result = new EvaluationCompetenceResult
                                {
                                    ...
                                    // Ignorer CreatedAt et State car nous n'en avons pas besoin ici
                                };
                                results.Add(result);
                            }
                        }
                ...
                foreach (var result in results)
                {
                    try
                    {
                        ...
                        // 4.2 Vérifier si la compétence existe déjà pour cet employé (avec SQL direct)
                        int existingSkillId = 0;
                        ...
                        try
                        {
                            using (var command = _context.Database.GetDbConnection().CreateCommand())
                            {
                                ...
                                if (value != null && value != DBNull.Value)
                                {
                                    existingSkillId = Convert.ToInt32(value);
                                }
                            }
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine($"Erreur lors de la vérification de l'existence de la compétence: {ex.Message}");
                            continue;
                        }
