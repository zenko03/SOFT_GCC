    ...
    string? sortDirection = null)
      ...
      int? status = null  // Paramètre conservé pour compatibilité mais ignoré
        ...
        public async Task<EvaluationInterviews?> GetInterviewByParticipantIdAsync(int participantId)
        {
            ...
            Console.WriteLine($"Recherche d'entretien pour le participant ID: {participantId}");

            // Vérifier combien de participants avec cet ID existent
            var participantsCount = await _context.interviewParticipants
                .Where(p => p.UserId == participantId)
                .CountAsync();
            Console.WriteLine($"Nombre de participations trouvées: {participantsCount}");

            // Trouver d'abord les IDs de tous les entretiens associés à ce participant
            var interviewIds = await _context.interviewParticipants
                .Where(p => p.UserId == participantId)
                .Select(p => p.InterviewId)
                .ToListAsync();

            Console.WriteLine($"IDs d'entretiens trouvés: {string.Join(", ", interviewIds)}");

            // Si plusieurs entretiens trouvés, prendre le plus récent
            EvaluationInterviews? interview = null;

            if (interviewIds.Any())
            {
                interview = await _context.evaluationInterviews
                    .Where(i => interviewIds.Contains(i.InterviewId))
                    .OrderByDescending(i => i.InterviewDate) // Prendre le plus récent
                    .FirstOrDefaultAsync();
            }

            if (interview != null)
            {
                Console.WriteLine($"Entretien trouvé - ID: {interview.InterviewId}, EvaluationId: {interview.EvaluationId}, Date: {interview.InterviewDate}, Status: {interview.status}");

                // Vérifier si l'évaluation associée existe
                var evaluation = await _context.Evaluations.FindAsync(interview.EvaluationId);
                if (evaluation != null)
                {
                    Console.WriteLine($"Évaluation associée trouvée - ID: {evaluation.EvaluationId}, State: {evaluation.state}");
                }
                else
                {
                    Console.WriteLine($"ATTENTION: Aucune évaluation trouvée avec ID: {interview.EvaluationId}");
                }
            }
            else
            {
