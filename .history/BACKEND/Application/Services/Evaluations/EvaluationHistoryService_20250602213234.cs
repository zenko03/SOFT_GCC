...
namespace soft_carriere_competence.Application.Services.Evaluations
{
    ...
    string? employeeName = null)
        ...
        public async Task<KPIResult> GetKPIsAsync(DateTime? startDate, DateTime? endDate, string? departmentName)
        {
            try
            {
                Console.WriteLine($"Recherche de KPI pour département: {departmentName ?? "tous"}, startDate: {startDate}, endDate: {endDate}");
                ...
                if (startDate.HasValue)
                {
                    Console.WriteLine($"Filtrage par date de début: {startDate}");
                    query = query.Where(e => e.StartDate >= startDate.Value);
                }
            
                if (endDate.HasValue)
                {
                    Console.WriteLine($"Filtrage par date de fin: {endDate}");
                    query = query.Where(e => e.EndDate <= endDate.Value);
                }
                ...
                // Récupérer le nombre total d'employés (pourrait être adapté selon le cas d'usage)
                var totalEmployees = await _context.Employee.CountAsync();
                Console.WriteLine($"Nombre total d'employés: {totalEmployees}");
                ...
                if (!evaluations.Any())
                {
                    ...
                    // Valeurs par défaut pour éviter les zéros qui ne sont pas informatifs
                ...
                // Calcul des KPI avec des valeurs filtrées et validées
                var completedEvaluations = evaluations.Count(e => e.status == 20 || e.status == 30); // En cours ou terminées
                var approvedEvaluations = evaluations.Count(e => e.status == 30); // Approuvées/terminées
            
                // S'assurer que les scores sont disponibles
                ...
                Console.WriteLine($"Évaluations complétées: {completedEvaluations}, approuvées: {approvedEvaluations}");
                Console.WriteLine($"Scores valides: {validScores.Count}, moyenne: {overallAverage}");

                // Calculer les KPIs finaux avec validation 
                var participationRate = totalEmployees > 0 ? (completedEvaluations / (double)totalEmployees) * 100 : 0;
