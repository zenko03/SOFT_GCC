...
namespace soft_carriere_competence.Application.Services.Evaluations
{
    ...
    string? employeeName = null)
        ...
        public async Task<IEnumerable<object>> GetEmployeesAsync()
        {
            try
            {
                // Utiliser le DbSet Employee qui est défini dans ApplicationDbContext
                // et mapper vers un objet anonyme avec les propriétés nécessaires pour le frontend
                return await _context.Employee
                    .Select(e => new
                    {
                        ...
                        // Pas de propriété Position dans le modèle Employee
                    })
                    .ToListAsync();
            }
            catch (Exception ex)
