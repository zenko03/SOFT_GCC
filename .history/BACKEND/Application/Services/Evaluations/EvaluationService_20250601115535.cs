    ...
    public class EvaluationService
    {
        ...
        public async Task<List<TrainingSuggestionResultDto>> GetTrainingSuggestionsByQuestionsAsync(Dictionary<int, int> ratings)
        {
            try
            {
                ...
                // 1. Récupérer toutes les suggestions de formation avec les questions associées
                var suggestionQuery = from suggestion in _context.TrainingSuggestions
                                     join question in _context.evaluationQuestions
                                         on suggestion.questionId equals question.questionId
                                     where ratings.ContainsKey(suggestion.questionId) && 
                                           ratings[suggestion.questionId] < suggestion.scoreThreshold
                                     select new TrainingSuggestionResultDto
                                     {
                ...
                var result = await suggestionQuery.ToListAsync();

                if (result.Count == 0)
                {
                    Console.WriteLine("Aucune suggestion de formation trouvée");
                }
                else
                {
                    Console.WriteLine($"{result.Count} suggestions de formation trouvées");
                }
