        ...
        public async Task UpdateEmployeeSkillsAfterEvaluation(int evaluationId)
        {
            try
            {
                ...
                foreach (var result in competenceResults)
                {
                    ...
                    // Récupérer le Domain Skill à partir de la base
                    // Note: Nous devons trouver la compétence puis chercher le domaine associé
                    ...
                    // Nous devons maintenant trouver le domaine de cette compétence
                    // Puisque Skill n'a pas de colonne DomainSkillId, nous devons chercher 
                    // dans les EmployeeSkill existants ou choisir un domaine par défaut
                    ...
                    {
                        // Recherche intelligente du DomainSkillId le plus approprié
                        ...
                        {
                            // Utiliser le domaine le plus courant pour cette compétence
                        ...
                        {
                            // 2. Recherche par correspondance de nom entre Skill et DomainSkill
                            var skillName = await _context.Skill
                                .Where(s => s.SkillId == skillId)
                                .Select(s => s.Name)
                                .FirstOrDefaultAsync();
                                
                            // Rechercher un domaine dont le nom est contenu dans le nom de la compétence 
                            // ou vice versa (correspondance partielle)
                            var matchingDomain = await _context.DomainSkill
                                .Where(d => (skillName != null && d.Name != null && skillName.Contains(d.Name)) 
                                        || (d.Name != null && skillName != null && d.Name.Contains(skillName)))
                                .FirstOrDefaultAsync();
                                        
                            if (matchingDomain != null)
                            {
                                domainSkillId = matchingDomain.DomainSkillId;
                                Console.WriteLine($"Utilisation du domaine ({domainSkillId}) par correspondance de nom avec la compétence {skillId} ({skillName})");
                            }
                            else
                            {
                                // 3. Si aucune correspondance n'est trouvée, utiliser le premier domaine
                                var firstDomain = await _context.DomainSkill.FirstOrDefaultAsync();
                                if (firstDomain != null)
                                {
                                ...
                                else
                                {
                                    ...
                                    Console.WriteLine("Aucun domaine trouvé dans la base de données.");
                                    continue;
                                }
                            }
                        }
