...
namespace soft_carriere_competence.Application.Services.Evaluations
{
    public class UserService : IUserService
    {
        ...
        public async Task<string> LoginAsync(LoginDto dto)
        {
            ...
            // L'identifier peut être soit un nom d'utilisateur soit un email
            if (dto.Identifier.Contains("@"))
            {
                // Si l'identifiant contient @, on le considère comme un email
                user = await _userRepository.GetFirstOrDefaultAsync(u => u.Email == dto.Identifier);
            }
            else
            {
                // Sinon, on le considère comme un nom d'utilisateur
                user = await _userRepository.GetFirstOrDefaultAsync(u => u.Username == dto.Identifier);
            }
