    ...
    string? employeeName = null)
        ...
        public async Task<KPIResult> GetKPIsAsync(DateTime? startDate, DateTime? endDate, string? departmentName)
        {
            try
            {
                ...
                // MODIFICATION: Compter les employés uniques qui ont participé aux évaluations
                var uniqueParticipatingEmployees = evaluations
                    .Where(e => e.status != 40) // Exclure les évaluations annulées
                ...
                if (!string.IsNullOrEmpty(departmentName))
                {
                    ...
                    {
                        totalEligibleEmployeesQuery = totalEligibleEmployeesQuery.Where(e => e.DepartmentId == departmentId.Value);
                    }
                }
