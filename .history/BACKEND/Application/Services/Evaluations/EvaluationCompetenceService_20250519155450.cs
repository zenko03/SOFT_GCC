        ...
        public async Task UpdateEmployeeSkillsAfterEvaluation(int evaluationId)
        {
            try
            {
                ...
                foreach (var result in competenceResults)
                {
                    ...
                    {
                        // Trouver un domaine approprié
                        // Option 1: Chercher dans d'autres enregistrements EmployeeSkill avec le même SkillId
                        var domainSkillId = await _context.EmployeeSkill
                            .Where(es => es.SkillId == skillId)
                            .Select(es => es.DomainSkillId)
                            .FirstOrDefaultAsync();

                        // Option 2: Si aucun domaine n'est trouvé, prendre le premier domaine disponible
                        if (domainSkillId == 0)
                        {
