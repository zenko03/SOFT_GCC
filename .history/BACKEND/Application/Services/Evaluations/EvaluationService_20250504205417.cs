...
using soft_carriere_competence.Application.Services.EvaluationCompetenceService;
    ...
    public class EvaluationService
    {
            ...
            List<MultiCriteriaRatingDto> detailedRatings = null)
        {
            ...
            if (detailedRatings != null && detailedRatings.Count > 0)
            {
                ...
                {
                    if (detailedRating == null || !detailedRating.QuestionId.HasValue) continue;

                    int questionId = detailedRating.QuestionId.Value;
            ...
            try
            {
                var competenceService = new EvaluationCompetenceService(_context);
                await competenceService.CalculateAndSaveCompetenceResultsAsync(evaluationId);
            }
            catch (Exception ex)
            {
