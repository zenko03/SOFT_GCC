                ...
                foreach (var result in competenceResults)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            var domainSkills = await _context.DomainSkill.ToListAsync();
                            var skillName = skill.Name?.ToLower() ?? "";
                            
                            // Recherche par correspondance de nom
                            var matchingDomain = domainSkills
                                .FirstOrDefault(ds => skillName.Contains(ds.DomainSkillName?.ToLower() ?? "") || 
                                                     (ds.DomainSkillName?.ToLower() ?? "").Contains(skillName));
