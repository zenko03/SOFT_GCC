    ...
    string? sortDirection = null)
        ...
        public async Task<bool> StartInterviewAsync(int interviewId)
        {
            ...
            try {
                ...
                Console.WriteLine($"Statut actuel de l'interview: {interview.status}");

                // Mettre à jour le statut
                interview.status = InterviewStatus.InProgress;
                Console.WriteLine($"Nouveau statut de l'interview défini à: {interview.status}");

                // Suivre les modifications en base
                _context.evaluationInterviews.Update(interview);
                Console.WriteLine("Interview marqué pour mise à jour");
                
                // Sauvegarder les changements
                int rowsAffected = await _context.SaveChangesAsync();
                Console.WriteLine($"SaveChangesAsync terminé, {rowsAffected} lignes affectées");
  ...
  )
        {
            ...
            if (interview.managerApproval == true && interview.directorApproval == true)
            {
                ...
                try {
                    ...
                    if (interview.EvaluationId.HasValue && interview.EvaluationId.Value > 0)
                    {
                        // Mettre à jour l'état de l'évaluation à 30 (archivé)
                        var evaluation = await _context.Evaluations.FindAsync(interview.EvaluationId.Value);
                        if (evaluation != null)
                        {
