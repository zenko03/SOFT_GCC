    ...
    string? sortDirection = null)
        ...
        public async Task<(bool Success, string Message, int? InterviewId)> ScheduleInterviewAsync(int evaluationId, DateTime scheduledDate, List<int> participantIds, int? employeeId = null)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Entretien créé avec ID: {interview.InterviewId}");
                ...
                Console.WriteLine("Planification d'entretien réussie");
      ...
      int? status = null  // Paramètre conservé pour compatibilité mais ignoré
        ...
        public async Task<EvaluationInterviews?> GetInterviewByParticipantIdAsync(int participantId)
        {
            try
            {
                ...
                // Rechercher d'abord les interviews où l'employé est participant
                var interviewsByEmployee = await _context.interviewParticipants
                    .Where(p => p.EmployeeId == participantId)
                    .Select(p => p.InterviewId)
                    .ToListAsync();

                Console.WriteLine($"Entretiens trouvés via EmployeeId: {string.Join(", ", interviewsByEmployee)}");

                // Puis rechercher les interviews où l'utilisateur est participant
                var interviewsByUser = await _context.interviewParticipants
                    .Where(p => p.UserId == participantId)
                    .Select(p => p.InterviewId)
                    .ToListAsync();

                Console.WriteLine($"Entretiens trouvés via UserId: {string.Join(", ", interviewsByUser)}");

                // Combiner les deux listes
                var allInterviewIds = interviewsByEmployee.Concat(interviewsByUser).Distinct().ToList();

                if (!allInterviewIds.Any())
                {
                ...
                return await _context.evaluationInterviews
                    .Where(i => allInterviewIds.Contains(i.InterviewId))
