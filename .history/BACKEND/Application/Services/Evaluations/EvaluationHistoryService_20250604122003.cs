    ...
    string? employeeName = null)
        ...
        public async Task<KPIResult> GetKPIsAsync(DateTime? startDate, DateTime? endDate, string? departmentName)
        {
            try
            {
                ...
                // MODIFICATION: Récupérer les évaluations complètes à partir de la base de données
                // Ajouter AsNoTracking() pour améliorer les performances et éviter les problèmes de suivi d'entités
                var completeEvaluations = await _context.Evaluations
                    .AsNoTracking()
                    .Where(e => filteredEvaluationIds.Contains(e.EvaluationId))
                    .Where(e => e.EmployeeId != 0) // Filtrer pour exclure les enregistrements avec EmployeeId = 0
                    .ToListAsync();
