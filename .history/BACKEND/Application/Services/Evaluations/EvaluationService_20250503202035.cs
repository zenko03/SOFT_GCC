    ...
    public class EvaluationService
    {
            ...
            List<MultiCriteriaRatingDto> detailedRatings = null)
        {
            ...
            foreach (var rating in ratings)
            {
                ...
                if (!questionExists) throw new Exception($"Question with ID {questionId} not found.");

                // Chercher s'il y a une notation détaillée pour cette question
                var detailedRating = detailedRatings?.FirstOrDefault(r => r.QuestionId == questionId);
                bool hasDetailedCriteria = detailedRating != null;
                
                // Valeur à stocker en base (simple note ou JSON)
                string responseValue;
                string responseType;
                
                if (hasDetailedCriteria)
                {
                    // S'assurer que la note globale est à jour
                    detailedRating.OverallRating = detailedRating.CalculateOverallRating();
                    
                    // Convertir en JSON pour stockage
                    responseValue = detailedRating.ToJson();
                    responseType = "MULTI_CRITERIA";
                    
                    // Synchroniser avec la note simple pour compatibilité
                    if (detailedRating.OverallRating != ratingValue)
                    {
                        // Log la différence pour débogage
                        Console.WriteLine($"Note incohérente pour Q{questionId}: Simple={ratingValue}, Détaillée={detailedRating.OverallRating}");
                    }
                }
                else
                {
                ...
                if (existingResponsesDict.TryGetValue(questionId, out var existingResponse))
                {
                    ...
                    existingResponse.EndTime = DateTime.UtcNow;
                    existingResponse.State = 10; // TERMINEE
