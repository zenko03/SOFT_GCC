    ...
    public class EvaluationCompetenceService
    {
        ...
        public async Task<bool> CalculateAndSaveCompetenceResultsAsync(int evaluationId)
        {
            try
            {
                ...
                // 2. Récupérer toutes les questions sélectionnées pour cette évaluation
                var selectedQuestions = await _context.evaluationSelectedQuestions
                    .Where(esq => esq.EvaluationId == evaluationId)
                    .ToListAsync();

                Console.WriteLine($"Nombre de questions sélectionnées: {selectedQuestions.Count}");
                ...
                foreach (var response in responses)
                {
                    ...
                    // CORRECTION ICI: Utiliser directement les questions sélectionnées pour obtenir la compétence
                    ...
                    if (selectedQuestion != null)
                    {
                        ...
                        {
                            if (decimal.TryParse(response.ResponseValue, out var parsedScore))
                            {
                                score = parsedScore;
                            }
                        }
                ...
                foreach (var competenceEntry in competenceResults)
                {
                    ...
                    Console.WriteLine($"Calcul pour la compétence ID: {competenceId}");
