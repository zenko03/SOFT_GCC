    ...
    string? sortDirection = null)
      ...
      int? status = null
  )
        {
            ...
            // Si un statut est fourni explicitement, l'utiliser
            if (status.HasValue && Enum.IsDefined(typeof(InterviewStatus), status.Value))
            {
                interview.status = (InterviewStatus)status.Value;
                Console.WriteLine($"Mise à jour explicite du statut: {interview.status}");
            }
            // Sinon, mise à jour du statut en fonction des validations
