...
namespace soft_carriere_competence.Application.Services.Evaluations
{
    public class EvaluationService
    {
        ...
        {
            return await _context.evaluationQuestions.Include(p => p.position).Include(et => et.EvaluationType).ToListAsync();
        }
       ...
       string generalEvaluation)
            ...
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    foreach (var employeeQuestion in dto.EmployeeQuestions)
                    {
                        ...
                        foreach (var questionId in employeeQuestion.SelectedQuestionIds)
                        {
                            var selectedQuestion = new EvaluationSelectedQuestions
                            {
                                EvaluationId = evaluationId,
                                QuestionId = questionId,
                                CompetenceLineId = await GetCompetenceLineIdForQuestion(questionId)
                            };
        ...
        private async Task<int> GetCompetenceLineIdForQuestion(int questionId)
        {
            ...
            return question.CompetenceLineId;
        }
    }
