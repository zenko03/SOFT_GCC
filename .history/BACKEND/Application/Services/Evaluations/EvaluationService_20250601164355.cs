    ...
    public class EvaluationService
    {
        ...
        public async Task<List<TrainingSuggestionResultDto>> GetTrainingSuggestionsByQuestionsAsync(Dictionary<int, int> ratings)
        {
            try
            {
                ...
                // 1. Vérifier d'abord quelles questions ont des suggestions dans la base de données
                var questionsWithSuggestions = await _context.TrainingSuggestions
                    .Select(ts => ts.questionId)
                    .Distinct()
                    .ToListAsync();
                
                Console.WriteLine($"Nombre de questions avec suggestions: {questionsWithSuggestions.Count}");
                
                // 2. Vérifier quelles questions évaluées ont des suggestions
                var matchingQuestions = ratings.Keys
                    .Where(qId => questionsWithSuggestions.Contains(qId))
                    .ToList();
                
                Console.WriteLine($"Nombre de questions évaluées avec suggestions: {matchingQuestions.Count}");
                
                if (!matchingQuestions.Any())
                {
                    Console.WriteLine("Aucune correspondance entre les questions évaluées et les suggestions disponibles");
                    return new List<TrainingSuggestionResultDto>();
                }

                // 3. Récupérer les suggestions qui correspondent aux questions notées dont la note est inférieure au seuil
                var suggestions = await _context.TrainingSuggestions
                    .Where(ts => ratings.ContainsKey(ts.questionId) &&
                                 ratings[ts.questionId] <= ts.scoreThreshold)
                    .ToListAsync();
                
                Console.WriteLine($"Nombre de suggestions trouvées avec le critère de seuil: {suggestions.Count}");
                
                if (!suggestions.Any())
                {
                    // Vérifier les seuils pour comprendre pourquoi aucune suggestion n'est trouvée
                    var thresholds = await _context.TrainingSuggestions
                        .Where(ts => matchingQuestions.Contains(ts.questionId))
                        .Select(ts => new { QuestionId = ts.questionId, Threshold = ts.scoreThreshold })
                        .ToListAsync();
                        
                    Console.WriteLine("Seuils configurés pour les questions évaluées:");
                    foreach (var threshold in thresholds)
                    {
                        int rating = ratings.ContainsKey(threshold.QuestionId) ? ratings[threshold.QuestionId] : -1;
                        Console.WriteLine($"Question {threshold.QuestionId}: Seuil {threshold.Threshold}, Note actuelle: {rating}");
                    }
                }

                // 4. Joindre avec les questions pour récupérer le texte des questions
                var questionsMap = await _context.evaluationQuestions
                    .Where(q => suggestions.Select(s => s.questionId).Contains(q.questionId))
                    .ToDictionaryAsync(q => q.questionId, q => q.question);

                // 5. Mapper les suggestions en DTOs
                var result = suggestions.Select(s => new TrainingSuggestionResultDto
                {
                    Question = questionsMap.ContainsKey(s.questionId) ? questionsMap[s.questionId] : $"Question {s.questionId}",
                    Training = s.Training,
                    Details = s.Details
                }).ToList();
