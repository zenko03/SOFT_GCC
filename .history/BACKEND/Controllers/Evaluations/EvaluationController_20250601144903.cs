...
namespace soft_carriere_competence.Controllers.Evaluations
{
	...
	public class EvaluationController : ControllerBase
	{
		...
		[HttpGet("fix-training-suggestions")]
		public async Task<IActionResult> FixTrainingSuggestions()
		{
			try
			{
				var response = new
				{
					diagnostiqueAvant = await DiagnostiquerTrainingSuggestions(),
					corrections = await CorrigerTrainingSuggestions(),
					diagnostiqueApres = await DiagnostiquerTrainingSuggestions()
				};
		...
		private async Task<object> DiagnostiquerTrainingSuggestions()
		{
			// Récupérer toutes les suggestions
			var allSuggestions = await _context.TrainingSuggestions.ToListAsync();
			
			// Vérifier les entrées où scoreThreshold n'est pas défini correctement
			var problematicSuggestions = allSuggestions
				.Where(s => s.scoreThreshold <= 0)
				.ToList();
				
			// Vérifier la structure de données
			var premiereSuggestion = allSuggestions.FirstOrDefault();
			
			return new
			{
				totalSuggestions = allSuggestions.Count,
				problematicSuggestionsCount = problematicSuggestions.Count,
				problematicSuggestions = problematicSuggestions.Select(s => new { 
					id = s.TrainingSuggestionId,
					questionId = s.questionId,
					scoreThreshold = s.scoreThreshold,
					training = s.Training
				}).Take(5).ToList(),
				firstSuggestion = premiereSuggestion != null ? new {
					id = premiereSuggestion.TrainingSuggestionId,
					questionId = premiereSuggestion.questionId,
					scoreThreshold = premiereSuggestion.scoreThreshold,
					training = premiereSuggestion.Training,
					competenceLineId = premiereSuggestion.CompetenceLineId
				} : null
			};
		}
		
		private async Task<object> CorrigerTrainingSuggestions()
		{
			// Récupérer toutes les suggestions
			var allSuggestions = await _context.TrainingSuggestions.ToListAsync();
			int corrected = 0;
			
			// S'assurer que scoreThreshold est défini à 3 pour toutes les suggestions
			foreach (var suggestion in allSuggestions)
			{
				if (suggestion.scoreThreshold <= 0 || suggestion.scoreThreshold > 5)
				{
					suggestion.scoreThreshold = 3;
					corrected++;
				}
				
				// S'assurer que state est actif (1)
		...
		[HttpGet("verify-training-suggestions")]
		public async Task<IActionResult> VerifyTrainingSuggestions()
		{
			try
			{
				// 1. Vérifier si on peut récupérer toutes les suggestions
				var allSuggestions = await _context.TrainingSuggestions.ToListAsync();
				
				// 2. Vérifier le mapping entre entité et table
				var tableInfo = await _context.Database.ExecuteSqlRawAsync(
					"SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Training_suggestions'");
				
				// 3. Exécuter une requête brute SQL pour vérifier les données avec SQL direct
				var rawSuggestions = await _context.Database.ExecuteSqlRawAsync(
					"SELECT * FROM Training_suggestions WHERE scoreThreshold < 5");
				
				var diagnosticResults = new 
				{
					entityMappingExample = allSuggestions.FirstOrDefault() != null ? new
					{
						TrainingSuggestionId = allSuggestions.First().TrainingSuggestionId,
						evaluationTypeId = allSuggestions.First().evaluationTypeId,
						questionId = allSuggestions.First().questionId,
						CompetenceLineId = allSuggestions.First().CompetenceLineId,
						training = allSuggestions.First().Training,
						details = allSuggestions.First().Details,
						state = allSuggestions.First().state,
						scoreThreshold = allSuggestions.First().scoreThreshold
					} : null,
					totalCount = allSuggestions.Count,
					lowScoreThresholdCount = allSuggestions.Count(s => s.scoreThreshold <= 0),
					highScoreThresholdCount = allSuggestions.Count(s => s.scoreThreshold > 5),
					scoreSummary = allSuggestions.GroupBy(s => s.scoreThreshold)
						.Select(g => new { threshold = g.Key, count = g.Count() })
						.OrderBy(g => g.threshold)
						.ToList(),
					rawTableResult = rawSuggestions
				};
		...
		[HttpGet("{evaluationId}/comments")]
		public async Task<IActionResult> GetEvaluationComments(int evaluationId)
		{
			try
			{
				...
				// Créer un dictionnaire questionId -> commentaire
				...
				foreach (var response in responses)
				{
					try
					{
						// Tenter de désérialiser en tant que MultiCriteriaRatingDto
						// si la réponse est au format JSON
						if (response.ResponseType == "MULTI_CRITERIA")
						{
							var dto = System.Text.Json.JsonSerializer.Deserialize<MultiCriteriaRatingDto>(response.ResponseValue);
							if (dto != null && !string.IsNullOrEmpty(dto.Comment))
							{
								comments[response.QuestionId] = dto.Comment;
							}
						}
					}
