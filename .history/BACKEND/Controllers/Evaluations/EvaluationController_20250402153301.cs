	...
	public class EvaluationController : ControllerBase
	{
		...
		public async Task<IActionResult> GetEvaluationDetails(int id)
		{
			try
			{
				...
				Console.WriteLine($"UserId: {user.Id}, PostId: {user.PositionId}, EvaluationTypeId: {evaluation.EvaluationTypeId}");
				...
				var questions = await _context.evaluationQuestions
					.Where(q => q.evaluationTypeId == evaluation.EvaluationTypeId &&
						   q.positionId == user.PositionId) // Assurez-vous que c'est le bon nom de propriété
					.Select(q => new
					{
		...
		[HttpGet("evaluation/{evaluationId}/options")]
		...
		[HttpPost("evaluation/{evaluationId}/save-progress")]
		...
		[HttpGet("evaluation/{evaluationId}/time-remaining")]
		...
		[HttpPost("evaluation/{evaluationId}/submit")]
		public async Task<IActionResult> SubmitEvaluation(int evaluationId, [FromBody] EvaluationSubmissionDto submission)
		{
			try
			{
				...
				{
					response.EvaluationId = evaluationId;
					await _responseService.SaveResponseAsync(response);
				}

				// Mettre à jour l'évaluation avec le score global et les commentaires
				...
				evaluation.OverallScore = submission.AverageScore;
				evaluation.Comments = submission.OverallFeedback;
				evaluation.state = 15; // État "Soumis"
