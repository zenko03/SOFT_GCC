...
namespace soft_carriere_competence.Controllers.Evaluations
{
	...
	public class EvaluationController : ControllerBase
	{
		...
		public async Task<IActionResult> GetSelectedQuestionsAndResponses(int evaluationId)
		{
			...
			{
				var selectedQuestions = await _context.evaluationSelectedQuestions
					.Include(sq => sq.evaluationQuestion)
					.Include(sq => sq.competenceLine)
					.Where(sq => sq.EvaluationId == evaluationId)
					.Select(sq => new
					{
						QuestionId = sq.QuestionId,
						Question = sq.evaluationQuestion.question,
						CompetenceLineId = sq.CompetenceLineId,
						CompetenceName = sq.competenceLine.CompetenceName,
						Response = _context.evaluationResponses
							.FirstOrDefault(r => r.EvaluationId == evaluationId && r.QuestionId == sq.QuestionId)
					})
					.ToListAsync();
