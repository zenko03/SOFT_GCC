				...
				var selectedQuestions = await _context.evaluationSelectedQuestions
					...
					.Select(sq => new
					{
						...
						response = _context.evaluationResponses
							.Where(r => r.EvaluationId == evaluationId && r.QuestionId == sq.SelectedQuestion.questiondId)
							.Select(r => new
							{
							...
							.FirstOrDefault(),
						correctAnswer = _context.evaluationQuestionOptions
							.Where(o => o.QuestionId == sq.SelectedQuestion.questiondId && o.IsCorrect)
