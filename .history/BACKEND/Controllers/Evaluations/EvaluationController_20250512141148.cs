...
namespace soft_carriere_competence.Controllers.Evaluations
{
	...
	public class EvaluationController : ControllerBase
	{
		...
		public async Task<IActionResult> GetEvaluationTemplates()
		{
			try
			{
				var evaluationTypes = await _context.Evaluation_type
					.Where(et => et.state == 1)
					.Select(et => new
					{
						id = et.Evaluation_type_id,
						title = et.designation,
						description = et.designation, // Vous pouvez ajouter une description plus détaillée si nécessaire
						questionCount = _context.Evaluation_questions
							.Count(q => q.evaluationTypeId == et.Evaluation_type_id && q.state == 1)
					})
					.ToListAsync();
		...
		public async Task<IActionResult> GetQuestionsByEvaluationType(int evaluationTypeId)
		{
			try
			{
				var questions = await _context.Evaluation_questions
					...
					.Select(q => new
					{
						...
						responseType = _context.ResponseTypes
							...
							.Select(rt => rt.TypeName)
							.FirstOrDefault() ?? "TEXT",
						maxTimeInMinutes = q.MaxTimeInMinutes ?? 15 // Valeur par défaut de 15 minutes
					})
					.ToListAsync();
		...
		public async Task<IActionResult> UpdateQuestionsTime([FromBody] List<QuestionTimeUpdateDto> questions)
		{
			try
			{
				...
				{
					var question = await _context.Evaluation_questions
						.FirstOrDefaultAsync(q => q.Question_id == questionUpdate.QuestionId);

					if (question != null)
					{
						question.MaxTimeInMinutes = questionUpdate.MaxTimeInMinutes;
					}
				}
