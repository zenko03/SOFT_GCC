		...
		public async Task<IActionResult> SaveEvaluationResults([FromBody] EvaluationResultsDto dto)
		{
			...
			if (dto.HasDetailedRatings())
			{
				...
				foreach (var detailedRating in dto.DetailedRatings)
				{
					...
					Console.WriteLine($"    Pertinence: {detailedRating.Relevance ?? 0}/5");
					Console.WriteLine($"    Technique: {detailedRating.Technical ?? 0}/5");
					Console.WriteLine($"    Clarté: {detailedRating.Clarity ?? 0}/5");
