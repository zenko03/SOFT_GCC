		...
		public async Task<IActionResult> GetSelectedQuestionsAndResponses(int evaluationId)
		{
			try
			{
				var selectedQuestions = await _context.evaluationSelectedQuestions
					...
					.Join(_context.evaluationQuestions,
						...
						(esq, eq) => new
						{
							...
							// Récupérer la configuration du temps sans valeur par défaut
				...
				foreach (var selectedQuestion in selectedQuestions)
				{
					...
					// Utiliser la valeur par défaut de 15 minutes si aucune configuration n'est trouvée
					int maxTime = selectedQuestion.MaxTimeInMinutes > 0 ? selectedQuestion.MaxTimeInMinutes : 15;
