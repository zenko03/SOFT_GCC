...
namespace soft_carriere_competence.Controllers.Evaluations
{
	...
	public class EvaluationController : ControllerBase
	{
		...
		[HttpPost("questions")]
		public async Task<IActionResult> CreateEvaluationQuestion([FromBody] EvaluationQuestion questionData)
		{
			...
			try
			{
				...
				var evaluationType = await _context.EvaluationTypes.FindAsync(questionData.evaluationTypeId);
				if (evaluationType == null)
				{
					return BadRequest(new { error = "Le type d'évaluation spécifié n'existe pas." });
				}

				var position = await _context.positions.FindAsync(questionData.positionId);
				if (position == null)
				{
					return BadRequest(new { error = "La position spécifiée n'existe pas." });
				}

				var responseType = await _context.ResponseTypes.FindAsync(questionData.ResponseTypeId);
				if (responseType == null)
				{
					return BadRequest(new { error = "Le type de réponse spécifié n'existe pas." });
				}

				// Vérifier si la ligne de compétence existe (si elle est spécifiée)
				if (questionData.CompetenceLineId.HasValue)
				{
					var competenceLine = await _context.competenceLines.FindAsync(questionData.CompetenceLineId.Value);
					if (competenceLine == null)
					{
						return BadRequest(new { error = "La ligne de compétence spécifiée n'existe pas." });
					}
				}

				// Créer une nouvelle question avec les propriétés simples
				var newQuestion = new EvaluationQuestion
				{
					question = questionData.question,
					evaluationTypeId = questionData.evaluationTypeId,
					positionId = questionData.positionId,
					CompetenceLineId = questionData.CompetenceLineId,
					ResponseTypeId = questionData.ResponseTypeId,
					state = questionData.state,
					// Assigner explicitement les entités de navigation
					EvaluationType = evaluationType,
					Position = position,
					ResponseType = responseType
				};

				// Si CompetenceLineId a une valeur, charger l'entité CompetenceLine
				if (questionData.CompetenceLineId.HasValue)
				{
					newQuestion.CompetenceLine = await _context.competenceLines.FindAsync(questionData.CompetenceLineId.Value);
				}
				...
				return CreatedAtAction(nameof(GetEvaluationQuestionById), new { id = newQuestion.questionId }, newQuestion);
			}
			catch (Exception ex)
			{
		...
		public async Task<IActionResult> UpdateEvaluationQuestion(int id, [FromBody] EvaluationQuestion questionUpdate)
		{
			if (questionUpdate == null || questionUpdate.questionId != id)
			{
			...
			try
			{
				// Récupérer la question existante avec toutes ses relations
				...
				// Mettre à jour seulement les propriétés simples, et conserver les entités de navigation
				existingQuestion.question = questionUpdate.question;
				existingQuestion.evaluationTypeId = questionUpdate.evaluationTypeId;
				existingQuestion.positionId = questionUpdate.positionId;
				existingQuestion.CompetenceLineId = questionUpdate.CompetenceLineId;
				existingQuestion.ResponseTypeId = questionUpdate.ResponseTypeId;
				existingQuestion.state = questionUpdate.state;
