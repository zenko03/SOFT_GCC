	...
	public class EvaluationController : ControllerBase
	{
		...
		public async Task<IActionResult> GetEvaluationDetails(int id)
		{
			try
			{
				...
				// Récupérer les questions sélectionnées via l'endpoint dédié
				var selectedQuestions = await _context.evaluationSelectedQuestions
					...
					.Join(_context.evaluationQuestions,
						sq => sq.QuestionId,
						q => q.questiondId,
						(sq, q) => new { SelectedQuestion = sq, Question = q })
					.Join(_context.competenceLines,
						sq => sq.SelectedQuestion.CompetenceLineId,
						cl => cl.CompetenceLineId,
						(sq, cl) => new
						{
							QuestionId = sq.SelectedQuestion.QuestionId,
							Question = sq.Question.question,
							CompetenceLineId = sq.SelectedQuestion.CompetenceLineId,
							CompetenceName = cl.CompetenceName
						})
					.ToListAsync();
				...
				var result = new
				{
					...
					employeeName = $"{evaluation.User.first_name} {evaluation.User.last_name}",
					position = evaluation.User.Position?.title ?? "Non défini",
					department = evaluation.User.Department?.Department_name ?? "Non défini",
		...
		public async Task<IActionResult> GetSelectedQuestionsAndResponses(int evaluationId)
		{
					...
					.Join(_context.competenceLines,
						...
						(sq, cl) => new
						{
							...
							Response = _context.evaluationResponses
								.FirstOrDefault(r => r.EvaluationId == evaluationId && r.QuestionId == sq.SelectedQuestion.QuestionId)
						})
					.ToListAsync();

				if (!selectedQuestions.Any())
				{
					return NotFound("Aucune question sélectionnée trouvée pour cette évaluation.");
				}
			...
			{
				return StatusCode(500, $"Erreur lors de la récupération des questions sélectionnées : {ex.Message}");
			}
		}
	}
}
