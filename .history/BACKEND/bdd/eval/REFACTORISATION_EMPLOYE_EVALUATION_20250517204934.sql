...
GROUP BY 
    ...
    -- Supprimer la vue VEmployeeDetails existante
DROP VIEW IF EXISTS VEmployeeDetails;

-- Recréer la vue VEmployeeDetails adaptée
...
SELECT 
    ...
    ISNULL(ep.Position_name, 'Non défini') AS Position,
    ISNULL(ep.Position_id, 0) AS PositionId,
    -- Note: Employees n'a pas directement de rôle, utiliser un champ approprié ou NULL
    NULL AS Role,
    d.Department_name AS Department,
    ev.Evaluations_id AS EvaluationId,
