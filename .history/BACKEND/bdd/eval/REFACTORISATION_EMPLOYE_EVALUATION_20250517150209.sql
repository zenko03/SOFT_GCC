...
-- Modifier la table Evaluations pour ajouter employee_id
ALTER TABLE Evaluations 
...
-- Modifier la table InterviewParticipants pour ajouter Employee_id
ALTER TABLE InterviewParticipants 
ADD Employee_id INT NULL;
...
-- Modifier la table Evaluation_progress pour ajouter employee_id
ALTER TABLE Evaluation_progress 
ADD employee_id INT NULL;
...
-- Modifier la table TemporaryAccounts pour ajouter Employee_id
ALTER TABLE TemporaryAccounts 
ADD Employee_id INT NULL;
...
SELECT 
    ...
    ISNULL(cp.Position_id, 0) AS positionId,
    e.FirstName AS FirstName,
    e.Name AS LastName,
    ISNULL(p.position_name, 'Non défini') AS Position,
...
-- Joindre la dernière affectation active (career_plan) pour obtenir le poste actuel
LEFT JOIN (
    SELECT cp1.*
    FROM career_plan cp1
    INNER JOIN (
        SELECT Employee_id, MAX(Career_plan_id) AS max_career_id
        FROM career_plan 
        WHERE State = 1 -- Uniquement les affectations actives
        GROUP BY Employee_id
    ) cp2 ON cp1.Career_plan_id = cp2.max_career_id
) cp ON e.Employee_id = cp.Employee_id
...
SELECT 
    ...
    ta.ExpirationDate,
    ev.Evaluations_id,
    ev.startDate AS EvaluationStart,
    ev.endDate AS EvaluationEnd
...
SELECT 
    ...
    (SELECT STRING_AGG(participants.full_name, ', ')
     FROM (
         ...
         JOIN Employee e_part ON ip.Employee_id = e_part.Employee_id
         WHERE ip.InterviewId = ei.InterviewId
     ) AS participants
    ) AS ParticipantNames,
    (SELECT STRING_AGG(CAST(participant_ids.Employee_id AS NVARCHAR), ', ')
     FROM (
         SELECT DISTINCT ip.ParticipantId, e_part.Employee_id
         FROM InterviewParticipants ip
         JOIN Employee e_part ON ip.Employee_id = e_part.Employee_id
         WHERE ip.InterviewId = ei.InterviewId
     ) AS participant_ids
...
WHERE ev.state = 20
GROUP BY 
    ev.Evaluations_id, ev.employee_id, e.FirstName, e.Name, et.designation, 
    ev.startDate, ev.endDate, ev.overallScore, ev.comments, 
...
BEGIN
    ...
    PRINT 'La colonne Employee_id a été ajoutée à la table Users. Vous devrez établir manuellement la correspondance.';
END
