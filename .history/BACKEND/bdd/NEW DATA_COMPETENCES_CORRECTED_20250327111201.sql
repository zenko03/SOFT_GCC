...
-- 5. Création du script pour insérer les suggestions de formation
-- Mais à exécuter après avoir inséré les questions!
/*
Pour créer les suggestions de formation, nous devons d'abord connaître les IDs des questions qui ont été créées.
Exécutez la première partie du script (1-4), puis utilisez la requête suivante pour obtenir les IDs des questions :

SELECT Question_id, question, CompetenceLineId FROM Evaluation_questions;

Ensuite, utilisez ces IDs pour insérer les suggestions de formation avec le script suivant :
...
VALUES 
    -- Remplacez X par l'ID de la question correspondante :
    (1, X, 1, 1, 'Java 17 et nouveautés', 'Formation approfondie sur les dernières fonctionnalités de Java', 3, 1),
    -- et ainsi de suite...
*/

-- Exemple de comment le script final devrait être (mais avec les vrais IDs) :
/*
INSERT INTO Training_suggestions (evaluationTypeId, questionId, CompetenceLineId, TrainingId, training, details, scoreThreshold, state)
VALUES
    -- Supposons que les IDs des questions Java sont 1, 2, 3
    ...
    -- Supposons que les IDs des questions PHP sont 4, 5
