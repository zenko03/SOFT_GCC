...
namespace soft_carriere_competence.Application.Services.Evaluations
{
    public class EvaluationResponseService
    {
        ...
        public async Task<EvaluationResponses> SaveQuestionTimeAsync(int evaluationId, int questionId, int timeSpent)
        {
        ...
        public async Task<bool> UpdateProgressAsync(int evaluationId, int userId, int totalQuestions, int answeredQuestions)
        {
            try
            {
                // Récupérer l'entrée de progression existante
                ...
                else
                {
                    ...
                    progress.totalQuestions = totalQuestions > 0 ? totalQuestions : progress.totalQuestions;
                    progress.answeredQuestions = answeredQuestions;

                    // Calculer le pourcentage avec une vérification pour éviter la division par zéro
                    var total = progress.totalQuestions > 0 ? progress.totalQuestions : 1;
                    progress.progressPercentage = (decimal)progress.answeredQuestions / total * 100;

                    progress.lastUpdate = DateTime.UtcNow;

                    // Vérification explicite des changements
                    _context.Entry(progress).State = EntityState.Modified;
                }

                // Sauvegarde explicite
                var result = await _context.SaveChangesAsync();
                Console.WriteLine($"UpdateProgressAsync - Rows affected: {result}");
                return result > 0;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Erreur dans UpdateProgressAsync: {ex.Message}");
                return false;
            }
        }
