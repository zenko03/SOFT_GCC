...
function LanguageCrudPage() {
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const response = await axios.get(urlApi('/Language'));
    ...
    const handleSubmit = async (e) => {
        ...
        setError(null);
        try {
            await axios.post(urlApi('/Language'), formData);
            fetchData();
            setFormData({ name: '' });
        } catch (error) {
    ...
    const handleDeleteConfirmed = async (itemToDelete) => {
        ...
        setFormData({ name: '' });
        try {
            await axios.delete(urlApi(`/Language/${itemToDelete}`));
            fetchData();
        } catch (error) {
    ...
    const handleSubmitModified = async (e) => {
        ...
        setError(null);
        try {
            await axios.put(urlApi(`/Language/${idItem}`), formData);
            fetchData();
        } catch (error) {
            setError(`Erreur lors de l'insertion : ${error.response?.data || error.message}`);
        } finally {
    ...
    const getDataForModifiedPage = async (id) => { // Recevoir l'id en paramètre
        ...
        setError(null);
        try {
            const response = await axios.get(urlApi(`/Language/${id}`)); // Utiliser l'id passé
            setIdItem(id);
            setFormData({
    ...
    return (
        <Template>
            ...
            <div className="col-lg-12 skill-header">
                <i className="mdi mdi-book-open-variant skill-icon"></i>
                <h4 className="skill-title">ENTITÉ LANGUAGE</h4>
            </div>  
            {isLoading && <Loader />}
                                    ...
                                    <tbody>
                                        {data.map((item, id) => (
                                            <tr key={item.languageId}>
                                                <td className="text-center">{item.languageId}</td>
                                                <td>{item.name}</td>
                                                <td>
                                                    <Button
                                                        onClick={() => handleChangeToModifiedPage(item.languageId)}
                                                    ...
                                                    <Button
                                                        onClick={() => handleDeleteConfirmed(item.languageId)}
                                                        className="btn btn-danger btn-sm"
...
export default LanguageCrudPage;