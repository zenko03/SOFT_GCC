...
function SchoolCrudPage() {
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const response = await axios.get(urlApi('/School'));
    ...
    const handleSubmit = async (e) => {
        ...
        setError(null);
        try {
            await axios.post(urlApi('/School'), formData);
            fetchData();
            setFormData({ name: '' });
        } catch (error) {
    ...
    const handleDeleteConfirmed = async (itemToDelete) => {
        ...
        setFormData({ name: '' });
        try {
            await axios.delete(urlApi(`/School/${itemToDelete}`));
            fetchData();
        } catch (error) {
    ...
    const handleSubmitModified = async (e) => {
        ...
        setError(null);
        try {
            await axios.put(urlApi(`/School/${idItem}`), formData);
            fetchData();
        } catch (error) {
            setError(`Erreur lors de l'insertion : ${error.response?.data || error.message}`);
        } finally {
    ...
    const getDataForModifiedPage = async (id) => { // Recevoir l'id en paramètre
        ...
        setError(null);
        try {
            const response = await axios.get(urlApi(`/School/${id}`)); // Utiliser l'id passé
            setIdItem(id);
            setFormData({
    ...
    return (
        <Template>
            ...
            <div className="col-lg-12 skill-header">
                ...
                <h4 className="skill-title">ENTITÉ ECOLE</h4>
            </div>  
            {isLoading && <Loader />}
                                    ...
                                    <tbody>
                                        {data.map((item, id) => (
                                            <tr key={item.schoolId}>
                                                <td className="text-center">{item.schoolId}</td>
                                                <td>{item.name}</td>
                                                <td>
                                                    <Button
                                                        onClick={() => handleChangeToModifiedPage(item.schoolId)}
                                                    ...
                                                    <Button
                                                        onClick={() => handleDeleteConfirmed(item.schoolId)}
                                                        className="btn btn-danger btn-sm"
