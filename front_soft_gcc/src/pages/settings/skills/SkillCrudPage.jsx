...
function SkillCrudPage() {
    ...
    const handleSubmitModified = async (e) => {
        ...
        setError(null);
        try {
            await axios.put(urlApi(`/Skill/${idItem}`), formData);
            fetchData();
        } catch (error) {
            setError(`Erreur lors de l'insertion : ${error.response?.data || error.message}`);
        } finally {
    ...
    const getDataForModifiedPage = async (id) => { // Recevoir l'id en paramètre
        ...
        setError(null);
        try {
            const response = await axios.get(urlApi(`/Skill/${id}`)); // Utiliser l'id passé
            setIdItem(id);
            setFormData({
    ...
    return (
        <Template>
            ...
            <div className="col-lg-12 skill-header">
                <i className="mdi mdi-star-circle skill-icon"></i>
                <h4 className="skill-title">ENTITÉ COMPETENCE</h4>
            </div> 
            {isLoading && <Loader />}
                                    ...
                                    <tbody>
                                        {data.map((item, id) => (
                                            <tr key={item.skillId}>
                                                ...
                                                <td>
                                                    <Button
                                                        onClick={() => handleChangeToModifiedPage(item.skillId)}
