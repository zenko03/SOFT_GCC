...
function PaymentMethodCrudPage() {
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const response = await axios.get(urlApi('/PaymentMethod'));
    ...
    const handleSubmit = async (e) => {
        ...
        setError(null);
        try {
            await axios.post(urlApi('/PaymentMethod'), formData);
            fetchData();
    ...
    const handleDeleteConfirmed = async (itemToDelete) => {
        ...
        setError(null);
        try {
            await axios.delete(urlApi(`/PaymentMethod/${itemToDelete}`));
            fetchData();
    ...
    const handleSubmitModified = async (e) => {
        ...
        setError(null);
        try {
            await axios.put(urlApi(`/PaymentMethod/${idItem}`), formData);
            fetchData();
        } catch (error) {
            setError(`Erreur lors de l'insertion : ${error.response?.data || error.message}`);
        } finally {
    ...
    const getDataForModifiedPage = async (id) => { 
        ...
        setError(null);
        try {
            const response = await axios.get(urlApi(`/PaymentMethod/${id}`)); 
            setIdItem(id);
            setFormData({
    ...
    return (
        <Template>
            ...
            <div className="col-lg-12 skill-header">
                <i className="mdi mdi-credit-card skill-icon"></i>
                <h4 className="skill-title">ENTITÉ METHODE DE PAIEMENT</h4>
            </div> 
            {isLoading && <Loader />}
                                ...
                                <form className="forms-sample" onSubmit={handleSubmitModified}>
                                    ...
                                    <div className="card-body">
                                        <div className="form-group">
                                            <label htmlFor="paymentMethodName">Désignation</label>
                                            <input type="text" name="paymentMethodName" value={formData.paymentMethodName} onChange={handleChange} className="form-control" id="name" required />
                                        </div>
                                        <div className="button-save-profil">
                                            <button type="submit" className="btn btn-success btn-fw" disabled={isLoading}>Modifier</button>
                                            <button type="reset" className="btn btn-light btn-fw" onClick={() => setFormData({ paymentMethodName: '' })}>Annuler</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                                    ...
                                    <tbody>
                                        {data.map((item, id) => (
                                            <tr key={item.paymentMethodId}>
                                                <td className="text-center">{item.paymentMethodId}</td>
                                                <td>{item.paymentMethodName}</td>
                                                <td>
                                                    <Button
                                                        onClick={() => handleChangeToModifiedPage(item.paymentMethodId)}
                                                    ...
                                                    <Button
                                                        onClick={() => handleDeleteConfirmed(item.paymentMethodId)}
                                                        className="btn btn-danger btn-sm"
...
export default PaymentMethodCrudPage;