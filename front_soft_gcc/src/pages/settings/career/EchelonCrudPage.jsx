...
function EchelonCrudPage() {
    ...
    const handleSubmitModified = async (e) => {
        ...
        setError(null);
        try {
            await axios.put(urlApi(`/Echelon/${idItem}`), formData);
            fetchData();
        } catch (error) {
            setError(`Erreur lors de l'insertion : ${error.response?.data || error.message}`);
        } finally {
    ...
    const getDataForModifiedPage = async (id) => { 
        ...
        setError(null);
        try {
            const response = await axios.get(urlApi(`/Echelon/${id}`)); 
            setIdItem(id);
            setFormData({
                echelonId: response.data.echelonId,
                echelonName: response.data.echelonName 
            });
        } catch (err) {
            setError(`Erreur lors de la récupération des données : ${err.message}`);
        } finally {
                                    ...
                                    <tbody>
                                        {data.map((item, id) => (
                                            <tr key={item.echelonId}>
                                                ...
                                                <td>
                                                    <Button
                                                        onClick={() => handleChangeToModifiedPage(item.echelonId)}
