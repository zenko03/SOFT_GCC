...
function EmployeeTypeCrudPage() {
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const response = await axios.get(urlApi('/EmployeeType'));
    ...
    const handleSubmit = async (e) => {
        ...
        setError(null);
        try {
            await axios.post(urlApi('/EmployeeType'), formData);
            fetchData();
    ...
    const handleDeleteConfirmed = async (itemToDelete) => {
        ...
        setError(null);
        try {
            await axios.delete(urlApi(`/EmployeeType/${itemToDelete}`));
            fetchData();
    ...
    const handleSubmitModified = async (e) => {
        ...
        setError(null);
        try {
            await axios.put(urlApi(`/EmployeeType/${idItem}`), formData);
            fetchData();
        } catch (error) {
            setError(`Erreur lors de l'insertion : ${error.response?.data || error.message}`);
        } finally {
    ...
    const getDataForModifiedPage = async (id) => { 
        ...
        setError(null);
        try {
            const response = await axios.get(urlApi(`/EmployeeType/${id}`)); 
            setIdItem(id);
            setFormData({
                employeeTypeId: response.data.employeeTypeId,
                employeeTypeName: response.data.employeeTypeName 
            });
        } catch (err) {
            setError(`Erreur lors de la récupération des données : ${err.message}`);
        } finally {
    ...
    return (
        <Template>
            ...
            <div className="col-lg-12 skill-header">
                ...
                <h4 className="skill-title">ENTITÉ TYPE D'EMPLOYÉ</h4>
            </div> 
            {isLoading && <Loader />}
                                ...
                                <form className="forms-sample" onSubmit={handleSubmitModified}>
                                    ...
                                    <div className="card-body">
                                        <div className="form-group">
                                            <label htmlFor="employeeTypeName">Désignation</label>
                                            <input type="text" name="employeeTypeName" value={formData.employeeTypeName} onChange={handleChange} className="form-control" id="name" required />
                                        </div>
                                        <div className="button-save-profil">
                                            <button type="submit" className="btn btn-success btn-fw" disabled={isLoading}>Modifier</button>
                                            <button type="reset" className="btn btn-light btn-fw" onClick={() => setFormData({ employeeType: '' })}>Annuler</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                        ...
                        <div className="card">
                            ...
                            <div className="card-body">
                                <table className="table table-hover table-bordered">
                                    ...
                                    <tbody>
                                        {data.map((item, id) => (
                                            <tr key={item.employeeTypeId}>
                                                <td className="text-center">{item.employeeTypeId}</td>
                                                <td>{item.employeeTypeName}</td>
                                                <td>
                                                    <Button
                                                        onClick={() => handleChangeToModifiedPage(item.employeeTypeId)}
                                                    ...
                                                    <Button
                                                        onClick={() => handleDeleteConfirmed(item.employeeTypeId)}
                                                        className="btn btn-danger btn-sm"
...
export default EmployeeTypeCrudPage;