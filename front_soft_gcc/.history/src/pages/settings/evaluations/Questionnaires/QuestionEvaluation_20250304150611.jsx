...
function QuestionEvaluation() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        console.log(formData); // Vérifiez les valeurs ici
        try {
            if (currentQuestion) {
                // Update existing question
                await axios.put(`https://localhost:7082/api/Evaluation/questions/${currentQuestion.questiondId}`, {
                    question: formData.question,
                    evaluationTypeId: Number(formData.evaluationTypeId), // Assurez-vous que c'est un nombre
                    postId: Number(formData.postId) // Assurez-vous que c'est un nombre
                });
            } else {
                // Create new question
    ...
    // Application des filtres avec conversion en Number()
