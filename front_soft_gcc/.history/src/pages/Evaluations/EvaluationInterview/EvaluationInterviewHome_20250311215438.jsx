...
function EvaluationInterviewHome() {
  ...
  const renderActionButton = useCallback((employee) => {
    ...
    const canManagerValidate =
      isManager &&
      employee.interviewStatus === 25 && // PendingValidation
    ...
    // 1. Vérifier les statuts explicites (d'après votre enum InterviewStatus)
    if (employee.interviewStatus === 30) { // Completed
      return <span className="text-success">Entretien terminé</span>;
    }

    if (employee.interviewStatus === 35) { // Rejected
      return <span className="text-danger">Entretien rejeté</span>;
    }

    // 2. Entretiens en attente de validation
    if (employee.interviewStatus === 25) { // PendingValidation
      if (isRH) {
    ...
    if (employee.interviewStatus === 20) { // InProgress
      if (isRH) {
    ...
    // 4. Gestion des entretiens planifiés (avec date)
    if (employee.interviewStatus === 15) { // Planned
      // Si date passée
      if (isPastDate) {
        if (isRH) {
          return (
      ...
      if (isFutureDate) {
        return (
          <button
            ...
            onClick={() => handleOpenModal(employee.employeeId)}
            aria-label={`Planifier l'entretien pour ${employee.firstName} ${employee.lastName}`}
          >
            Planifier
          </button>
        );
    ...
    return null; // Aucun état applicable
  }, [user, today]);
