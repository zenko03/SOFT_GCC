...
const ParticipantsSelector = ({ participantsList, selectedParticipants, setSelectedParticipants, selectedEmployee }) => {
  ...
  useEffect(() => {
    if (
      selectedParticipants.includes(selectedEmployee.id) &&
      !participantsList.some((p) => p.id === selectedEmployee.id)
    ) {
      participantsList.push(selectedEmployee); // Ajoute l'utilisateur sélectionné
    }
  }, [selectedParticipants, participantsList, selectedEmployee]);
