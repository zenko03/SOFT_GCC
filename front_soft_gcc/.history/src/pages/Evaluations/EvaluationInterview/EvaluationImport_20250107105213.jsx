...
const EvaluationImport = ({ onExtract, interview, employeeId }) => {
    ...
    const parseExtractedData = (rawText) => {
        ...
        lines.forEach((line) => {
            if (line.toLowerCase().includes('employeeid')) {
                data.employeeId = line.split(':')[1]?.trim();
            } else if (line.toLowerCase().includes('employeename')) {
                data.employeeName = line.split(':')[1]?.trim();
            } else if (line.toLowerCase().includes('position')) {
                data.position = line.split(':')[1]?.trim();
            } else if (line.toLowerCase().includes('evaluationdate')) {
                data.evaluationDate = line.split(':')[1]?.trim();
            }
        });
