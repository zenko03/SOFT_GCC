...
function EvaluationInterviewHome() {
  ...
  const handleOpenModal = () => {
    setShowModal(true);
    const selectedEmployee = selectedEmployees[0]; // On suppose qu'on ne sélectionne qu'un seul employé pour l'instant
    const employeeEvaluation = employees.find(emp => emp.employeeId === selectedEmployee);
    
    setEvaluationDetails({
      evaluationId: employeeEvaluation.evaluationId,
      scheduledDate: '',
  ...
  const handleMassPlanning = async () => {
    if (!evaluationDetails.evaluationId || !evaluationDetails.scheduledDate || evaluationDetails.participants.length === 0) {
              ...
              <tbody>
                {filteredEmployees.length > 0 ? (
                  filteredEmployees.map((employee) => {
                    ...
                    const interviewDate = employee.interviewDate; // La date d'entretien prévue pour l'employé
                      ...
                      <tr key={employee.employeeId}>
                        ...
                        <td>
                          {interviewDate ? (
                            ...
                            ) : new Date(interviewDate) > new Date(today) ? null : null
                          ) : (
                            <button
                ...
                <div className="modal-body">
                  <form>
                    <div className="form-group">
                      <label htmlFor="evaluationId">ID de l'évaluation</label>
                      <input
                        type="number"
                        id="evaluationId"
                        className="form-control"
                        value={evaluationDetails.evaluationId}
                        onChange={(e) => handleEvaluationDetailsChange({ name: 'evaluationId', value: e.target.value })}
                      />
                    </div>
                    <div className="form-group">
                    ...
                    <div className="form-group">
                      ...
                      <select
                        ...
                        className="form-control"
                        multiple
                        value={evaluationDetails.participants || []}
                        onChange={(e) => handleEvaluationDetailsChange({ name: 'participants', value: Array.from(e.target.selectedOptions).map(option => parseInt(option.value, 10)) })}
                      >
