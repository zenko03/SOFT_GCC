...
function EvaluationInterviewHome() {
  ...
  const handleMassPlanning = async () => {
    if (!evaluationDetails.evaluationId) {
      alert('Impossible de planifier : aucun ID d’évaluation valide.');
      return;
    }

    if (!evaluationDetails.scheduledDate) {
      alert('Veuillez sélectionner une date pour l’entretien.');
      return;
    }

    if (evaluationDetails.participants.length === 0) {
      alert('Veuillez sélectionner au moins un participant avant de planifier.');
      return;
    }
    ...
    try {
      ...
      fetchEmployeesWithoutEvaluations(); // Rafraîchir la liste
      setSelectedEmployees([]); // Réinitialiser la sélection
      handleCloseModal();
    } catch (error) {
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleEvaluationDetailsChange({
                            name: 'participants',
                            value: Array.from(e.target.selectedOptions, (option) => parseInt(option.value, 10)),
                          })
                        }
                      >
