...
const EvaluationImport = ({ interview, employeeId, onExtract }) => {
    ...
    const parseDataFromXML = (xmlContent) => {
        // Récupère tout le texte dans les balises <w:t>
        const textContent = Array.from(xmlContent.matchAll(/<w:t[^>]*>(.*?)<\/w:t>/g))
            .map(match => match[1])
            .join(' '); // Concatène tout le texte avec des espaces

        console.log('Texte brut extrait:', textContent);
        ...
        Object.keys(matches).forEach((key) => {
            const match = textContent.match(matches[key]);
