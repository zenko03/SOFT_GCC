...
const EvaluationImport = ({ interview, employeeId, onExtract }) => {
    ...
    const parseDataFromXML = (xmlContent) => {
        // Utilisation d'un parser DOM pour extraire le contenu XML
        const parser = new DOMParser();
        const xmlDoc = parser.parseFromString(xmlContent, "application/xml");

        // Extraire tous les nœuds <w:t> (contenus de texte dans Word)
        ...
        // Recherche des informations dans le texte extrait
        const data = {};
        const matches = {
        ...
        Object.keys(matches).forEach((key) => {
            const match = extractedText.match(matches[key]);
