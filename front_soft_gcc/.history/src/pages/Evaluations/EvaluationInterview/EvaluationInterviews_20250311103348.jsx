...
const EvaluationInterviews = () => {
    ...
    useEffect(() => {
        if (!state) {
            // Si aucun état n'a été transmis via la navigation
        ...
        if (user === null) {
            const checkUser = setTimeout(() => {
                // Si après un délai l'utilisateur n'est toujours pas chargé
                if (!user) {
                    toast.error('Vous devez être connecté pour accéder à cette page');
                    navigate('/login'); // Redirection vers la page de connexion
                }
            }, 2000); // Attente de 2 secondes pour laisser le temps au contexte de charger

            return () => clearTimeout(checkUser); // Nettoyage du timeout
        } else {
