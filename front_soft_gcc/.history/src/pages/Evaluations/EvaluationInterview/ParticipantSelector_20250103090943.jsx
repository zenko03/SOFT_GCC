...
const ParticipantsSelector = ({ participantsList, selectedParticipants, setSelectedParticipants, selectedEmployee }) => {
  ...
  useEffect(() => {
    // Ajouter automatiquement l'employé sélectionné à la liste des participants s'il n'y est pas déjà
    if (
      selectedEmployee &&
  ...
  useEffect(() => {
    if (
      ...
      if (existsInList) {
        ...
        console.warn(`Employee with ID ${selectedEmployee.employeeId} not found in participantsList`);
      }
    }
  }, [selectedEmployee, participantsList, selectedParticipants, setSelectedParticipants]);
