...
const ParticipantsSelector = ({
  ...
  useEffect(() => {
    console.log("Vérification de l'employé sélectionné :", selectedEmployee);
    if (
      selectedEmployee &&
      selectedEmployee.employeeId &&
      !selectedParticipants.includes(selectedEmployee.employeeId)
    ) {
      console.log("Ajout automatique de l'employé :", selectedEmployee.employeeId);
  ...
  const handleSelectParticipant = (index, value) => {
    const participantId = value ? parseInt(value, 10) : null;
    console.log(`Participant sélectionné (index ${index}):`, participantId);
    if (participantId && !isNaN(participantId) && !selectedParticipants.includes(participantId)) {
      setSelectedParticipants((prev) => {
        const updated = [...prev, participantId];
        console.log("Participants après ajout :", updated);
        return updated;
      });
    }
    setDropdowns((prev) =>
      prev.map((v, i) => (i === index ? participantId : v))
    );
  };

  // Ajouter une nouvelle liste déroulante
  const addDropdown = () => {
    console.log("Ajout d'une nouvelle liste déroulante");
    setDropdowns((prev) => [...prev, null]);
  };

  // Retirer un participant
  const handleRemoveParticipant = (participantId) => {
    console.log("Suppression du participant :", participantId);
    setSelectedParticipants((prev) => {
      const updated = prev.filter((id) => id !== participantId);
      console.log("Participants après suppression :", updated);
      return updated;
    });
    setDropdowns((prev) =>
      prev.filter((v) => v !== participantId)
    );
  };
