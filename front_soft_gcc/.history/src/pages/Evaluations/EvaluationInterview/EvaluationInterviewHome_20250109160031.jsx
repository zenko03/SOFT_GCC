                ...
                {filteredEmployees.length > 0 ? (
                  filteredEmployees.map((employee) => {
                    ...
                    const canManagerValidate =
                      isManager &&
                      (employee.managerApproval === null || employee.managerComments === null && employee.interviewStatus===25);
                    const canDirectorValidate =
                      isDirector &&
                      (employee.directorApproval === null || employee.directorComments === null && employee.interviewStatus===25);
                    const interviewCompleted =
                      employee.managerApproval !== null &&
                      employee.managerComments !== null &&
                      employee.directorApproval !== null &&
                      employee.directorComments !== null;

                    const isToday = compareDates(employee.interviewDate, today); // Vérifie si la date est aujourd'hui
                    ...
                    return (
                      <tr key={employee.employeeId}>
                        ...
                        <td>
                          {employee.interviewStatus === 25 ? ( // Priorité à "En attente de validation"
                            <span className="text-warning">En attente de validation</span>
                          ) : interviewCompleted ? (
                            <span className="text-success">Entretien terminé</span>
                          ) : canManagerValidate || canDirectorValidate ? (
                            <button
                              className="btn btn-primary btn-sm"
                              onClick={() => handleValidation(employee.employeeId)}
                            >
                              Valider
                            </button>
                          ) : isToday ? ( // Affiche "Démarrer l'entretien" uniquement si date est aujourd'hui
                            <button
