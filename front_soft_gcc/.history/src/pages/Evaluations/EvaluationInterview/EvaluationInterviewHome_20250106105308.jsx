...
function EvaluationInterviewHome() {
  ...
  const startInterview = async (employeeId,navigate) => {
    ...
    try {
      ...
      const interviewResponse = await fetch(`https://localhost:7082/api/EvaluationInterview/get-interview-by-participant/${employeeId}`);
      if (!interviewResponse.ok) {
        const error = await interviewResponse.json();
        console.error(`Erreur lors de la récupération de l'entretien : ${error.message}`);
        alert("Impossible de récupérer l'entretien pour cet employé.");
        return;
      }
      ...
      const { interviewId, state } = interview;
      ...
      const startResponse = await fetch(`https://localhost:7082/api/EvaluationInterview/start-interview/${interviewId}`, {
        method: "PUT",
      });
  
      if (!startResponse.ok) {
        const error = await startResponse.json();
        console.error(`Erreur lors du démarrage de l'entretien : ${error.message}`);
        alert("Impossible de démarrer l'entretien.");
        return;
      }
  
      // Succès : Rediriger vers EvaluationInterviews avec les données nécessaires
      alert("L'entretien a été démarré avec succès !");
      navigate("/evaluation-interviews", { state: { interview, employeeId } });
    } catch (error) {
