...
function EvaluationInterviewHome() {
  ...
  const startInterview = async (employeeId) => {
  
    try {
      // Étape 1 : Récupérer l'ID de l'entretien à partir de l'ID de l'employé
      const interviewResponse = await fetch(`/api/EvaluationInterview/get-interview-by-participant/${employeeId}`);
      ...
      const startResponse = await fetch(`/api/EvaluationInterview/start-interview/${interviewId}`, {
        method: "PUT",
      });
