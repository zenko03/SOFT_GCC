...
import { 
  ...
  FormControl, FormLabel, TextField, CircularProgress, Alert, Divider 
} from '@mui/material';
import { 
  FaUser, FaCalendarAlt, FaClipboardList, FaCheck, 
  FaArrowRight, FaArrowLeft, FaSave, FaPaperPlane, 
  FaExclamationTriangle, FaSpinner, FaStar, FaStarHalfAlt, FaRegStar 
} from 'react-icons/fa';
...
const EvaluationPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Convertir les réponses en format attendu par l'API
      const formattedAnswers = Object.keys(answers).map(questionId => ({
        questionId: parseInt(questionId),
        score: parseInt(answers[questionId]),
      }));
      
      // Calculer le score moyen const averageScore = formattedAnswers.reduce((acc, curr) => acc + curr.score, 0) / formattedAnswers.length;
