...
function Step3({ generalScore, ratings }) {
  ...
  const fetchSuggestions = async () => {
    try {
      const response = await axios.get(`https://localhost:7082/api/Evaluation/suggestions?average=${average}`);
      console.log("Suggestions de formation :", response.data);
    } catch (error) {
  ...
  const handleValidation = async () => {
    try {
      const response = await axios.post('https://localhost:7082/api/Evaluation/validate', {
