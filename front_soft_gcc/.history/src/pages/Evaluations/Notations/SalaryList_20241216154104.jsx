...
function SalaryList() {
  ...
  const saveEvaluationResults = async () => {
    ...
    const data = {
      evaluationId: selectedEmployee.evaluationId, // L'évaluation liée à l'employé
      ratings, // Assurez-vous que ratings est bien au format { questionId: score }
      overallScore: remarks.overallScore, // Le score global
      strengths: remarks.strengths, // Points forts
      weaknesses: remarks.weaknesses, // Points faibles
      generalEvaluation: remarks.generalEvaluation, // Évaluation générale
    };
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        ...
        console.log("employee recuperer DATA: " + response.data);
  ...
  // Ajoutez cet état pour contrôler quand afficher le modal
  ...
  const handleOpenModal = async (employeeId) => {
    try {
      ...
      // Une fois les données de l'employé récupérées, définissez isEmployeeLoaded à true
  ...
  // Réinitialisez l'état isEmployeeLoaded lorsque vous fermez le modal
  const handleCloseModal = () => {
    setShowModal(false);
    setIsEmployeeLoaded(false); // Réinitialiser
  };
  const handleSaveRemarks = (newRemarks) => {
    setRemarks(newRemarks); // Mettre à jour l'état global
  };
