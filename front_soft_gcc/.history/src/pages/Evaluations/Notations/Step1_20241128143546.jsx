...
import EvaluationService from "../../../services/Evaluations/EvaluationService"; // Importation correcte du service
...
function Step1({ notes, setNotes, employeePostId }) {
  ...
  useEffect(() => {
    alert('Not a bad idea '+employeePostId);
    console.log(employeePostId);
  })
  ...
  useEffect(() => {
    const loadEvaluationTypes = async () => {
      try {
        ...
        alert("Types d'évaluation récupérés :", types); // Débogage
  ...
  console.log("Détails de l'employé sélectionné :", selectedEmployees);
  ...
  const handleEvaluationTypeClick = async (evaluationTypeId) => {
    ...
    setLoading(true);
    try {
      const fetchedQuestions = await EvaluationService.fetchEvaluationQuestions(evaluationTypeId, employeePostId); // Utilisation correcte
  ...
  console.log("Types d'évaluation dans le render :", evaluationTypes);
          ...
          <ul className="modal-menu-list">
            {/* {evaluationTypes.map((type) => (
              <li
                ...
                onClick={() => handleEvaluationTypeClick(type.evaluationTypes.evaluationTypeId)}
              >
                {type.designation}
              </li>
            ))} */}
