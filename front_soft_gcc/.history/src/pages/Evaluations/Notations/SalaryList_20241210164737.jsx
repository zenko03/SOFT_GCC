...
function SalaryList() {
  ...
  const saveEvaluationResult = async () => {
    try {
      ...
      const response = await axios.post('https://localhost:7082/api/Evaluation/save-evaluations-result', evaluationData);
      console.log('Évaluation sauvegardée:', response.data);
    } catch (error) {
      console.error('Erreur lors de la sauvegarde de l\'évaluation:', error);
    }
  };

  // Récupérer les suggestions de formation
  const fetchTrainingSuggestions = async () => {
    try {
      const response = await axios.get('https://localhost:7082/api/Evaluation/suggestions');
      setTrainingSuggestions(response.data);
    } catch (error) {
      console.error('Erreur lors de la récupération des suggestions de formation:', error);
    }
  };
  ...
  const validateEvaluation = async () => {
    try {
      const response = await axios.post('https://localhost:7082/api/Evaluation/validate-evaluations', {
        employeeId: selectedEmployee.id,
        evaluationType: selectedEvaluationType,
      });
      console.log('Évaluation validée:', response.data);
    } catch (error) {
    ...
    switch (currentStep) {
      ...
      case 3:
        return <Step3
          ...
          saveEvaluationResult={saveEvaluationResult}
