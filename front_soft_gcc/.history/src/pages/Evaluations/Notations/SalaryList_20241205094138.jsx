...
function SalaryList() {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const response = await axios.get('http://localhost:5189/api/User');
  ...
  const handleOpenModal = async (employeeId) => {
    try {
      const response = await axios.get(`https://localhost:5189/api/User/${employeeId}`);
      const employeeData = response.data;

      const evalResponse = await axios.get('https://localhost:5189/api/Evaluation/types');
