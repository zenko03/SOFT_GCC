...
function SalaryList() {
  ...
  const handleNextStep = async () => {
    if (currentStep === 2) {
      try {
        await axios.post('/api/Evaluation/save-evaluation-results', {
  ...
  const handleValidateEvaluation = async () => {
    try {
      await axios.post('/api/Evaluation/validate-evaluation', {
        employeeId: selectedEmployee.id,
        ratings,
        remarks,
      });
      console.log('Évaluation validée avec succès.');
          ...
          <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable custom-modal" role="document">
            <div className="modal-content">
              ...
              <div className="modal-footer">
                ...
                {currentStep < 3 && (
                  <button type="button" className="btn btn-primary" onClick={handleNextStep} disabled={!allQuestionsRated()}>
                    Suivant
                  </button>
                )}
                {currentStep === 3 && (
                  <button type="button" className="btn btn-success" onClick={handleValidateEvaluation}>
                    Valider
                  </button>
                )}
